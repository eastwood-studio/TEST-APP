# operations_module.py — v3.18 — 1778059674
# ══════════════════════════════════════════════════════════════════════════════
...
def page_operations(can_fn, DB_PATH, fmt_eur_fn=None):
    ...
    with tab_hist:
        ...
        if df_ops.empty:
            ...
            # Formater les montants (sans décimales inutiles) et la quantité
            for _mc in ["Total HT","Total TTC","Montant TVA","Frais envoi"]:
            ...
            if "🔗 Facture Drive" in df_styled.columns and "Catégorie" in df_styled.columns:
                df_styled["🔗 Facture Drive"] = df_styled.apply(
                    lambda row: (
                        row["🔗 Facture Drive"] if (row.get("🔗 Facture Drive","") or "").strip()
                        else ("⚠ Facture manquante" if str(row.get("Catégorie","") or "") in _CATS_NEED_FAC else "—")
                    ), axis=1)
            ...
            _CHAMPS_OBLIGATOIRES = {
                ...
                "🔗 Facture Drive": lambda v: str(v or "").strip() not in ("","None","nan","⚠ Facture manquante","—"),
            ...
            # Rendre le lien facture cliquable
            if "🔗 Facture Drive" in df_styled.columns:
                df_styled["🔗 Facture Drive"] = df_styled["🔗 Facture Drive"].apply(
                    lambda v: f'<a href="{v}" target="_blank">📄 Voir</a>'
                    if v and str(v).startswith("http") else (
                    '<span style="color:#c1440e;font-size:10px;">⚠ Manquante</span>'
                    if "manquante" in str(v).lower() else str(v or "—")))
