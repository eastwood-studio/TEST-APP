# operations_module.py — v3.12 — 1777008251
# ══════════════════════════════════════════════════════════════════════════════
        ...
        with tab_new:
            ...
            if type_op == "Vente":
                ...
                for _vi in range(int(n_art_v)):
                    ...
                    _va1,_va2,_va3 = st.columns([3,1,2])
                    ...
                    articles_vente.append({"ref":_vref,"nom":_vnom,"qte":_vqte})

                _all_refs_v = ", ".join(a["ref"] for a in articles_vente if a["ref"])
                _all_noms_v = ", ".join(f"{a['qte']}× {a['nom']}" for a in articles_vente if a["nom"])
                if _all_refs_v:
...
{orig_str}
            ...
            if 'articles_vente' not in dir():
                articles_vente = []
            # Pour vente multi-articles, construire ref_produit et info_process
            if type_op == "Vente" and articles_vente:
                ...
                if _arts_filled:
                    ...
                    info_process = ", ".join(f"{a['qte']}x {a['nom']}" for a in _arts_filled if a.get("nom"))
