# operations_module.py — v3.7 — 1776676059
# ══════════════════════════════════════════════════════════════════════════════
                        ...
                        for _, row_o in df_imp_ops.iterrows():
                            try:
                                ...
                                def _clean_num(v, default=0):
                                    """Nettoie une valeur numérique : supprime €, espaces, remplace , par ."""
                                    try:
                                ...
                                ht    = _clean_num(row_o.get("total_ht",   0))
                                ttc   = _clean_num(row_o.get("total_ttc",  0))
                                tva_v = _clean_num(row_o.get("tva",         0))
                                prix_u= _clean_num(row_o.get("prix_unitaire",0))
                                qte   = _clean_num(row_o.get("quantite",   1), default=1) or 1
                                ...
                                # Assurer que les colonnes article et source existent
                                try:
                                    conn.execute("ALTER TABLE transactions ADD COLUMN article TEXT DEFAULT ''")
                                    conn.commit()
                                except Exception: pass
                                try:
                                    conn.execute("ALTER TABLE transactions ADD COLUMN source TEXT DEFAULT ''")
                                    conn.commit()
                                except Exception: pass
