# operations_module.py — v3.20 — 1778083655
# ══════════════════════════════════════════════════════════════════════════════
...
def ensure_columns(conn):
    ...
    new_cols = [
        ...
        ("lien_facture",     "TEXT DEFAULT ''"),   # Lien Google Drive / facture
        ("num_suivi",        "TEXT DEFAULT ''"),   # N° suivi / commande / facture
        ("fournisseur_op",   "TEXT DEFAULT ''"),   # Fournisseur lié à l'opération
    ]
...
def page_operations(can_fn, DB_PATH, fmt_eur_fn=None):
    ...
    with tab_hist:
        ...
        if df_ops.empty:
            ...
            if "lien_facture" not in df_display.columns:
                df_display["lien_facture"] = ""
            if "num_suivi" not in df_display.columns:
                df_display["num_suivi"] = ""
            if "fournisseur_op" not in df_display.columns:
                df_display["fournisseur_op"] = ""
            if "prix_unitaire" not in df_display.columns:
                df_display["prix_unitaire"] = df_display.get("prix_unitaire", 0.0)
            if "info_complementaire" not in df_display.columns:
                df_display["info_complementaire"] = ""
            ...
            # Réinitialiser df_display depuis le df_ops rechargé
            df_display = df_ops.copy()
            for _col, _def in [
                ("frais_envoi", 0.0), ("collection_op", ""), ("info_complementaire", ""),
                ("lien_facture", ""), ("num_suivi", ""), ("fournisseur_op", ""), ("prix_unitaire", 0.0)]:
                if _col not in df_display.columns:
                    df_display[_col] = _def
            cols_show = [
                "num_operation","date_op","description","categorie","collection_op",
                "info_process","ref_produit","type_op","devise","type_tva",
                "quantite","prix_unitaire","total_ht","frais_envoi","total_ttc","tva",
                "payeur","beneficiaire","info_complementaire",
                "fournisseur_op","num_suivi","lien_facture"
            ]
            if "date_op" in df_display.columns:
                df_display["date_op"] = df_display["date_op"].apply(fmt_date_fr)
            cols_rename = {
                "num_operation":"N°","date_op":"Date",
                "description":"Description","categorie":"Catégorie",
                "collection_op":"Collection","info_process":"Article",
                "ref_produit":"SKU","type_op":"Type","devise":"Devise",
                "type_tva":"Type TVA","quantite":"Qté","prix_unitaire":"PU",
                "total_ht":"HT","frais_envoi":"Frais envoi",
                "total_ttc":"TTC","tva":"TVA",
                "payeur":"Payeur","beneficiaire":"Bénéficiaire",
                "info_complementaire":"Info complémentaire",
                "fournisseur_op":"Fournisseur","num_suivi":"N° Suivi",
                "lien_facture":"🔗 Facture"
            }
            ...
            # Formater les montants (sans décimales inutiles)
            for _mc in ["HT","TVA","TTC","Frais envoi","PU"]:
                if _mc in df_styled.columns:
                    df_styled[_mc] = df_styled[_mc].apply(
                        lambda v: "—" if (v is None or str(v).strip() in ("","nan","0","0.0","None")) else
                        (f"{int(float(v)):,} €".replace(",","‎ ") if float(v)==int(float(v)) else f"{float(v):,.2f} €".replace(",","‎ "))
                        if v not in (None,"") else "—")
            if "Qté" in df_styled.columns:
                df_styled["Qté"] = df_styled["Qté"].apply(
                    lambda v: "" if not v else (str(int(float(v))) if float(v)==int(float(v)) else str(v))
                    if v not in (None,"") else "")

            # Alertes facture manquante (catégories qui nécessitent une facture)
            _CATS_NEED_FAC = ("Matières premières","Matière première","Composants","Composant",
                              "Transport / Logistique","Confection / Production","Production / Confection",
                              "Communication","Stockage","Légal / Administratif")
            if "🔗 Facture" in df_styled.columns and "Catégorie" in df_styled.columns:
                df_styled["🔗 Facture"] = df_styled.apply(
                    lambda row: (
                        row.get("🔗 Facture","") if str(row.get("🔗 Facture","") or "").strip() not in ("","—","None","nan")
                        else ("⚠ Manquante" if str(row.get("Catégorie","") or "") in _CATS_NEED_FAC else "—")
                    ), axis=1)
            ...
            _CHAMPS_OBLIGATOIRES = {
                ...
                "🔗 Facture": lambda v: str(v or "").strip() not in ("","None","nan","⚠ Manquante","—"),
            ...
            # Traiter le lien facture avant l'affichage
            if "🔗 Facture" in df_styled.columns:
                df_styled["🔗 Facture"] = df_styled["🔗 Facture"].apply(
                    lambda v: "📄 Voir →" if v and str(v).startswith("http")
                    else ("⚠ Manquante" if "manquante" in str(v).lower() else str(v or "—")))
            ...
            if _can_write and not _can_read_only:
                with st.expander("✏️ Modifier / Supprimer une opération"):
                    ...
                    with st.form(f"edit_op_{sel_id}"):
                        ...
                        e_num_suivi  = st.text_input("N° Suivi / Commande / Facture", value=str(row_edit.get("num_suivi","") or ""), placeholder="ex: FAC-2025-001")
                        e_four_op    = st.text_input("Fournisseur", value=str(row_edit.get("fournisseur_op","") or ""))
                        e_lien_fac   = st.text_input("🔗 Lien Google Drive (facture)", value=str(row_edit.get("lien_facture","") or ""), placeholder="https://drive.google.com/...")
                        ...
                        with save_col:
                            if st.form_submit_button("💾 Enregistrer", type="primary"):
                                ...
                                for col, val in [
                                    ...
                                    ("source",            e_drive_link),
                                    ("num_suivi",         e_num_suivi),
                                    ("fournisseur_op",    e_four_op),
                                    ("lien_facture",      e_lien_fac),
                                ]:
  ...
  <span style="font-family:'DM Mono',monospace;font-size:11px;color:{EW_B};margin-left:auto;">
    ...
    if tab_new is not None:
        with tab_new:
            ...
            # ── Numéro de suivi + lien facture ───────────────────────────────
            _nsc1, _nsc2 = st.columns(2)
            with _nsc1:
                new_num_suivi = st.text_input("N° Suivi / Commande / Facture",
                    placeholder="ex: FAC-2025-001 / TRK123456",
                    key="op_num_suivi")
            with _nsc2:
                new_lien_fac = st.text_input("🔗 Lien Google Drive (facture)",
                ...
                if _errors:
                    ...
                    db_conn.execute("""INSERT INTO transactions
                        ...
                        (date_op.year, date_op.month, str(date_op),
                         ...
                         new_num_suivi or "", new_lien_fac or ""))
