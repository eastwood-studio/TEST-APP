# operations_module.py — v3.5 — 1775686094 — 1775685755 — 1775684013 — 1775681612 — 1775665376
# ══════════════════════════════════════════════════════════════════════════════
...
def page_operations(can_fn, DB_PATH, fmt_eur_fn=None):
    ...
    with tab_hist:
        ...
        if df_ops.empty:
            ...
            # Supprimer une opération (Jules only)
            if can_fn("finance_write"):
                with st.expander("🗑 Supprimer une opération", expanded=False):
                    del_num = st.number_input("N° opération à supprimer", min_value=1, step=1, key="del_op_num")
                    if st.button("⚠ Supprimer définitivement", key="btn_del_op", type="primary"):
                        row_del = conn.execute(
                            "SELECT id FROM transactions WHERE num_operation=?", (int(del_num),)).fetchone()
                        if row_del:
                            conn.execute("DELETE FROM transactions WHERE id=?", (row_del[0],))
                            conn.commit()
                            st.success(f"✓ Opération N°{del_num} supprimée.")
                            st.rerun()
                        else:
                            st.error(f"N°{del_num} introuvable.")
...
{orig_str}
    ...
    if tab_imp_ops is not None:
        with tab_imp_ops:
            ...
            st.markdown("""
<div class="info-box">
Format CSV attendu : <code>date_op,type_op,categorie,description,total_ht,devise,payeur,ref_produit,collection_op</code><br>
Date format : YYYY-MM-DD · type_op : Achat / Vente / Utilisation / Achat perso
</div>""", unsafe_allow_html=True)

            # Template
            tmpl_ops = pd.DataFrame([{
                "date_op":"2026-04-08","type_op":"Achat","categorie":"Matière première",
                "description":"Achat tissu whipcord 5m","total_ht":225.0,
                "devise":"EUR","payeur":"Eastwood","ref_produit":"",
                "collection_op":"Chapter I — Hunting & Fishing"
            }])
            buf_t = io.BytesIO()
            tmpl_ops.to_csv(buf_t, index=False, encoding="utf-8-sig")
            st.download_button("⬇ Template CSV", buf_t.getvalue(), file_name="template_operations.csv", mime="text/csv")

            st.markdown("---")
            imp_ops_file = st.file_uploader("Fichier CSV", type=["csv"], key="imp_ops")
            if imp_ops_file:
                try:
                    df_imp_ops = pd.read_csv(imp_ops_file)
                    st.dataframe(df_imp_ops.head(10), use_container_width=True, hide_index=True)
                    if st.button("✓ Importer", type="primary", key="btn_imp_ops"):
                        ok_o = 0
                        next_n = get_next_num(conn)
                        for _, row_o in df_imp_ops.iterrows():
                            try:
                                ht = float(row_o.get("total_ht",0) or 0)
                                tva_a = round(ht * 0.20, 2) if row_o.get("type_op")=="Vente" else 0
                                conn.execute("""INSERT INTO transactions
                                    (annee,mois,date_op,ref_produit,info_process,description,
                                     categorie,type_op,quantite,unite,prix_unitaire,type_tva,
                                     total_ht,total_ttc,tva,devise,taux_change,montant_original,
                                     payeur,beneficiaire,collection_op,num_operation)
                                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                                    (2026, 4, str(row_o.get("date_op","")),
                                     str(row_o.get("ref_produit","") or ""),
                                     str(row_o.get("description","")[:50]),
                                     str(row_o.get("description","")),
                                     str(row_o.get("categorie","Autre frais")),
                                     str(row_o.get("type_op","Achat")),
                                     1.0, "Euros", ht, "Déductible",
                                     ht, ht+tva_a, tva_a,
                                     str(row_o.get("devise","EUR")), 1.0, ht,
                                     str(row_o.get("payeur","Eastwood")), "Eastwood",
                                ...
                                pass
                        conn.commit()
                        st.success(f"✓ {ok_o} opérations importées.")
                        st.rerun()
                except Exception as e:
                    st.error(f"Erreur : {e}")
