# operations_module.py — v3.14 — 1777014500
# ══════════════════════════════════════════════════════════════════════════════
        ...
        with tab_new:
            ...
            if type_op == "Vente":
                ...
                with cv3:
                    ...
                    plateforme = st.text_input("Plateforme / Source", placeholder="Shopify, DM Instagram...")
            ...
            # ══ FOURNISSEUR (achats) ═══════════════════════════════════════════
            if type_op != "Vente":
                st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:9px;color:{EW_B};text-transform:uppercase;letter-spacing:.15em;margin:10px 0 4px;">Fournisseur</div>', unsafe_allow_html=True)
                try:
                    _fours_list = ["— Sélectionner / Nouveau —"] + [r[0] for r in conn.execute("SELECT DISTINCT nom FROM contacts WHERE type_contact='Fournisseur' ORDER BY nom").fetchall() if r[0]]
                except Exception:
...
{orig_str}
            ...
            if st.button("✓ Enregistrer l'opération", type="primary"):
                ...
                if type_op != "Vente" and _new_four_data.get("nom","").strip():
                    if not _new_four_data.get("email","").strip():
                        _errors.append("L'**email** du fournisseur est obligatoire.")
                    if not _new_four_data.get("pays","").strip():
