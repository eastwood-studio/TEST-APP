# operations_module.py — v3.13 — 1777010800
# ══════════════════════════════════════════════════════════════════════════════
...
def page_operations(can_fn, DB_PATH, fmt_eur_fn=None):
    ...
    with tab_hist:
        if _can_read_only:
            # Lecture seule : pas de filtres, période fixe année courante
            st.markdown(
                f'<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;'
                f'background:#f5f0e8;border:1px solid #d9c8ae;border-radius:5px;'
                f'padding:6px 12px;margin-bottom:10px;">'
                f'👁 Vue lecture seule — Historique complet</div>',
        ...
        if df_ops.empty:
            ...
            # ── Modifier une ligne (avec suppression intégrée) ─────────────────
            if _can_write:
...
⚠ Cette action est <strong>irréversible</strong>. Les opérations supprimées ne pourront pas être récupérées.
            ...
            # ── Modification en masse par sélection de numéros ────────────────
            if _can_write:
  ...
  <span style="font-family:'DM Mono',monospace;font-size:11px;color:{EW_B};margin-left:auto;">
    ...
    if tab_new is not None:
        with tab_new:
            ...
            with c2:
                categorie = st.selectbox("Catégorie *", ["— Sélectionner —"] + CATEGORIES)
                devise    = st.selectbox("Devise", DEVISES)
            with c3:
                # Filtrer les produits par collection
                try:
                    _colls_new = ["Toutes"] + [r[0] for r in conn.execute(
                        "SELECT DISTINCT collection FROM products WHERE collection IS NOT NULL ORDER BY collection"
                    ).fetchall()]
                except Exception:
                    _colls_new = ["Toutes"]
                ...
                # Charger les produits de la collection sélectionnée
                try:
                    if _coll_filter_new == "Toutes":
                        _prods_new = conn.execute(
                            "SELECT nom, ref, couleurs FROM products ORDER BY nom"
                        ).fetchall()
                    else:
                        _prods_new = conn.execute(
                            "SELECT nom, ref, couleurs FROM products WHERE collection=? ORDER BY nom",
                            (_coll_filter_new,)
                        ).fetchall()
                    _prod_labels_new = ["— Saisie libre —"] + [
                        f"{p[0]}{' — '+p[2] if p[2] else ''} ({p[1]})" for p in _prods_new
                    ]
                ...
                if _prod_sel_new != "— Saisie libre —":
                    _auto_ref  = _prod_refs_new[_prod_idx_new] or ""
                    _auto_nom  = _prod_noms_new[_prod_idx_new] or ""
                    st.session_state["_new_op_ref"]  = _auto_ref
                    st.session_state["_new_op_nom"]  = _auto_nom
                    st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:10px;color:{EW_G};padding:3px 0;">✓ SKU auto : {_auto_ref}</div>', unsafe_allow_html=True)
                    ref_produit = _auto_ref
                else:
                    ref_produit = st.text_input("Réf. SKU libre", placeholder="ex: EWSJACKET-001A",
                                                value=st.session_state.pop("_new_op_ref",""),
                                                key="new_op_sku_libre")
                    st.session_state.pop("_new_op_nom", None)

            # Collection dynamique
            collections = get_collections_list(conn)
            col_op = st.selectbox("Collection", collections, key="new_op_coll_sel")

            # Article pré-rempli depuis la sélection produit
            _default_article = st.session_state.pop("_new_op_nom", "") if "new_op_prod" in st.session_state and st.session_state.get("new_op_prod","") != "— Saisie libre —" else ""
            if not _default_article and st.session_state.get("new_op_prod","") not in ("","— Saisie libre —"):
                _idx_p = _prod_labels_new.index(st.session_state["new_op_prod"]) if st.session_state.get("new_op_prod") in _prod_labels_new else 0
                _default_article = _prod_noms_new[_idx_p] if _idx_p > 0 else ""

            # Si vente avec multi-articles, info_process déjà défini
            if type_op != "Vente" or not st.session_state.get("vente_n_art",0):
                info_process = st.text_input("Article / Process",
                    value=_default_article,
                    placeholder="ex: Waterfowl Jacket Tobacco")
            ...
            if type_op == "Vente":
                ...
                n_art_v = st.number_input("Nombre d'articles différents", min_value=1, max_value=8, value=1, step=1, key="vente_n_art")
                articles_vente = []
                _TAILLES_PROD = ["— Sans taille —","XS","S","1(S)","M","2(M)","L","3(L)","XL","4(XL)","XXL","Unique"]
                for _vi in range(int(n_art_v)):
                    ...
                    _va1,_va2,_va3,_va4 = st.columns([3,1,1,2])
                    with _va1: _vsel = st.selectbox("Produit", _popts_v, key=f"vente_art_{_vi}")
                    with _va2: _vqte = st.number_input("Qté", min_value=1, value=1, step=1, key=f"vente_qte_{_vi}")
                    with _va3:
                        _vtaille = st.selectbox("Taille *", _TAILLES_PROD, key=f"vente_taille_{_vi}")
                    with _va4:
                        if _vsel == "— Saisie libre —":
                            _vref = st.text_input("SKU", key=f"vente_ref_{_vi}")
                            _vnom = _vref
                        else:
                            _vref = _pref_v.get(_vsel,""); _vnom = _pnom_v.get(_vsel,"")
                            st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:10px;color:#395f30;padding:5px 0;">✓ {_vref}</div>', unsafe_allow_html=True)
                    articles_vente.append({"ref":_vref,"nom":_vnom,"qte":_vqte,"taille":_vtaille})

                _all_refs_v = ", ".join(a["ref"] for a in articles_vente if a["ref"])
                # Consolider TOUS les articles dans info_process (avec taille)
                _all_noms_v = " + ".join(
                    f"{a['qte']}× {a['nom']}{(' T.'+a['taille']) if a.get('taille') and a['taille']!='— Sans taille —' else ''}"
                    for a in articles_vente if a.get("nom")
                )
                if _all_refs_v:
                    st.markdown(f'<div style="font-size:11px;color:{EW_B};margin:4px 0;">Refs : {_all_refs_v}</div>', unsafe_allow_html=True)

            else:
                client_nom = client_email = client_tel = client_adresse = ""
                type_vente = plateforme = ""; articles_vente = []
            ...
            if st.button("✓ Enregistrer l'opération", type="primary"):
                ...
                if type_op in ("Achat","Achat perso") and ref_produit and unite in ("Article","Pièces"):
                    ...
                    try:
                        ...
                        _is_pf = False
                # Taille vente obligatoire
                if type_op == "Vente" and articles_vente:
                    _missing_tailles = [a for a in articles_vente if a.get("nom") and a.get("taille","") == "— Sans taille —"]
                    if _missing_tailles:
                        _errors.append(f"La **taille** est obligatoire pour : {', '.join(a['nom'] for a in _missing_tailles)}")
