# operations_module.py — v3.6 — 1776502599
# ══════════════════════════════════════════════════════════════════════════════
                        ...
                        with save_col:
                            if st.form_submit_button("💾 Enregistrer", type="primary"):
                                ...
                                conn.execute("""UPDATE transactions SET
                                    ...
                                    description=?, ref_produit=?, article=?,
                                    quantite=?, unite=?,
                                    total_ht=?, tva=?, total_ttc=?,
                                    type_tva=?, devise=?, frais_envoi=?,
                                    collection_op=?, payeur=?, beneficiaire=?,
                                    info_complementaire=?
                                    WHERE id=?""",
                                    (str(e_date), e_date.year, e_date.month, e_type, e_cat,
                                     e_desc, e_sku, e_art, e_qte, e_unit,
                                     e_ht, tva_amt, ttc_new,
...
<strong>Format natif Eastwood :</strong> colonnes standards de l'app (voir template).
            ...
            if imp_ops_file:
                ...
                if not df_imp_raw.empty:
                    ...
                    unmapped = [c for c in df_imp_raw.columns if c not in df_imp_ops.columns]
                    if unmapped and is_gsheet:
                        st.caption(f"Colonnes ignorées : {', '.join(unmapped)}")

                    if st.button("✓ Importer toutes les opérations", type="primary", key="btn_imp_ops"):
                        ok_o = 0; err_o = 0
                        next_n = get_next_num(conn)
                        for _, row_o in df_imp_ops.iterrows():
                            try:
                                # Parser la date
                                _date_raw = str(row_o.get("date_op","") or "")
                                try:
                                    ...
                                    for _fmt in ["%Y-%m-%d","%d/%m/%Y","%m/%d/%Y","%d-%m-%Y"]:
                                        try:
                                            ...
                                            _ann = _dt.year; _mois = _dt.month
                                            break
                                        except Exception:
                                            continue
                                    else:
                                        _date_str = _date_raw[:10] or str(date.today())
                                        _ann = int(_date_str[:4]) if len(_date_str) >= 4 else date.today().year
                                        _mois = int(_date_str[5:7]) if len(_date_str) >= 7 else date.today().month
                                except Exception:
                                    _date_str = str(date.today()); _ann = date.today().year; _mois = date.today().month

                                ht = float(str(row_o.get("total_ht","0") or 0).replace(",",".").replace(" ","") or 0)
                                ttc = float(str(row_o.get("total_ttc","0") or 0).replace(",",".").replace(" ","") or 0)
                                tva_v = float(str(row_o.get("tva","0") or 0).replace(",",".").replace(" ","") or 0)
                                prix_u = float(str(row_o.get("prix_unitaire","0") or 0).replace(",",".").replace(" ","") or 0)
                                qte = float(str(row_o.get("quantite","1") or 1).replace(",",".").replace(" ","") or 1)

                                # Type TVA normalisé
                                _tva_raw = str(row_o.get("type_tva","Aucun") or "Aucun").strip()
                                _tva_map = {"aucun":"Aucun","collecté":"Collectée","collectée":"Collectée",
                                           "déductible":"Déductible","deductible":"Déductible",
                                           "autoliquidé":"Autoliquidée","autoliquidée":"Autoliquidée",
                                           "autoliquidé":"Autoliquidée"}
                                _type_tva = _tva_map.get(_tva_raw.lower(), _tva_raw) if _tva_raw.lower() in _tva_map else _tva_raw

                                # Type op
                                _type_op = str(row_o.get("type_op","Achat") or "Achat").strip()
                                if _type_op.lower() in ("achat perso","achat personnel"): _type_op = "Achat perso"
                                elif _type_op.lower() in ("achat","purchase"): _type_op = "Achat"
                                elif _type_op.lower() in ("vente","sale","ventes"): _type_op = "Vente"
                                ...
                                conn.execute("""INSERT INTO transactions
                                    ...
                                    (_ann, _mois, _date_str,
                                     ...
                                     str(row_o.get("article","") or row_o.get("description",""))[:60],
                                     str(row_o.get("article","") or "")[:60],
                                     str(row_o.get("description","") or ""),
                                     str(row_o.get("categorie","Autre frais") or "Autre frais"),
                                     _type_op, qte,
                                     str(row_o.get("unite","Euros") or "Euros"),
                                     prix_u, _type_tva,
                                     ht, ttc if ttc else ht, tva_v,
                                     str(row_o.get("devise","EUR") or "EUR"), 1.0, ht,
                                     str(row_o.get("payeur","Eastwood") or "Eastwood"),
                        ...
                        conn.commit()
                        if ok_o > 0:
                            st.success(f"✓ **{ok_o} opérations importées** avec succès !")
                        if err_o > 0:
                            st.warning(f"⚠ {err_o} lignes ignorées (données manquantes ou invalides).")
                        if ok_o > 0:
                            st.rerun()
