# contacts_module.py — v2.11 — 1776967740
# ══════════════════════════════════════════════════════════════════════════════
# MODULE CONTACTS v2
...
def page_contacts(can_fn, DB_PATH):
    ...
    _tabs = ["👤 Contacts", "🏭 Fournisseurs", "📸 Modèle Booklist", "🎬 Shootings", "📥 Import Excel", "📤 Export"]
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
  ...
  <div>
    ...
    with tab_book:
        ...
        if df_mod.empty:
            ...
            for _, mod in df_mod.iterrows():
                ...
                _lc   = str(mod.get("localisation","") or "")
                    ...
                    with st.container():
                        ...
                        with st.form(f"edit_mod_{mid}"):
                            ...
                            with _e2:
                                ...
                                m_loc    = st.text_input("Localisation", value=str(mod.get("localisation","") or ""))
    ...
    # ── IMPORT EXCEL ───────────────────────────────────────────────────────────
    with tab_shoot:
        st.markdown('<div class="section-title">Dossiers Shooting</div>', unsafe_allow_html=True)
        _sh_statuts = ["En préparation","En cours","Terminé","Archivé"]
        try:
            df_sh = pd.read_sql("SELECT * FROM shooting_dossiers ORDER BY date_shooting DESC, id DESC", conn)
        except Exception:
            df_sh = pd.DataFrame()

        if can_fn("contacts_add"):
            with st.expander("➕ Nouveau dossier shooting"):
                with st.form("new_shooting"):
                    _ns1,_ns2,_ns3 = st.columns(3)
                    with _ns1:
                        ns_nom  = st.text_input("Nom du shooting *", placeholder="Capsule Summer — Club Shirt")
                        ns_coll = st.text_input("Collection", placeholder="Chapter III")
                    with _ns2:
                        ns_date = st.text_input("Date", placeholder="15-17 August 2025")
                        ns_lieu = st.text_input("Lieu", placeholder="Berlin — Tempelhof")
                    with _ns3:
                        ns_stat = st.selectbox("Statut", _sh_statuts)
                        ns_desc = st.text_area("Description / Notes", height=60)
                    if st.form_submit_button("✓ Créer le dossier", type="primary"):
                        if ns_nom.strip():
                            conn.execute("INSERT INTO shooting_dossiers (nom,description,collection,date_shooting,lieu,statut) VALUES (?,?,?,?,?,?)",
                                (ns_nom,ns_desc,ns_coll,ns_date,ns_lieu,ns_stat))
                            conn.commit(); st.success("✓ Dossier créé."); st.rerun()

        if df_sh.empty:
            st.info("Aucun shooting. Créez un dossier ci-dessus.")
        else:
            for _, sh in df_sh.iterrows():
                shid = int(sh["id"])
                _sh_open = st.session_state.get(f"sh_open_{shid}", False)
                _sh_edit = st.session_state.get(f"sh_edit_{shid}", False)
                _sc_sh = {"En préparation":"#c9800a","En cours":"#395f30","Terminé":"#7b506f","Archivé":"#ccc"}.get(str(sh.get("statut","")), "#888")
                try:
                    df_sh_m = pd.read_sql("SELECT m.*, sm.notes as sh_notes FROM modeles m JOIN shooting_modeles sm ON sm.modele_id=m.id WHERE sm.shooting_id=?", conn, params=[shid])
                except Exception:
                    df_sh_m = pd.DataFrame()
                _sh_nm = str(sh.get("nom","") or ""); _sh_cl = str(sh.get("collection","") or "")
                _sh_dt = str(sh.get("date_shooting","") or ""); _sh_li = str(sh.get("lieu","") or "")
                _sh_st = str(sh.get("statut","") or ""); _n_mod = len(df_sh_m)
                st.markdown(
                    f'<div style="background:#f5f0e8;border:1px solid #d9c8ae;border-radius:8px;padding:10px 14px;margin-bottom:4px;display:flex;align-items:center;gap:12px;">'
                    f'<div style="flex:1;"><span style="font-size:14px;font-weight:600;color:#1a1a1a;">{_sh_nm}</span>'
                    f'&nbsp;&nbsp;<span style="font-family:DM Mono,monospace;font-size:8px;color:{_sc_sh};border:1px solid {_sc_sh};border-radius:3px;padding:1px 5px;">{_sh_st}</span></div>'
                    f'<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;">{_sh_cl}{" · " if _sh_cl and _sh_dt else ""}{_sh_dt}{" · " if _sh_dt and _sh_li else ""}{_sh_li}</div>'
                    f'<div style="font-family:DM Mono,monospace;font-size:9px;color:#395f30;white-space:nowrap;">{_n_mod} modèle(s)</div></div>',
                    unsafe_allow_html=True)
                _bc1,_bc2,_bc3 = st.columns([1,1,8])
                with _bc1:
                    if st.button("👁" if not _sh_open else "✕", key=f"sh_tog_{shid}"):
                        st.session_state[f"sh_open_{shid}"] = not _sh_open; st.rerun()
                with _bc2:
                    if st.button("✏️", key=f"sh_edit_btn_{shid}"):
                        st.session_state[f"sh_edit_{shid}"] = not _sh_edit; st.rerun()
                if _sh_edit and can_fn("contacts_edit"):
                    with st.form(f"edit_sh_{shid}"):
                        _es1,_es2,_es3 = st.columns(3)
                        with _es1: es_nom = st.text_input("Nom", value=_sh_nm); es_coll = st.text_input("Collection", value=_sh_cl)
                        with _es2: es_date = st.text_input("Date", value=_sh_dt); es_lieu = st.text_input("Lieu", value=_sh_li)
                        with _es3:
                            es_stat = st.selectbox("Statut", _sh_statuts, index=_sh_statuts.index(_sh_st) if _sh_st in _sh_statuts else 0)
                            es_desc = st.text_area("Notes", value=str(sh.get("description","") or ""), height=50)
                        _esb1,_esb2 = st.columns([4,1])
                        with _esb1: _save = st.form_submit_button("💾 Enregistrer", type="primary")
                        with _esb2: _del = st.form_submit_button("🗑") if can_fn("contacts_delete") else False
                        if _save:
                            conn.execute("UPDATE shooting_dossiers SET nom=?,collection=?,date_shooting=?,lieu=?,statut=?,description=? WHERE id=?",
                                (es_nom,es_coll,es_date,es_lieu,es_stat,es_desc,shid))
                            conn.commit(); st.session_state.pop(f"sh_edit_{shid}",None); st.rerun()
                        if _del:
                            conn.execute("DELETE FROM shooting_modeles WHERE shooting_id=?", (shid,))
                            conn.execute("DELETE FROM shooting_dossiers WHERE id=?", (shid,))
                            conn.commit(); st.rerun()
                if _sh_open:
                    if not df_sh_m.empty:
                        for _, sm in df_sh_m.iterrows():
                            _sm_nm = f"{sm.get('prenom','') or ''} {sm.get('nom','') or ''}".strip()
                            _smc1,_smc2 = st.columns([6,1])
                            with _smc1:
                                st.markdown(
                                    f'<div style="font-size:12px;padding:4px 10px;background:#fff;border:1px solid #e8e0d4;border-radius:5px;margin-bottom:3px;">'
                                    f'<strong>{_sm_nm}</strong> · <span style="color:#8a7968;font-size:10px;">{sm.get("genre","")} · {sm.get("localisation","")}</span>'
                                    f'{(" · <em>"+str(sm["sh_notes"])+"</em>") if sm.get("sh_notes") else ""}</div>',
                                    unsafe_allow_html=True)
                            with _smc2:
                                if st.button("🗑", key=f"rm_sm_{shid}_{sm['id']}"):
                                    conn.execute("DELETE FROM shooting_modeles WHERE shooting_id=? AND modele_id=?", (shid, int(sm["id"])))
                                    conn.commit(); st.rerun()
                    else:
                        st.markdown('<div style="font-size:11px;color:#8a7968;padding:4px 0;">Aucun modèle dans ce shooting.</div>', unsafe_allow_html=True)
                    try:
                        _all_mods = pd.read_sql("SELECT id, prenom, nom, genre, localisation FROM modeles ORDER BY nom, prenom", conn)
                    except Exception:
                        _all_mods = pd.DataFrame()
                    if not _all_mods.empty:
                        _mod_opts_list = ["— Sélectionner —"] + [
                            f"{str(r.get('prenom','') or '')} {str(r.get('nom','') or '')} ({str(r.get('genre',''))}, {str(r.get('localisation',''))})".strip()
                            for _, r in _all_mods.iterrows()
                        ]
                        _mod_ids_list = [None] + [int(r["id"]) for _, r in _all_mods.iterrows()]
                        _add1,_add2,_add3 = st.columns([3,3,1])
                        with _add1: _sel_idx = st.selectbox("Ajouter un modèle", range(len(_mod_opts_list)), format_func=lambda i: _mod_opts_list[i], key=f"add_mod_{shid}")
                        with _add2: _add_notes = st.text_input("Notes (pièces, rôle...)", key=f"add_notes_{shid}")
                        with _add3:
                            if st.button("➕", key=f"add_mod_btn_{shid}"):
                                _mid_add = _mod_ids_list[_sel_idx]
                                if _mid_add:
                                    conn.execute("INSERT OR IGNORE INTO shooting_modeles (shooting_id,modele_id,notes) VALUES (?,?,?)", (shid, _mid_add, _add_notes))
                                    conn.commit(); st.rerun()
