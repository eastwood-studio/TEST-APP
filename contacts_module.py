# contacts_module.py — v2.14 — 1778146265
# ══════════════════════════════════════════════════════════════════════════════
# MODULE CONTACTS v2
...
SOUS_TYPES = {
    ...
    "Fournisseur":   ["Matière première", "Composant", "Packaging", "Production / Confection",
...
def init_contacts_v2(conn):
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS photographes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        prenom TEXT,
        nom TEXT,
        telephone TEXT,
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS shootings (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        photographe_id INTEGER,
        nom_shooting TEXT,
        date_shooting TEXT,
        type_shooting TEXT,
        collection TEXT,
        ...
        notes TEXT,
        lien_drive TEXT,
        FOREIGN KEY(photographe_id) REFERENCES photographes(id)
    )""")

    # Migration photographes
    existing_ph = [r[1] for r in c.execute("PRAGMA table_info(photographes)").fetchall()]
    for _phc, _phd in [("specialite","TEXT"),("type_photo","TEXT"),("style","TEXT"),
                       ("tarif_journee","REAL"),("materiel","TEXT"),("langues","TEXT")]:
        if _phc not in existing_ph:
            try: c.execute(f"ALTER TABLE photographes ADD COLUMN {_phc} {_phd}")
            except Exception: pass
...
def page_contacts(can_fn, DB_PATH):
    ...
    # ── Synchro contacts → fournisseurs (si des contacts Fournisseur ne sont pas dans la table fournisseurs)
    try:
        df_ct_fourn = pd.read_sql(
            "SELECT * FROM contacts WHERE type_contact='Fournisseur'", conn)
        for _, row in df_ct_fourn.iterrows():
            ex = conn.execute("SELECT id FROM fournisseurs WHERE nom=?", (row["nom"],)).fetchone()
            if not ex:
                conn.execute("""INSERT OR IGNORE INTO fournisseurs
                    (nom,sous_type,pays,ville,adresse,email,telephone,instagram,site_web,notes,
                     statut_fourn,deja_travaille,importance)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (row.get("nom",""), row.get("sous_type",""), row.get("pays",""),
                     row.get("ville",""), row.get("adresse",""), row.get("email",""),
                     row.get("telephone",""), row.get("instagram",""), row.get("web",""),
                     row.get("notes",""), "Actif", 1, row.get("importance","Normal")))
        conn.commit()
    except Exception:
        pass
    ...
    _tabs = ["👤 Contacts", "🏭 Fournisseurs", "📸 Modèle Booklist", "📷 Photographes", "📥 Import Excel", "📤 Export"]
    ...
    tab_photo = tab_objs[idx]; idx += 1
    tab_imp   = tab_objs[idx]; idx += 1
    tab_exp   = tab_objs[idx]; idx += 1
    ...
    # ── FOURNISSEURS ───────────────────────────────────────────────────────────
    with tab_fourn:
        # Sous-catégories depuis l'inventaire stock
        try:
            _stk_subcats = ["Tous"] + sorted(set(r[0] for r in conn.execute(
        ...
        with ff1: f_stype  = st.selectbox("Type", ["Tous"]+SOUS_TYPES["Fournisseur"], key="ff_stype")
        with ff2: f_subcat = st.selectbox("Sous-catégorie stock", _stk_subcats, key="ff_subcat")
        with ff3: f_statf  = st.selectbox("Statut", ["Tous"]+STATUT_FOURNISSEUR, key="ff_stat")
        with ff4: f_travf  = st.selectbox("Collaboration", ["Tous","Déjà travaillé","Pas encore"], key="ff_trav")
        with ff5: f_pays_f = st.selectbox("Pays", ["Tous"]+PAYS_LIST, key="ff_pays")
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
  ...
  <div>
    ...
    {f'<div style="font-size:11px;color:{EW_B};margin-top:4px;">{row["notes"]}</div>' if row.get("notes") else ''}
        ...
        if can_fn("contacts_add"):
            ...
            with st.expander("➕ Nouveau fournisseur"):
                ...
                try:
                    _prods_db = conn.execute("SELECT ref, nom FROM products ORDER BY nom").fetchall()
                    _prods_opts = ["— Saisie libre —"] + [f"{p[1]} ({p[0]})" for p in _prods_db]
                    _prods_refs = {"— Saisie libre —":""} | {f"{p[1]} ({p[0]})": p[0] for p in _prods_db}
                    _prods_noms = {"— Saisie libre —":""} | {f"{p[1]} ({p[0]})": p[1] for p in _prods_db}
                except Exception:
                    _prods_opts = ["— Saisie libre —"]; _prods_refs = {}; _prods_noms = {}
                ...
                with st.form("new_fourn"):
                    ...
                    with nf2:
                        ...
                        nf_nom_ct = st.text_input("Nom contact")
                        nf_web    = st.text_input("Site web")
                        nf_ig     = st.text_input("Instagram")
                    with nf3:
                        nf_adr    = st.text_input("Adresse complète", placeholder="18 Rue de Picardie, 75003 Paris, France")
                        # Auto-extraction ville + pays
                        nf_pays_auto = ""; nf_ville_auto = ""
                        if nf_adr:
                            _pts = [p.strip() for p in nf_adr.split(",")]
                            if len(_pts) >= 2:
                                nf_pays_auto  = _pts[-1]
                                nf_ville_auto = re.sub(r"^\d+\s*","", _pts[-2]).strip()
                        nf_pays   = st.selectbox("Pays", PAYS_LIST,
                            index=PAYS_LIST.index(nf_pays_auto) if nf_pays_auto in PAYS_LIST else 0)
                        nf_ville  = st.text_input("Ville", value=nf_ville_auto)
                        nf_moq    = st.text_input("MOQ général")
                        nf_delai  = st.text_input("Délai production général", placeholder="ex: 6 semaines")
                    nf_notes  = st.text_area("Notes", height=50)

                    # Produits fabriqués — ligne par produit
                    st.markdown('<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.1em;margin:8px 0 3px;">Produits fabriqués</div>', unsafe_allow_html=True)
                    nf_nb_prods = st.number_input("Nombre de produits fabriqués", min_value=0, max_value=12, value=0, step=1, key="nf_nb_prods")
                    nf_prods_lines = []
                    for _npi in range(int(nf_nb_prods)):
                        _npc1,_npc2,_npc3 = st.columns([3,1,1])
                        with _npc1:
                            _np_sel = st.selectbox(f"Produit {_npi+1}", _prods_opts, key=f"nf_prod_{_npi}")
                            if _np_sel == "— Saisie libre —":
                                _np_nom = st.text_input("Nom produit", key=f"nf_prod_nom_{_npi}")
                                _np_sku = ""
                            else:
                                _np_nom = _prods_noms.get(_np_sel,"")
                                _np_sku = _prods_refs.get(_np_sel,"")
                                st.markdown(f'<div style="font-size:9px;color:#395f30;font-family:DM Mono,monospace;">SKU : {_np_sku}</div>', unsafe_allow_html=True)
                        with _npc2:
                            _np_moq = st.text_input("MOQ", key=f"nf_prod_moq_{_npi}")
                        with _npc3:
                            _np_del = st.text_input("Délai", key=f"nf_prod_del_{_npi}")
                        if _np_nom:
                            nf_prods_lines.append(f"{_np_nom} ({_np_sku}) — MOQ:{_np_moq} — Délai:{_np_del}")
                        ...
                        if not nf_nom:
                            ...
                            conn.execute("""INSERT INTO fournisseurs
                                ...
                                (nf_nom,nf_stype,nf_stat,int(nf_trav),nf_email,nf_tel,
                                 ...
                                 nf_prods_str,nf_moq,nf_delai,nf_notes))
                            # Sync avec contacts
                            try:
                                conn.execute("""INSERT OR IGNORE INTO contacts
                                    (nom,type_contact,sous_type,email,telephone,instagram,
                                     activite,adresse,ville,pays,importance)
                                    VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
                                    (nf_nom,"Fournisseur",nf_stype,nf_email,nf_tel,nf_ig,
                                     f"Délai: {nf_delai} · MOQ: {nf_moq}",nf_adr,nf_ville,nf_pays,"Normal"))
                            except Exception: pass
                            conn.commit(); st.success("✓ Fournisseur ajouté et synchronisé dans Contacts."); st.rerun()
    ...
    with tab_photo:
        # ── Table shootings ─────────────────────────────────────────────────────
        TYPES_PHOTO = ["Mode","Portrait","Lookbook","Campaign","Editorial","E-commerce","Street","Studio","Autre"]
        STYLES_PHOTO = ["Analogique","Numérique","Film","Documentaire","Minimaliste","Haute couture","Streetwear","Autre"]
        STATUTS_PHOTO = ["À contacter","En discussion","Validé","Déjà collaboré","Inactif"]
        TYPES_SHOOTING = ["Lookbook","Campaign","Editorial","E-commerce","Shooting presse","BTS / Behind the scenes","Autre"]
        STATUTS_SHOOTING = ["Planifié","En cours","Terminé","Annulé"]

        _ph1, _ph2, _ph3 = st.columns([2, 2, 3])
        with _ph1: ph_stat = st.selectbox("Statut", ["Tous"] + STATUTS_PHOTO, key="ph_stat")
        with _ph2: ph_pays = st.selectbox("Pays", ["Tous","France","Allemagne","Japon","UK","USA","Autre"], key="ph_pays")
        with _ph3: ph_srch = st.text_input("Recherche", placeholder="nom, style, type...", key="ph_srch")

        qph = "SELECT * FROM photographes WHERE 1=1"
        pph = []
        if ph_stat != "Tous": qph += " AND statut=?"; pph.append(ph_stat)
        if ph_pays != "Tous": qph += " AND pays=?";   pph.append(ph_pays)

        try:
            df_ph = pd.read_sql(qph + " ORDER BY nom, prenom", conn, params=pph)
        except Exception:
            df_ph = pd.DataFrame()

        if ph_srch and not df_ph.empty:
            df_ph = df_ph[df_ph.apply(lambda r: ph_srch.lower() in str(r).lower(), axis=1)]

        st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:9px;color:{EW_B};margin:4px 0 12px;">{len(df_ph)} photographe(s)</div>', unsafe_allow_html=True)

        # ── Fiches photographes ─────────────────────────────────────────────────
        if df_ph.empty:
            st.info("Aucun photographe. Ajoutez-en un ci-dessous.")
        else:
            for _, ph in df_ph.iterrows():
                phid = ph["id"]
                _ph_editing = st.session_state.get(f"ph_edit_{phid}", False)
                _sc_ph = {"Validé":"#395f30","Déjà collaboré":"#7b506f","En discussion":"#c9800a",
                          "À contacter":"#8a7968","Inactif":"#ccc"}.get(str(ph.get("statut","")), "#8a7968")
                _ph_nom = f"{ph.get('prenom','')} {ph.get('nom','')}".strip() or "—"

                with st.expander(
                    f"📷 {_ph_nom} · {ph.get('localisation','') or ph.get('pays','')} · "
                    f"{ph.get('type_photo','') or ph.get('specialite','')} · "
                    f"● {ph.get('statut','')}", expanded=_ph_editing):

                    if not _ph_editing:
                        # ── Vue lecture ──────────────────────────────────────────
                        _c1, _c2, _c3 = st.columns([2, 2, 2])
                        with _c1:
                            st.markdown(f"**{_ph_nom}**")
                            if ph.get("email"):    st.markdown(f"✉️ {ph['email']}")
                            if ph.get("telephone"): st.markdown(f"📞 {ph['telephone']}")
                            if ph.get("instagram"): st.markdown(f"📸 {ph['instagram']}")
                            if ph.get("site_web"):  st.markdown(f"🌐 {ph['site_web']}")
                        with _c2:
                            if ph.get("type_photo"):   st.markdown(f"**Type** : {ph['type_photo']}")
                            if ph.get("style"):        st.markdown(f"**Style** : {ph['style']}")
                            if ph.get("specialite"):   st.markdown(f"**Spécialité** : {ph['specialite']}")
                            if ph.get("materiel"):     st.markdown(f"**Matériel** : {ph['materiel']}")
                            if ph.get("langues"):      st.markdown(f"**Langues** : {ph['langues']}")
                        with _c3:
                            if ph.get("tarif_journee"):     st.markdown(f"**Tarif journée** : {ph['tarif_journee']:.0f} €")
                            if ph.get("tarif_demi_journee"): st.markdown(f"**Tarif ½ journée** : {ph['tarif_demi_journee']:.0f} €")
                            if ph.get("notes"): st.markdown(f"*{ph['notes']}*")

                        # Shootings effectués avec ce photographe
                        try:
                            df_sh = pd.read_sql(
                                "SELECT * FROM shootings WHERE photographe_id=? ORDER BY date_shooting DESC",
                                conn, params=[phid])
                        except Exception:
                            df_sh = pd.DataFrame()
                        if not df_sh.empty:
                            st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;margin:8px 0 4px;">Shootings ({len(df_sh)})</div>', unsafe_allow_html=True)
                            for _, sh in df_sh.iterrows():
                                _sh_date = str(sh.get("date_shooting","") or "")[:10]
                                _sh_lien = sh.get("lien_drive","") or ""
                                _sh_info = f"**{sh.get('nom_shooting','')}** — {sh.get('type_shooting','')} · {sh.get('collection','')} · {sh.get('lieu','')} · {sh.get('statut','')}"
                                if _sh_lien and _sh_lien.startswith("http"):
                                    st.markdown(f"📅 {_sh_date} · {_sh_info} · [📁 Drive]({_sh_lien})")
                                else:
                                    st.markdown(f"📅 {_sh_date} · {_sh_info}")

                        if can_fn("contacts_add"):
                            if st.button("✏️ Modifier", key=f"ph_edit_btn_{phid}"):
                                st.session_state[f"ph_edit_{phid}"] = True
                                st.rerun()
                    else:
                        # ── Vue édition ──────────────────────────────────────────
                        ec1, ec2, ec3 = st.columns(3)
                        with ec1:
                            ep_prenom = st.text_input("Prénom", value=str(ph.get("prenom","") or ""), key=f"ph_pren_{phid}")
                            ep_nom    = st.text_input("Nom",    value=str(ph.get("nom","") or ""),    key=f"ph_nom_{phid}")
                            ep_email  = st.text_input("Email",  value=str(ph.get("email","") or ""),  key=f"ph_mail_{phid}")
                            ep_tel    = st.text_input("Téléphone", value=str(ph.get("telephone","") or ""), key=f"ph_tel_{phid}")
                            ep_ig     = st.text_input("Instagram", value=str(ph.get("instagram","") or ""), key=f"ph_ig_{phid}")
                            ep_web    = st.text_input("Site web",  value=str(ph.get("site_web","") or ""), key=f"ph_web_{phid}")
                        with ec2:
                            ep_loc    = st.text_input("Localisation", value=str(ph.get("localisation","") or ""), key=f"ph_loc_{phid}")
                            ep_pays   = st.text_input("Pays", value=str(ph.get("pays","France") or "France"), key=f"ph_pays_{phid}")
                            ep_type   = st.selectbox("Type de photo", TYPES_PHOTO, index=TYPES_PHOTO.index(ph.get("type_photo","Autre")) if ph.get("type_photo") in TYPES_PHOTO else 0, key=f"ph_type_{phid}")
                            ep_style  = st.selectbox("Style", STYLES_PHOTO, index=STYLES_PHOTO.index(ph.get("style","Autre")) if ph.get("style") in STYLES_PHOTO else 0, key=f"ph_style_{phid}")
                            ep_spec   = st.text_input("Spécialité", value=str(ph.get("specialite","") or ""), key=f"ph_spec_{phid}")
                            ep_mat    = st.text_input("Matériel", value=str(ph.get("materiel","") or ""), placeholder="Canon R5, Phase One...", key=f"ph_mat_{phid}")
                        with ec3:
                            ep_stat   = st.selectbox("Statut", STATUTS_PHOTO, index=STATUTS_PHOTO.index(ph.get("statut","À contacter")) if ph.get("statut") in STATUTS_PHOTO else 0, key=f"ph_stat_{phid}")
                            ep_tj     = st.number_input("Tarif journée (€)", min_value=0.0, step=50.0, value=float(ph.get("tarif_journee",0) or 0), key=f"ph_tj_{phid}")
                            ep_thj    = st.number_input("Tarif ½ journée (€)", min_value=0.0, step=50.0, value=float(ph.get("tarif_demi_journee",0) or 0), key=f"ph_thj_{phid}")
                            ep_lang   = st.text_input("Langues", value=str(ph.get("langues","") or ""), key=f"ph_lang_{phid}")
                            ep_notes  = st.text_area("Notes", value=str(ph.get("notes","") or ""), height=60, key=f"ph_notes_{phid}")

                        # Shooting lié à ce photographe
                        st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;margin:10px 0 4px;border-top:0.5px solid #d9c8ae;padding-top:8px;">Ajouter un shooting</div>', unsafe_allow_html=True)
                        sh1, sh2, sh3 = st.columns(3)
                        with sh1:
                            sh_nom     = st.text_input("Nom du shooting", key=f"sh_nom_{phid}", placeholder="Lookbook Chapter II")
                            sh_date    = st.date_input("Date", key=f"sh_date_{phid}", value=date.today())
                            sh_type    = st.selectbox("Type", TYPES_SHOOTING, key=f"sh_type_{phid}")
                        with sh2:
                            sh_coll    = st.text_input("Collection", key=f"sh_coll_{phid}")
                            sh_lieu    = st.text_input("Lieu", key=f"sh_lieu_{phid}")
                            sh_modeles = st.text_input("Modèles", key=f"sh_mods_{phid}", placeholder="Alice, Bob")
                        with sh3:
                            sh_stat    = st.selectbox("Statut shooting", STATUTS_SHOOTING, key=f"sh_stat_{phid}")
                            sh_lien    = st.text_input("Lien Google Drive", key=f"sh_lien_{phid}", placeholder="https://drive.google.com/...")
                            sh_notes   = st.text_input("Notes shooting", key=f"sh_notes_{phid}")

                        bc1, bc2, bc3 = st.columns(3)
                        with bc1:
                            if st.button("💾 Sauvegarder", key=f"ph_save_{phid}"):
                                try:
                                    conn.execute("""UPDATE photographes SET
                                        prenom=?,nom=?,email=?,telephone=?,instagram=?,site_web=?,
                                        localisation=?,pays=?,type_photo=?,style=?,specialite=?,
                                        materiel=?,statut=?,tarif_journee=?,tarif_demi_journee=?,
                                        langues=?,notes=?
                                        WHERE id=?""",
                                        (ep_prenom,ep_nom,ep_email,ep_tel,ep_ig,ep_web,
                                         ep_loc,ep_pays,ep_type,ep_style,ep_spec,
                                         ep_mat,ep_stat,ep_tj,ep_thj,ep_lang,ep_notes,phid))
                                    if sh_nom.strip():
                                        conn.execute("""INSERT INTO shootings
                                            (photographe_id,nom_shooting,date_shooting,type_shooting,
                                             collection,lieu,statut,modeles,notes,lien_drive)
                                            VALUES (?,?,?,?,?,?,?,?,?,?)""",
                                            (phid,sh_nom,str(sh_date),sh_type,sh_coll,sh_lieu,
                                             sh_stat,sh_modeles,sh_notes,sh_lien))
                                    conn.commit()
                                    st.session_state[f"ph_edit_{phid}"] = False
                                    st.success("✓ Sauvegardé")
                                    st.rerun()
                                except Exception as e:
                                    st.error(str(e))
                        with bc2:
                            if st.button("✕ Annuler", key=f"ph_cancel_{phid}"):
                                st.session_state[f"ph_edit_{phid}"] = False
                                st.rerun()

        # ── Ajouter un photographe ──────────────────────────────────────────────
        if can_fn("contacts_add"):
            st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;margin:16px 0 4px;border-top:0.5px solid #d9c8ae;padding-top:8px;">Nouveau photographe</div>', unsafe_allow_html=True)
            with st.expander("➕ Ajouter un photographe", expanded=False):
                na1, na2, na3 = st.columns(3)
                with na1:
                    n_ph_prenom = st.text_input("Prénom *", key="n_ph_pren")
                    n_ph_nom    = st.text_input("Nom *",    key="n_ph_nom")
                    n_ph_email  = st.text_input("Email",    key="n_ph_email")
                    n_ph_tel    = st.text_input("Téléphone",key="n_ph_tel")
                    n_ph_ig     = st.text_input("Instagram",key="n_ph_ig")
                with na2:
                    n_ph_loc   = st.text_input("Localisation / Ville", key="n_ph_loc")
                    n_ph_pays  = st.text_input("Pays", value="France", key="n_ph_pays")
                    n_ph_type  = st.selectbox("Type de photo", TYPES_PHOTO, key="n_ph_type")
                    n_ph_style = st.selectbox("Style", STYLES_PHOTO, key="n_ph_style")
                    n_ph_spec  = st.text_input("Spécialité", key="n_ph_spec")
                with na3:
                    n_ph_stat  = st.selectbox("Statut", STATUTS_PHOTO, key="n_ph_stat")
                    n_ph_tj    = st.number_input("Tarif journée (€)", min_value=0.0, step=50.0, key="n_ph_tj")
                    n_ph_mat   = st.text_input("Matériel", key="n_ph_mat")
                    n_ph_lang  = st.text_input("Langues", key="n_ph_lang")
                    n_ph_notes = st.text_area("Notes", height=60, key="n_ph_notes")

                if st.button("✓ Ajouter le photographe", type="primary", key="n_ph_add"):
                    if not n_ph_prenom.strip() and not n_ph_nom.strip():
                        st.error("Prénom ou Nom obligatoire.")
                    else:
                        try:
                            conn.execute("""INSERT INTO photographes
                                (prenom,nom,email,telephone,instagram,localisation,pays,
                                 type_photo,style,specialite,materiel,statut,
                                 tarif_journee,langues,notes)
                                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                                (n_ph_prenom,n_ph_nom,n_ph_email,n_ph_tel,n_ph_ig,
                                 n_ph_loc,n_ph_pays,n_ph_type,n_ph_style,n_ph_spec,
                                 n_ph_mat,n_ph_stat,n_ph_tj,n_ph_lang,n_ph_notes))
                            conn.commit()
                            st.success(f"✓ {n_ph_prenom} {n_ph_nom} ajouté(e)")
                            st.rerun()
                        except Exception as e:
                            st.error(str(e))
...
<strong>type_contact</strong> : Client / Fournisseur / Collaborateur / Autre
    ...
    with tab_exp:
        ...
        # ── Dédoublonnage ─────────────────────────────────────────────────────
        _df_with_email = df_all_ct[df_all_ct["email"].fillna("").str.strip() != ""]
        _dup_emails = _df_with_email[_df_with_email.duplicated(subset=["email"], keep=False)]
        _n_dups = len(_dup_emails["email"].unique()) if not _dup_emails.empty else 0

        if _n_dups > 0:
            st.markdown(
                f'<div style="background:#fff3cd;border:1px solid #f0c040;border-radius:6px;padding:8px 14px;'
                f'margin-bottom:10px;font-family:DM Mono,monospace;font-size:11px;color:#8a6000;">'
                f'⚠ {_n_dups} email(s) en doublon détecté(s) dans les contacts.</div>',
                unsafe_allow_html=True)
            with st.expander(f"Voir les doublons ({_n_dups} emails)"):
                for _dup_mail in sorted(_dup_emails["email"].unique()):
                    _dup_rows = df_all_ct[df_all_ct["email"] == _dup_mail][["id","nom","type_contact","created_at"]]
                    st.markdown(f"**{_dup_mail}** — {len(_dup_rows)} entrées")
                    st.dataframe(_dup_rows, hide_index=True, use_container_width=True)

            if can_fn("contacts_delete"):
                if st.button("🧹 Supprimer les doublons automatiquement (garder le plus récent)", type="secondary"):
                    _deleted = 0
                    for _dup_mail in _dup_emails["email"].unique():
                        _dup_ids = conn.execute(
                            "SELECT id FROM contacts WHERE email=? ORDER BY id DESC",
                            (_dup_mail,)
                        ).fetchall()
                        # Garder le premier (id le plus grand = plus récent), supprimer les autres
                        for _did in _dup_ids[1:]:
                            conn.execute("DELETE FROM contacts WHERE id=?", (_did[0],))
                            _deleted += 1
                    conn.commit()
                    st.success(f"✓ {_deleted} doublon(s) supprimé(s).")
                    st.rerun()
        else:
            st.markdown(
                f'<div style="font-family:DM Mono,monospace;font-size:10px;color:#395f30;margin-bottom:8px;">'
                f'✓ Aucun doublon d\'email détecté.</div>',
                unsafe_allow_html=True)
        ...
        # Dédoublonner par email pour l'export (garder le plus récent)
        _n_before = len(df_exp)
        df_exp_dedup = df_exp.copy()
        _has_email_mask = df_exp_dedup["email"].fillna("").str.strip() != ""
        _with_email = df_exp_dedup[_has_email_mask].drop_duplicates(subset=["email"], keep="first")
        _without_email = df_exp_dedup[~_has_email_mask]
        df_exp_dedup = pd.concat([_with_email, _without_email]).sort_index()
        _n_removed = _n_before - len(df_exp_dedup)

        _exp_info = f"**{len(df_exp_dedup)} contacts**"
        if _n_removed > 0:
            _exp_info += f" · <span style='color:#c9800a;'>{_n_removed} doublon(s) exclus de l'export</span>"
        _exp_info += f" · {len(df_exp_dedup[df_exp_dedup['email'].fillna('').str.strip()!=''])} avec email"
        st.markdown(_exp_info, unsafe_allow_html=True)

        # Export CSV contacts (3 fichiers séparés — openpyxl non dispo sur Python 3.14)
        col_exp1, col_exp2, col_exp3 = st.columns(3)
        with col_exp1:
            buf_ct = io.BytesIO()
            df_exp_dedup.drop(columns=["id","created_at"],errors="ignore").to_csv(buf_ct, index=False, encoding="utf-8-sig")
            st.download_button(
                "⬇ Contacts CSV",
                buf_ct.getvalue(),
