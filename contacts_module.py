# contacts_module.py — v2.10 — 1776951789
# ══════════════════════════════════════════════════════════════════════════════
# MODULE CONTACTS v2
    ...
    with tab_book:
        ...
        if df_mod.empty:
            ...
            for _, mod in df_mod.iterrows():
                ...
                def _cell(label, val, w="auto"):
                    col = EW_B if val and val != "—" else "#ccc"
                    return f'''<div style="border:1px solid #e8e0d4;border-radius:6px;padding:5px 10px;min-width:{w};flex:1;background:#fff;">
  <div style="font-family:DM Mono,monospace;font-size:7px;color:#bbb;text-transform:uppercase;letter-spacing:.1em;">{label}</div>
  <div style="font-size:11px;color:{col};font-weight:500;line-height:1.3;word-break:break-word;">{val or "—"}</div>
</div>'''

                st.markdown(f"""
<div style="display:flex;gap:0;background:#f5f0e8;border:1px solid #d9c8ae;border-radius:8px;overflow:hidden;margin-bottom:10px;min-height:138px;">

  <!-- PHOTO -->
  <div style="width:200px;min-width:200px;background:#e8e0d4;overflow:hidden;position:relative;">
    {_img_html if _img_html else f'<div style="width:200px;height:138px;display:flex;align-items:center;justify-content:center;color:#c8bfb3;font-family:DM Mono,monospace;font-size:8px;">PHOTO</div>'}
    <div style="position:absolute;top:6px;left:6px;background:rgba(255,255,255,.85);border-radius:4px;padding:2px 6px;font-family:DM Mono,monospace;font-size:8px;font-weight:700;color:{_sc};">{mod.get("statut","")}</div>
  </div>

  <!-- INFOS -->
  <div style="flex:1;padding:10px 14px;display:flex;flex-direction:column;gap:5px;">

    <!-- Ligne 1 : identité -->
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:2px;">
      <span style="font-size:15px;font-weight:600;color:#1a1a1a;">{_nm}</span>
      {_valide_dot}
      {_avec_dot}
      <span style="font-family:DM Mono,monospace;font-size:9px;color:{EW_B};margin-left:4px;">{_gn} · {_lc}{", " if _lc and _py else ""}{_py}</span>
      {f'<span style="font-family:DM Mono,monospace;font-size:9px;color:{EW_V};">@{_ig}</span>' if _ig else ''}
      <span style="margin-left:auto;font-family:DM Mono,monospace;font-size:8px;color:#bbb;">{_res}</span>
    </div>

    <!-- Ligne 2 : mensurations + contact -->
    <div style="display:flex;gap:5px;flex-wrap:wrap;">
      {_cell("Taille",_ht,"50px")}
      {_cell("Poids",_wt,"50px")}
      {_cell("Pointure",_pt,"55px")}
      {_cell("Taille vêt.",_tv,"55px")}
      {_cell("E-mail",_mail)}
      {_cell("Téléphone",_tel)}
    </div>

    <!-- Ligne 3 : style + dispo -->
    <div style="display:flex;gap:5px;flex-wrap:wrap;">
      {_cell("Style / Type",_sty)}
      {_cell("Période dispo",_dp)}
      {_cell("Shooting prévu",_sh)}
    </div>

    <!-- Ligne 4 : pièces + tenue + notes -->
    <div style="display:flex;gap:5px;flex-wrap:wrap;">
      {_cell("Pièces à porter",_pa)}
      {_cell("Tenue complète",_ten)}
      {_cell("Notes",_no)}
    </div>
