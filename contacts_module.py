                ...
                with cols_book[i % 3]:
                    ...
                    if mod.get("photo_data") is not None:
                        try:
                            ...
                            st.markdown(f"""
<div style="aspect-ratio:1114/552;background:{EW_C};overflow:hidden;margin-bottom:8px;">
                ...
                if can_fn("contacts_edit"):
                    ...
                    if ph and st.button("✓ Upload", key=f"mphu_{mid}"):
                        try:
                            ...
                            _i = _PIL.open(ph).convert("RGB").resize((1114,552),_PIL.LANCZOS)
                            _b = _io.BytesIO(); _i.save(_b,format="JPEG",quality=92)
                            _pb = _b.getvalue()
                        except Exception: _pb = ph.read()
