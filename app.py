  ...
  <div style="text-align:right;">
    ...
    with tab_sa:
            ...
            with st.expander("📍 Gérer les localisations"):
                _locs_current = get_localisations(get_conn())
            ...
            with st.expander("⚙ Gérer les localisations"):
                _cur_locs = [r[0] for r in conn.execute("SELECT nom FROM localisations ORDER BY nom").fetchall()]
                ...
                for _l in _cur_locs:
                    ...
                    with lc2:
                        if st.button("🗑", key=f"del_loc_{_l}", help="Supprimer"):
                            conn.execute("DELETE FROM localisations WHERE nom=?", (_l,))
                ...
                if st.button("➕ Ajouter localisation", key="btn_add_loc"):
                    if new_loc_name.strip():
                        conn.execute("INSERT OR IGNORE INTO localisations (nom, created_by) VALUES (?,?)",
