...
try:
    from github_persistence import (
        ...
        save_async as _gh_save_async,
    )
    _GH_PERSISTENCE = _gh_configured()
except Exception:
    _GH_PERSISTENCE = False
    _gh_load = _gh_save = _gh_save_async = None
...
class _AutoSaveConn:
    ...
    def commit(self):
        self._conn.commit()
        # Sauvegarde GitHub async après chaque commit (non bloquant)
        if _GH_PERSISTENCE and _gh_save_async and not _USE_SUPABASE:
            _gh_save_async(DB_PATH)
...
def init_db():
    ...
    # Sauvegarde GitHub (async, non bloquant)
    if _GH_PERSISTENCE and _gh_save_async and not _USE_SUPABASE:
        _gh_save_async(DB_PATH, "init: eastwood db structure")
...
<div style="font-family:'DM Mono',monospace;font-size:8px;color:#5a5650;
  ...
  Données
    ...
    if _GH_PERSISTENCE and not _USE_SUPABASE:
        # Mode GitHub persistence activé
        st.markdown("""
<div style="font-family:'DM Mono',monospace;font-size:9px;color:#395f30;padding:2px 0 6px;">
  ● Sync GitHub actif
</div>""", unsafe_allow_html=True)
        if st.button("☁ Sauvegarder maintenant", key="btn_gh_save"):
            with st.spinner("Sauvegarde..."):
                ok = _gh_save(DB_PATH, "manual: save from app")
            if ok:
                st.success("✓ Données sauvegardées sur GitHub.")
            else:
                st.error("Erreur — vérifiez GITHUB_TOKEN dans les secrets.")
    else:
...
<div style="font-family:'DM Mono',monospace;font-size:9px;color:#c9800a;padding:2px 0 6px;">
  ● Mode local (config GitHub manquante)
    ...
    # Restaurer depuis upload (toujours disponible)
    _db_upload = st.file_uploader(
        "↑ Restaurer",
        type=["db"],
        key="db_restore_upload",
        help="Upload eastwood_backup.db pour restaurer"
    )
    if _db_upload is not None:
        if st.button("✓ Restaurer", key="btn_restore_db"):
            try:
                _db_data = _db_upload.read()
                with open(DB_PATH, "wb") as _f_out:
                    _f_out.write(_db_data)
                with open(DB_BACKUP_PATH, "wb") as _f_bk:
                    _f_bk.write(_db_data)
                if _GH_PERSISTENCE and _gh_save_async:
                    _gh_save_async(DB_PATH, "restore: uploaded backup")
                st.success("✓ Données restaurées.")
                st.rerun()
            except Exception as _e_r:
                st.error(f"Erreur : {_e_r}")
