...
def run_data_migration(conn):
    ...
    try:
        done = conn.execute("SELECT value FROM app_settings WHERE key='data_migration_v1'").fetchone()
        if done:
            return
    except Exception:
        pass
...
def _insert_migration_data(conn):
    ...
    try:
        ...
        conn.commit()
        c.execute("INSERT OR REPLACE INTO app_settings (key,value) VALUES ('data_migration_v1','done')")
        conn.commit()
    except Exception:
