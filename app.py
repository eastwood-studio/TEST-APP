...
def init_db():
    ...
    # Auto-backup silencieux après init
    try:
...
<div style="font-family:'DM Mono',monospace;font-size:8px;color:#5a5650;
  ...
  Données
    ...
    # Bouton télécharger la DB complète
    if _os2.path.exists(DB_PATH):
        ...
        st.download_button(
            ...
            help="Télécharge toutes tes données. Renomme ce fichier 'eastwood_backup.db' et mets-le dans ton repo GitHub pour qu'il soit restauré au prochain démarrage.",
            key="dl_db_backup"
        )

    # Bouton restaurer depuis un fichier uploadé
    _db_upload = st.file_uploader(
        "↑ Restaurer une sauvegarde",
        type=["db"],
        key="db_restore_upload",
        help="Upload un fichier eastwood_backup.db pour restaurer tes données"
    )
    if _db_upload is not None:
        if st.button("✓ Appliquer la restauration", key="btn_restore_db"):
            try:
                _db_data = _db_upload.read()
                with open(DB_PATH, "wb") as _f_out:
                    _f_out.write(_db_data)
                # Aussi sauvegarder comme backup
                with open(DB_BACKUP_PATH, "wb") as _f_bk:
                    _f_bk.write(_db_data)
                st.success("✓ Données restaurées. L'app va redémarrer.")
                st.rerun()
            except Exception as _e_restore:
                st.error(f"Erreur restauration : {_e_restore}")
