  ...
  <strong>Envoi</strong> : {row.get('date_envoi','—') or '—'}
                    ...
                    if can("commandes_write"):
                        ...
                        for i, (label, val) in enumerate(steps.items()):
                            ...
                            if can("finance_write"):  # Jules only
                                st.markdown("---")
                                if st.button("🗑 Supprimer cette commande", key=f"del_cmd_{sel_cmd_id}",
                                             help="Suppression définitive"):
                                    conn_del = get_conn()
                                    conn_del.execute("DELETE FROM commandes WHERE id=?", (sel_cmd_id,))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    _ts = ["📋 Inventaire", "🏭 Stock Produits Finis", "🔬 Samples", "➕ Ajouter article"]
    _os = st.tabs(_ts)
    tab_si   = _os[0]
    tab_spf  = _os[1]
    tab_smp  = _os[2]
