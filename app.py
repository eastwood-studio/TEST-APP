...
<div style="height:1px;background:#1c1a17;margin-bottom:10px;"></div>""",
    ...
    _nav_list = list(_NAV_GROUPS.items())
    for _i, (_idx, _lbl) in enumerate(_nav_list):
        _pad = "2px 0 4px" if _idx == 0 else "10px 0 4px"
        _nth = _idx + 1
        _sep_css += f"""
[data-testid="stSidebar"] [data-testid="stRadio"] label:nth-of-type({_nth})::before{{
  content:"{_lbl.upper()}";display:block;
  font-family:'DM Mono',monospace;font-size:7px;letter-spacing:.20em;
  color:#6a6055;padding:{_pad};pointer-events:none;}}"""
    ...
    if tab_ca is not None:
        with tab_ca:
            ...
            with n1:
                ...
                _conn_nc2 = get_conn()
                try:
                    _last_cmd = _conn_nc2.execute("SELECT MAX(num_commande) FROM commandes WHERE num_commande LIKE 'CMD-EWS-%'").fetchone()
                    _last_n = 0
                    if _last_cmd and _last_cmd[0]:
                        try: _last_n = int(_last_cmd[0].split("-")[-1])
                        except: pass
                    _auto_num = f"CMD-EWS-{_last_n+1:06d}"
                except Exception:
                    _auto_num = "CMD-EWS-000001"
                finally:
                    _conn_nc2.close()
                num_cmd = _auto_num
                st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:11px;background:#f5f0e8;padding:6px 10px;">N° auto : {num_cmd}</div>', unsafe_allow_html=True)
