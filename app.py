...
try:
    from products_module import page_produits, init_products_db
            ...
            if st.button("✓ Enregistrer l'opération"):
                if not description:
                    ...
                    if ref_produit and unite in ("Article","Pièces","Mètre","Kg","Lot"):
                        sync_stock_from_transaction(conn, ref_produit, type_op, quantite)
                    conn.commit(); conn.close()
                    st.success("✓ Opération enregistrée." + (
                        f" Stock mis à jour pour {ref_produit}." if ref_produit else ""))
                    st.rerun()
