...
try:
    from products_module import (page_produits, page_couts_produits, init_products_db, sync_from_commande, get_collections_dynamic, COST_SECTIONS, compute_cost_totals, sync_product_status)
...
PERMISSIONS = {
    "superuser": {
        ...
        "tva_read": True, "tva_write": True,
        "couts_read": True, "couts_write": True,
        "products_delete": True,
    },
    "ops": {
        "finance_read": True, "finance_write": False,
        "ops_read": True, "ops_write": False,
        "commandes_read": True, "commandes_write": True,
        "stock_read": True, "stock_write": True,
        "contacts_read": True, "contacts_add": True,
        "contacts_edit": True, "contacts_delete": False,
        "tva_read": True, "tva_write": False,    # synthèse annuelle seule
        "couts_read": True, "couts_write": False, # lecture seule
...
_TYPES_STOCK = ["Produit fini", "Matière première", "Composant", "Packaging", "Sample", "Autre"]
...
/* Masquer éléments Streamlit indésirables */
...
<div style="text-align:center;margin-bottom:32px;">
  <img src="data:image/svg+xml;base64,{logo_b64}"
       style="width:180px;display:block;margin:0 auto 14px;"/>
  <div style="font-family:'DM Mono',monospace;font-size:8px;letter-spacing:.22em;
       color:#8a7968;text-transform:uppercase;">
    ...
    with st.container():
        st.markdown("""
<div style="background:#fff;border:1px solid #e0dbd2;
     padding:36px 32px 28px;box-shadow:0 1px 8px rgba(0,0,0,.05);">
</div>""", unsafe_allow_html=True)
...
<div style="font-family:'DM Mono',monospace;font-size:9px;color:#aaa49a;
     text-align:center;margin-top:24px;letter-spacing:.12em;">
  EASTWOOD STUDIO © 2025 · homemade by Jules Léger
</div>""", unsafe_allow_html=True)
...
def run_data_migration(conn):
    """Migration one-shot des données produits/stock/fournisseurs/packagings."""
    try:
        done = conn.execute("SELECT value FROM app_settings WHERE key='data_migration_v2'").fetchone()
        if done:
            return
    except Exception:
        pass
...
def _insert_migration_data(conn):
    """Insère toutes les données réelles Eastwood Studio depuis les fichiers sources."""
    try:
        c = conn.cursor()
        # Purger les anciens produits (mauvais SKU et données incorrectes)
        c.execute("DELETE FROM product_components")
        c.execute("DELETE FROM product_archives")
        c.execute("DELETE FROM products")
        c.execute("DELETE FROM stock WHERE type_produit IN ('Matière première','Composant','Packaging')")
        c.execute("DELETE FROM packaging_items")
        c.execute("DELETE FROM packaging_types")
        conn.commit()

        PRODUCTS = [
            ("Waterfowl Jacket","Chocolate","EWSJACKET-001A","Chapter I — Hunting & Fishing","T1(S) / T2(M) / T3(L)","Paris, France","3 months",5,"Out of stock","Veste Waterfowl en laine Whipcord Chocolate avec doublure tartan Shadow. Broderie patch EWS."),
            ("Waterfowl Jacket","Tobacco","EWSJACKET-001B","Chapter I — Hunting & Fishing","T1(S) / T2(M) / T3(L)","Paris, France","3 months",5,"Out of stock","Veste Waterfowl en laine Whipcord Tobacco avec doublure tartan Shadow. Broderie patch EWS."),
            ("Waterfowl Jacket","Greige","EWSJACKET-001C","Chapter I — Hunting & Fishing","T1(S) / T2(M) / T3(L)","Paris, France","3 months",5,"Out of stock","Veste Waterfowl en laine Whipcord Greige avec doublure tartan Shadow. Broderie patch EWS."),
            ("Waterfowl Jacket","Sand","EWSJACKET-001D","Chapter I — Hunting & Fishing","T1(S) / T2(M) / T3(L)","Paris, France","3 months",5,"Out of stock","Veste Waterfowl en laine Whipcord Sand avec doublure tartan Clear. Broderie patch EWS."),
            ("Waterfowl Jacket","Black","EWSJACKET-001E","Chapter I — Hunting & Fishing","T1(S) / T2(M) / T3(L)","Paris, France","4 months",5,"Out of stock","Veste Waterfowl en laine Whipcord Black avec doublure tartan Clear. Broderie patch EWS."),
            ("Miura Jacket","","EWSJACKET-002A","Chapter II — Le Souvenir","T1(S) / T2(M) / T3(L) / T4(XL)","Kiriu, Japan","4 months",5,"Out of stock","Veste Miura en rayonne satinée rembourrée verte. Broderie Yokoburi dos cigogne. Fabrication Kiryu Jumper Lab."),
            ("Akagi Jacket","","EWSJACKET-003A","Chapter II — Le Souvenir","T1(S) / T2(M) / T3(L) / T4(XL)","Kiriu, Japan","3 months",5,"Out of stock","Veste Akagi en gabardine de rayonne Tobacco. Broderie Yokoburi dos Paon 1950. Fabrication Kiryu Jumper Lab."),
            ("Recherche Club Shirt","Cloud Blue","EWSSHIRT-001A","Chapter II — Le Souvenir","T1(S) / T2(M) / T3(L) / T4(XL)","Paris, France","3 months",5,"Out of stock","Chemise Recherche Club en lin & viscose bleu pigeon avec voile coton. Broderie chainstitch dos."),
            ("Recherche Club Shirt","Pastel Green","EWSSHIRT-001B","Chapter II — Le Souvenir","T1(S) / T2(M) / T3(L) / T4(XL)","Paris, France","3 months",5,"Out of stock","Chemise Recherche Club en lin & viscose vert pastel avec voile coton. Broderie chainstitch dos."),
            ("Lutèce Plage Shirt","","EWSSHIRT-002A","Chapter II — Le Souvenir","T1(S) / T2(M) / T3(L) / T4(XL)","Paris, France","3 months",5,"Out of stock","Chemise Lutèce Plage en sergé de viscose all-over imprimé. Fabrication Paris."),
            ("Souvenir Cap","Deep Rust","EWSGEAR-001A","Chapter II — Le Souvenir","Adjustable Size","Paris, France","3 months",5,"Out of stock","Casquette Souvenir en coton avec lanière cuir végan. Coloris Deep Rust."),
            ("Souvenir Cap","Faded Green","EWSGEAR-001B","Chapter II — Le Souvenir","Adjustable Size","Paris, France","3 months",5,"Out of stock","Casquette Souvenir en coton avec lanière cuir végan. Coloris Faded Green."),
            ("Paris le trésor Silk Square","","EWSGEAR-002A","Chapter II — Le Souvenir","Unique Size (55cm×55cm)","Paris, France","3 months",5,"Out of stock","Carré de soie 55×55cm motif Trésor. Fabrication Paris."),
            ("Memory Wallet","","EWSGEAR-003A","Chapter II — Le Souvenir","Unique Size (10cm×6.6cm)","Paris, France","3 months",5,"Out of stock","Portefeuille Memory en cuir de veau tanné marron. Peinture main Xavier Boully."),
        ]
        for nom,coul,sku,coll,tailles,madein,deliv,moq,statut,desc in PRODUCTS:
            c.execute("""INSERT OR REPLACE INTO products
                (nom,ref,collection,statut,categorie,description,couleurs,tailles,made_in,moq,delivery,origine)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                (nom,sku,coll,statut,"Produit fini",desc,coul,tailles,madein,moq,deliv,"Paris, France"))
        ...
        STOCK = [
            ...
            ("EWSCELL-003A","Sergé de viscose All-Over","Matière première","Tissus imprimé / motif",21.67,0,"Mètre"),
            ("EWSCOTT-002A","Coton blanc casquette","Matière première","Maille",0,0,"Pièces"),
            ("EWSCELL-004A","Soie Motif Trésor","Matière première","Tissus imprimé / motif",50.0,0,"Pièces"),
            ...
            ("EWSTRIM-005A","Zip double face WALDES","Composant","Zip",0,0,"Pièces"),
            ("EWSTRIM-004A","Zip chainette doré Talon Vintage 1960","Composant","Zip",9.0,7,"Pièces"),
            ("EWSTRIM-004B","Zip doré Talon Vintage","Composant","Zip",0,0,"Pièces"),
            ("EWSTRIM-002A","Boutton simple Maron 25mm","Composant","Bouton",1.0,0,"Pièces"),
            ("EWSTRIM-006A","Boutons cat's eye noir","Composant","Bouton",0,0,"Pièces"),
            ("EWSTRIM-003A","Boutons nacrés Eastwood Studio","Composant","Bouton",0.41,150,"Pièces"),
            ("EWSTRIM-001A","Boutons Équipement Militaires Vintage 1960","Composant","Bouton",1.3,5,"Pièces"),
            ("EWSPKG-001A","Étiquette de composition","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-002A","Étiquette de taille T1 (S)","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-002B","Étiquette de taille T2 (M)","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-002C","Étiquette de taille T3 (L)","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-002D","Étiquette de taille T4 (XL)","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-003A","Étiquette fabrication Française","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-004A","Étiquette textile Eastwood Studio","Composant","Étiquette",0,0,"Pièces"),
            ("EWSETQ-005A","Tag numéro identification","Packaging","Packaging",0,0,"Pièces"),
        ...
        for ref,nom,type_p,sous_t,cout,qte,unite in STOCK:
            c.execute("""INSERT OR REPLACE INTO stock (ref,description,type_produit,sous_type,prix_unitaire,qte_stock,unite)
                VALUES (?,?,?,?,?,?,?)""", (ref,nom,type_p,sous_t,cout,qte,unite))

        STOCK_PF = {
            "EWSJACKET-001A":0,"EWSJACKET-001B":1,"EWSJACKET-001C":0,"EWSJACKET-001D":0,"EWSJACKET-001E":0,
        ...
        for sku,qte in STOCK_PF.items():
            prod_r = c.execute("SELECT nom,couleurs FROM products WHERE ref=?", (sku,)).fetchone()
            if prod_r:
                desc_pf = prod_r[0] + (" — " + prod_r[1] if prod_r[1] else "")
                c.execute("INSERT OR REPLACE INTO stock (ref,description,type_produit,qte_stock,unite) VALUES (?,?,'Produit fini',?,'Pièces')", (sku,desc_pf,qte))

        COMPS = {
            "EWSJACKET-001A":[("Laine Whipcord Chocolate","MP Principale (Main Fabric)","2","Mètre","EWSWOOL-001A"),("Mix coton laine tartan Shadow","Doublure","1.7","Mètre","EWSCOTT-001A"),("Patch brodé logo EWS","Broderie","1","Pièces",""),("Boutton simple Maron 25mm","Bouton","4","Pièces","EWSTRIM-002A"),("Boutons Équipement Militaires Vintage 1960","Bouton","8","Pièces","EWSTRIM-001A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSJACKET-001B":[("Laine Whipcord Tobacco","MP Principale (Main Fabric)","2","Mètre","EWSWOOL-001B"),("Mix coton laine tartan Shadow","Doublure","1.7","Mètre","EWSCOTT-001A"),("Patch brodé logo EWS","Broderie","1","Pièces",""),("Boutton simple Maron 25mm","Bouton","4","Pièces","EWSTRIM-002A"),("Boutons Équipement Militaires Vintage 1960","Bouton","8","Pièces","EWSTRIM-001A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSJACKET-001C":[("Laine Whipcord Greige","MP Principale (Main Fabric)","2","Mètre","EWSWOOL-001C"),("Mix coton laine tartan Shadow","Doublure","1.7","Mètre","EWSCOTT-001A"),("Patch brodé logo EWS","Broderie","1","Pièces",""),("Boutton simple Maron 25mm","Bouton","4","Pièces","EWSTRIM-002A"),("Boutons Équipement Militaires Vintage 1960","Bouton","8","Pièces","EWSTRIM-001A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSJACKET-001D":[("Laine Whipcord Sand","MP Principale (Main Fabric)","2","Mètre","EWSWOOL-001D"),("Mix coton laine tartan Clear","Doublure","1.7","Mètre","EWSCOTT-001B"),("Patch brodé logo EWS","Broderie","1","Pièces",""),("Boutton simple Maron 25mm","Bouton","4","Pièces","EWSTRIM-002A"),("Boutons Équipement Militaires Vintage 1960","Bouton","8","Pièces","EWSTRIM-001A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSJACKET-001E":[("Laine Whipcord Black","MP Principale (Main Fabric)","2","Mètre","EWSWOOL-001E"),("Mix coton laine tartan Clear","Doublure","1.7","Mètre","EWSCOTT-001B"),("Patch brodé logo EWS","Broderie","1","Pièces",""),("Boutton simple Maron 25mm","Bouton","4","Pièces","EWSTRIM-002A"),("Boutons Équipement Militaires Vintage 1960","Bouton","8","Pièces","EWSTRIM-001A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSJACKET-002A":[("Rayonne satinée rembourée Verte","MP Principale (Main Fabric)","2","Mètre","EWSCELL-001A"),("Rayonne satinée Nishijin All-Over","Doublure","2","Mètre","EWSCELL-001B"),("Dos brodé cigogne Yokoburi","Broderie","1","Pièces",""),("Zip double face WALDES","Zip","1","Pièces","EWSTRIM-005A"),("Boutons cat's eye noir","Bouton","2","Pièces","EWSTRIM-006A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSJACKET-003A":[("Gabardine de rayonne Tobacco","MP Principale (Main Fabric)","2","Mètre","EWSCELL-002A"),("Rayonne satinée Crème","Doublure","2","Mètre",""),("Dos brodé Paon Patron 1950 Yokoburi","Broderie","1","Pièces",""),("Zip chainette doré Talon Vintage 1960","Zip","1","Pièces","EWSTRIM-004A"),("Zip doré Talon Vintage","Zip","1","Pièces","EWSTRIM-004B"),("Boutons cat's eye noir","Bouton","2","Pièces","EWSTRIM-006A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSSHIRT-001A":[("Lin & Viscose Bleu Pigeon","MP Principale (Main Fabric)","1.7","Mètre","EWSBLND-001A"),("Lin & Viscose Blanc Cassé","MP Secondaire","0.9","Mètre","EWSBLND-001C"),("Voile de coton blanc","Doublure","0.6","Mètre","EWSCOTT-003A"),("Dos brodé Club Recherche EWS Chainstitch","Broderie","1","Pièces",""),("Boutons nacrés Eastwood Studio","Bouton","10","Pièces","EWSTRIM-003A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            ...
            "EWSSHIRT-002A":[("Sergé de viscose All-Over","MP Principale (Main Fabric)","1.9","Mètre","EWSCELL-003A"),("Voile de coton blanc","Doublure","0.6","Mètre","EWSCOTT-003A"),("Boutons nacrés Eastwood Studio","Bouton","5","Pièces","EWSTRIM-003A"),("Étiquette de composition","Étiquette","1","Pièces","EWSPKG-001A"),("Étiquette de taille","Étiquette","1","Pièces",""),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A"),("Étiquette textile Eastwood Studio","Étiquette","1","Pièces","EWSETQ-004A")],
            "EWSGEAR-001A":[("Coton blanc casquette","MP Principale (Main Fabric)","1","Pièces","EWSCOTT-002A"),("Lanière cuir végan","MP Secondaire","1","Pièces","EWSLEAT-002A"),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A")],
            "EWSGEAR-001B":[("Coton blanc casquette","MP Principale (Main Fabric)","1","Pièces","EWSCOTT-002A"),("Lanière cuir végan","MP Secondaire","1","Pièces","EWSLEAT-002A"),("Étiquette fabrication Française","Étiquette","1","Pièces","EWSETQ-003A")],
            "EWSGEAR-002A":[("Soie Motif Trésor","MP Principale (Main Fabric)","1","Pièces","EWSCELL-004A")],
            "EWSGEAR-003A":[("Wallet Cuir de veau tanné marron","MP Principale (Main Fabric)","1","Pièces","EWSLEAT-001A"),("Peinture main Xavier Boully","Broderie","1","Pièces","")],
        }
                ...
                for nom_c,cat_c,qte_c,unit_c,ref_c in comps:
                    try:
                        c.execute("INSERT INTO product_components (product_id,nom_exact,categorie_comp,quantite,unite,ref_stock) VALUES (?,?,?,?,?,?)",
                            (prod[0],nom_c,cat_c,float(str(qte_c).replace(",",".")),unit_c,ref_c))
                    except Exception: pass

        ARCHIVES = [
            ("Chapter I — Hunting & Fishing","Shooting","Shooting Chapter I — Berlin","2024","Shooting Berlin Tempelhof avec modèles. Ambiance chasse et pêche."),
            ("Chapter I — Hunting & Fishing","Moodboard","Moodboard Hunting & Fishing","2023","Références visuelles chasse anglaise, pêche vintage, vestes Whipcord."),
            ("Chapter II — Le Souvenir","Shooting","Shooting Chapter II — Berlin","2025","Shooting Berlin été 2025. Références Souvenir japonais, Berlin, Paris."),
            ("Chapter II — Le Souvenir","Moodboard","Moodboard Le Souvenir","2024","Références visuelles sukajan japonais, casquettes vintage, imprimés all-over."),
            ("Chapter II — Le Souvenir","Référence","Référence Yokoburi Kiryu","2024","Broderie Yokoburi traditionnelle japonaise. Kiryu Jumper Lab."),
        ]
        for coll,arc_type,arc_nom,annee,details in ARCHIVES:
            ...
            for (pid,) in prods_c:
                c.execute("INSERT INTO product_archives (product_id,type_archive,nom_archive,annee,details) VALUES (?,?,?,?,?)", (pid,arc_type,arc_nom,int(annee),details))

        FOURNISSEURS = [
            ("Rêve de Gosse","Matière première","France","Laines Whipcord et tartan pour Waterfowl Jackets"),
            ("Kiryu Jumper Lab","Production / Confection","Japon","Fabrication Miura & Akagi Jackets. Broderie Yokoburi."),
            ("Le bar à broder","Impression / Broderie","France","Broderie main patch logo EWS sur Waterfowl Jackets"),
            ("Salhedine Chainstitch","Impression / Broderie","France","Broderie chainstitch dos pour Recherche Club Shirt"),
            ("Dam Bouton","Composant","France","Boutons simples marron 25mm"),
            ("A&A Patrons (ma fashion mercerie)","Composant","France","Étiquettes de composition"),
            ("Rubantin","Composant","France","Étiquettes de taille"),
            ("mapetitemercerie","Composant","France","Étiquettes fabrication française et textile Eastwood Studio"),
            ("Textrend.fr","Matière première","France","Lin & Viscose pour Recherche Club Shirt"),
            ("Insho Atelier","Matière première","France","Sergé de viscose All-Over et Soie Motif Trésor"),
            ("Xiangtong","Matière première","France","Coton casquette et lanière cuir végan"),
            ("Yiwu Aiyi E-Commerce Firm","Composant","Chine","Boutons nacrés Eastwood Studio"),
            ("LeBonCoin Vintage","Composant","France","Boutons équipement militaire vintage 1960"),
            ("Ebay (vinty82)","Composant","France","Zip chainette doré Talon Vintage 1960"),
            ("Laperruque","Matière première","France","Cuir de veau tanné marron pour Memory Wallet"),
            ("Xavier Boully","Impression / Broderie","France","Peinture main sur Memory Wallet"),
        ]
        for nom_f,sous_t,pays,notes in FOURNISSEURS:
            ...
            if not ex:
                c.execute("INSERT INTO contacts (nom,type_contact,sous_type,pays,notes,importance) VALUES (?,'Fournisseur',?,?,?,'Normal')", (nom_f,sous_t,pays,notes))
            ex_f = c.execute("SELECT id FROM fournisseurs WHERE nom=?", (nom_f,)).fetchone()
            if not ex_f:
                try:
                    c.execute("INSERT INTO fournisseurs (nom,sous_type,pays,notes,deja_travaille,statut_fourn,importance) VALUES (?,?,?,?,1,'Actif — on travaille','Normal')", (nom_f,sous_t,pays,notes))
                except Exception: pass

        PKGS = [
            ("Packaging Wholesale","B2B — Tag + feuille recherche",[("EWSETQ-005A","Tag numéro identification",1),("EWSPKG-003A","Feuille recherche A4 craft",1)]),
            ("Packaging Chapter I","Waterfowl & Archives B2C",[("EWSETQ-005A","Tag numéro identification",1),("EWSPKG-004A","Sticker logo blanc cercle",2),("EWSPKG-002A","Pochette enveloppe postal XL craft",1),("EWSPKG-003A","Feuille recherche A4 craft",1)]),
            ("Packaging Prêt à Porté","B2C site web / popup",[("EWSETQ-005A","Tag numéro identification",1),("EWSPKG-004A","Sticker logo blanc cercle",2),("EWSPKG-003A","Feuille recherche A4 craft",1)]),
            ("Packaging Accessoires","B2C accessoires / popup",[("EWSETQ-005A","Tag numéro identification",1),("EWSPKG-004A","Sticker logo blanc cercle",2),("EWSPKG-003A","Feuille recherche A4 craft",1)]),
        ]
        for pkg_nom,pkg_notes,pkg_items in PKGS:
            ...
            for ref_i,nom_i,qte_i in pkg_items:
                ex_pi = c.execute("SELECT id FROM packaging_items WHERE packaging_type_id=? AND ref_stock=?", (pkg_id,ref_i)).fetchone()
                if not ex_pi:
                    c.execute("INSERT INTO packaging_items (packaging_type_id,ref_stock,nom_item,quantite) VALUES (?,?,?,?)", (pkg_id,ref_i,nom_i,qte_i))

        conn.commit()
        c.execute("INSERT OR REPLACE INTO app_settings (key,value) VALUES ('data_migration_v2','done')")
        conn.commit()
    except Exception:
...
def init_db():
    ...
    # Chaque CREATE TABLE est isolé pour éviter qu'une erreur bloque tout
    for _sql in [
        """CREATE TABLE IF NOT EXISTS transactions (
            ...
            devise TEXT DEFAULT 'EUR', taux_change REAL DEFAULT 1.0,
            montant_original REAL,
        ...
        """CREATE TABLE IF NOT EXISTS stock (
            ...
            type_produit TEXT,
            localisation TEXT DEFAULT 'En attente',
            qte_stock REAL DEFAULT 0, qte_utilisee REAL DEFAULT 0,
        ...
        try:
            ...
            pass  # Table déjà existante ou schéma incompatible — on continue

    # Seed localisations si vide
    try:
        if c.execute("SELECT COUNT(*) FROM localisations").fetchone()[0] == 0:
            for _loc in ["Chez Jules","Chez Corentin","Chez Alexis","En transit","Autre"]:
                c.execute("INSERT OR IGNORE INTO localisations (nom) VALUES (?)", (_loc,))
    except Exception:
        pass

    # Pas de seed demo — les opérations sont créées manuellement ou importées
    pass
    ...
    # Supprimer les données demo si elles existent (MIRA-001, GENE fantômes)
    try:
        _demo_marker = c.execute("SELECT value FROM app_settings WHERE key='demo_purged_v1'").fetchone()
        if not _demo_marker:
            c.execute("""DELETE FROM transactions WHERE
                (ref_produit IN ('MIRA-001','GENE','PANT-003','COMP-002') AND annee IN (2025,2026))
                OR (description LIKE '%Brévo%' AND annee=2025)
                OR (description LIKE '%Revolut%' AND annee=2025 AND total_ht=25)
                OR (description LIKE '%Miura%' AND annee=2025)
                OR (description LIKE '%Soierie Lyon%')
                OR (description LIKE '%Atelier Belleville%' AND annee=2025)
                OR (description LIKE '%Tokyo%' AND source LIKE '%Shibuya%')
            """)
            c.execute("INSERT OR REPLACE INTO app_settings (key,value) VALUES ('demo_purged_v1','1')")
    except Exception:
        pass
    ...
    # Migration données produits/stock/fournisseurs (one-shot)
    try:
        _conn_data = get_conn()
        run_data_migration(_conn_data)
        _conn_data.close()
    except Exception:
        pass
...
def get_localisations():
    try:
        _c = get_conn()
        rows = _c.execute("SELECT nom FROM localisations ORDER BY nom").fetchall()
        _c.close()
        return [r[0] for r in rows] if rows else ["Chez Jules","Chez Corentin","Chez Alexis","En transit","Autre"]
    except Exception:
        return ["Chez Jules","Chez Corentin","Chez Alexis","En transit","Autre"]

LOCALISATIONS = get_localisations()  # Sera rechargé à chaque rerun
    ...
    try:
        ...
        try:
            conn.execute("""CREATE TABLE IF NOT EXISTS stock (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                description TEXT, ref TEXT UNIQUE, type_produit TEXT,
                localisation TEXT DEFAULT 'En attente',
                qte_stock REAL DEFAULT 0, qte_utilisee REAL DEFAULT 0,
                qte_vendue REAL DEFAULT 0, prix_unitaire REAL DEFAULT 0,
                besoin_reassort INTEGER DEFAULT 0, notes TEXT
            )"""); conn.commit()
        except Exception: pass
...
<div style="height:1px;background:#1c1a17;margin-bottom:10px;"></div>""",
    ...
    # Pages réservées à Jules (superuser) — masquées pour Corentin et Alexis
    _JULES_ONLY_PAGES = {
        "📋  Finance",
        "⚖️  Balance interne",
    }
    # TVA : visible pour tous, mais en mode lecture seule
    # Coûts produits : visible pour tous, mais en mode lecture seule
    # Opérations visible pour tous, Commandes aussi — mais écriture bloquée dans le module
    _user_role_now = st.session_state.get("user_role","")
    if _user_role_now == "superuser":
        NAV_PAGES = _ALL_NAV_PAGES
    else:
        NAV_PAGES = [p for p in _ALL_NAV_PAGES if p not in _JULES_ONLY_PAGES]

    # Recalculer les groupes nav selon les pages visibles
    _NAV_GROUP_CSS_MAP = {}
    for i, p in enumerate(NAV_PAGES):
        if "Accueil" in p:
            _NAV_GROUP_CSS_MAP[i+1] = ("GÉNÉRAL", "2px 0 4px")
        elif "Opérations" in p:
            _NAV_GROUP_CSS_MAP[i+1] = ("OPÉRATIONS", "14px 0 4px")
        elif "Finance" in p:
            _NAV_GROUP_CSS_MAP[i+1] = ("FINANCE & COMPTABILITÉ", "14px 0 4px")
        elif "Produits" in p and "Coûts" not in p:
            _NAV_GROUP_CSS_MAP[i+1] = ("CATALOGUE", "14px 0 4px")
        elif "Contacts" in p:
            _NAV_GROUP_CSS_MAP[i+1] = ("RELATIONS", "14px 0 4px")
    ...
    # CSS navigation sidebar — adaptatif selon le rôle
    _sep_css = """
...
"""
    for _nth, (_lbl, _pad) in _NAV_GROUP_CSS_MAP.items():
        _sep_css += f"""
    ...
    with tab_cl:
        ...
        if df_cmd.empty or (not df_cmd.empty and dfc.empty):
            ...
            for _, row in dfc.iterrows():
                ...
                # Détecter les infos client manquantes
                _missing_info = []
                if not str(row.get("prenom","") or "").strip(): _missing_info.append("prénom")
                if not str(row.get("nom","") or "").strip():    _missing_info.append("nom")
                if not str(row.get("mail","") or "").strip():   _missing_info.append("email")
                if not str(row.get("adresse","") or "").strip():_missing_info.append("adresse")
                _incomplete_badge = " 📝" if _missing_info else ""
                ...
                title_str = (f"{'🟢' if pct==100 else '🟡' if pct>=60 else '🔴'} "
                             ...
                             f"{etat_val} [{type_badge}]{_incomplete_badge}")
        ...
        with tab_ca:
            ...
            with n1:
                ...
                try:
                    ...
                    _conn_nc2.close()
                num_cmd = _auto_num
                st.info(f"N° commande : **{num_cmd}**")
            with n2:
                num_fac  = st.text_input("N° Facture")
                num_excl = st.text_input("N° Exclusif")
                plateforme = st.selectbox("Plateforme", ["Hostinger","Instagram DM","Pop-up","Email","Showroom","Wholesale","Autre"])
            with n3:
                etat_n   = st.selectbox("État initial", ETATS_CMD)

            # ── Infos client ──────────────────────────────────────────────────
            st.markdown('<div class="section-title" style="margin-top:10px;">Client</div>', unsafe_allow_html=True)
            ...
            notes_ = st.text_area("Notes", height=50)

            # ── Articles de la commande (multi) ───────────────────────────────
            st.markdown('<div class="section-title" style="margin-top:10px;">Articles commandés</div>', unsafe_allow_html=True)

            # Charger produits pour sélection
            _conn_prods = get_conn()
            try:
                _prods_cmd = _conn_prods.execute(
                    "SELECT ref, nom, couleurs, collection FROM products ORDER BY collection, nom"
                ).fetchall()
                _prod_map_cmd = {f"{p[1]}{' — '+p[2] if p[2] else ''} ({p[0]})": {"ref":p[0],"nom":p[1],"coll":p[3]}
                                 for p in _prods_cmd}
                _prod_opts_cmd = ["— Saisie libre —"] + list(_prod_map_cmd.keys())
            except Exception:
                _prod_map_cmd = {}; _prod_opts_cmd = ["— Saisie libre —"]
            finally:
                _conn_prods.close()

            # Nombre d'articles (max 10)
            n_articles = st.number_input("Nombre d'articles différents", min_value=1, max_value=10, value=1, step=1, key="cmd_n_art")

            articles_cmd = []
            total_ht_cmd = 0.0
            for _ai in range(int(n_articles)):
                st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.12em;margin:8px 0 2px;">Article {_ai+1}</div>', unsafe_allow_html=True)
                _ac1, _ac2, _ac3, _ac4 = st.columns([3,1,2,1])
                with _ac1:
                    _sel = st.selectbox("Produit", _prod_opts_cmd, key=f"cmd_art_sel_{_ai}")
                with _ac2:
                    _qte = st.number_input("Qté", min_value=1, value=1, step=1, key=f"cmd_art_qte_{_ai}")
                with _ac3:
                    if _sel == "— Saisie libre —":
                        _ref = st.text_input("SKU libre", placeholder="EWSJACKET-001A", key=f"cmd_art_ref_{_ai}")
                        _nom_art = st.text_input("Nom article", key=f"cmd_art_nom_{_ai}")
                    else:
                        _info = _prod_map_cmd.get(_sel, {})
                        _ref = _info.get("ref","")
                        _nom_art = _info.get("nom","")
                        st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:10px;color:#395f30;padding:6px 0;">✓ SKU : {_ref}</div>', unsafe_allow_html=True)
                with _ac4:
                    _pu = st.number_input("PU HT (€)", min_value=0.0, value=0.0, step=10.0, key=f"cmd_art_pu_{_ai}")
                articles_cmd.append({"ref":_ref, "nom":_nom_art, "qte":_qte, "pu":_pu})
                total_ht_cmd += _pu * _qte

            total_ttc_cmd = round(total_ht_cmd * 1.20, 2)
            if total_ht_cmd > 0:
                st.info(f"Total HT : {fmt_eur(total_ht_cmd)} · Total TTC estimé : {fmt_eur(total_ttc_cmd)}")

            if st.button("✓ Enregistrer la commande", type="primary", key="btn_save_cmd"):
                type_cmd_key = "B2B" if "B2B" in _cmd_type_sel else "B2C"
                conn_s = get_conn()
                # Créer une commande par article
                _arts_with_data = [a for a in articles_cmd if a["ref"] or a["nom"]]
                for _art in _arts_with_data:
                    _art_ttc = round(_art["pu"] * _art["qte"] * 1.20, 2)
                    conn_s.execute("""INSERT INTO commandes
                        ...
                        (priorite, str(date_cmd), num_cmd, num_fac, num_excl,
                         _art["ref"], _art["qte"], _art["pu"], 20.0,
                         _art_ttc, _art_ttc, plateforme,
                         prenom_, nom_, mail_, tel_, adr_,
                         etat_n, notes_, type_cmd_key))
                    if PRODUCTS_MODULE and _art["ref"]:
                        try:
                            sync_from_commande(conn_s, _art["ref"], _art["qte"],
                                nom_, prenom_, mail_, tel_, adr_,
                                _art_ttc, plateforme, num_cmd or "CMD")
                        except Exception: pass
                conn_s.commit(); conn_s.close()
                n_saved = len(_arts_with_data)
                st.success(f"✓ Commande enregistrée — {n_saved} article(s).")
                st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Sync statuts catalogue au chargement
    try:
        if PRODUCTS_MODULE:
            _sync_conn = get_conn()
            sync_product_status(_sync_conn)
            _sync_conn.commit(); _sync_conn.close()
    except Exception:
        pass
    ...
    with tab_si:
        ...
        if df_stk_pf.empty:
            ...
            df_stk_pf["cout_u_display"] = df_stk_pf["prix_unitaire"]  # alias pour affichage
            ...
            with sf1: f_type_stk = st.selectbox("Type", ["Tous"]+[t for t in _TYPES_STOCK if t!="Sample"])
...
<div style="font-family:'DM Mono',monospace;font-size:9px;color:{tc};
  ...
  <span>{fmt_eur(val_type)}</span>
                ...
                for _, item in sub.iterrows():
                    ...
                    _inv_c1, _inv_c2 = st.columns([5, 1])
                    with _inv_c1:
                        st.markdown(f"""
<div style="background:#fff;border:0.5px solid #ede3d3;border-left:3px solid {bar_c};
     padding:8px 14px;margin:4px 0;display:flex;justify-content:space-between;align-items:center;">
  <div>
  ...
  <div style="text-align:right;white-space:nowrap;margin-left:12px;">
    ...
    <div style="font-size:10px;color:#8a7968;">{fmt_eur(item.get('prix_unitaire',0))}/u · <strong>Total : {fmt_eur(float(item.get('qte_stock',0) or 0) * float(item.get('prix_unitaire',0) or 0))}</strong></div>
  </div>
</div>""", unsafe_allow_html=True)
                    with _inv_c2:
                        if can("stock_write") and st.button("✏️", key=f"stk_edit_{item['id']}", help="Modifier"):
                            st.session_state[f"stk_edit_open_{item['id']}"] = not st.session_state.get(f"stk_edit_open_{item['id']}", False)

                    if st.session_state.get(f"stk_edit_open_{item['id']}"):
                        st.warning("⚠️ Toute modification manuelle doit correspondre à une opération enregistrée. Veuillez d'abord créer l'opération correspondante dans la page Opérations, puis revenir modifier ici.")
                        with st.form(f"stk_edit_form_{item['id']}"):
                            new_qte = st.number_input("Nouvelle quantité en stock", value=float(item.get("qte_stock",0) or 0), min_value=0.0)
                            new_loc = st.selectbox("Localisation", LOCALISATIONS, index=LOCALISATIONS.index(item.get("localisation","Paris — Atelier")) if item.get("localisation") in LOCALISATIONS else 0)
                            new_prix = st.number_input("Coût unitaire (€)", value=float(item.get("prix_unitaire",0) or 0), min_value=0.0, step=0.01)
                            _val_tot_stk = float(new_qte or 0) * float(new_prix or 0)
                            st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:10px;color:#395f30;">Valeur totale : {fmt_eur(_val_tot_stk)}</div>', unsafe_allow_html=True)
                            if st.form_submit_button("✓ Confirmer (j'ai créé l'opération)", type="primary"):
                                _c_stk = get_conn()
                                _c_stk.execute("UPDATE stock SET qte_stock=?,localisation=?,prix_unitaire=? WHERE id=?",
                                               (new_qte, new_loc, new_prix, item["id"]))
                                _c_stk.commit()
                                # Sync statut catalogue
                                try:
                                    if PRODUCTS_MODULE:
                                        sync_product_status(_c_stk, str(item.get("ref","") or ""))
                                except Exception: pass
                                _c_stk.close()
                                st.session_state.pop(f"stk_edit_open_{item['id']}", None)
  ...
  <div style="text-align:right;">
    ...
    with tab_sa:
            ...
            with c1:
                ...
                type_s  = st.selectbox("Type", _TYPES_STOCK)
            with c2:
                loc_s      = st.selectbox("Localisation", LOCALISATIONS)
                qte_s      = st.number_input("Quantité en stock", min_value=0.0, value=0.0)
                prix_s     = st.number_input("Coût unitaire (€)", min_value=0.0, value=0.0, step=0.01)
            with c3:
                reassort_s = st.checkbox("Besoin réassort")
                notes_s    = st.text_area("Notes", height=90)

            # Créer une nouvelle MP ou composant avec SKU auto
            with st.expander("➕ Créer nouvelle Matière Première ou Composant"):
                new_type_mp = st.selectbox("Type", ["Matière première","Composant"], key="new_mp_type")
                new_desc_mp = st.text_input("Description", placeholder="ex: Tissu Whipcord Noir", key="new_mp_desc")
                new_qte_mp  = st.number_input("Quantité initiale", min_value=0.0, value=0.0, key="new_mp_qte")
                new_loc_mp  = st.selectbox("Localisation", LOCALISATIONS, key="new_mp_loc")
                if st.button("✓ Créer", key="btn_create_mp"):
                    if new_desc_mp:
                        # SKU auto : CAT-DESC-XXX
                        prefix = "MAT" if new_type_mp == "Matière première" else "CMP"
                        last = get_conn().execute(f"SELECT MAX(ref) FROM stock WHERE ref LIKE '{prefix}-%'").fetchone()
                        n = 1
                        if last and last[0]:
                            try: n = int(last[0].split("-")[-1]) + 1
                            except: pass
                        auto_sku = f"{prefix}-{new_desc_mp[:3].upper().strip()}-{n:03d}"
                        _c_mp = get_conn()
                        _c_mp.execute("""INSERT OR IGNORE INTO stock
                            (description,ref,type_produit,localisation,qte_stock)
                            VALUES (?,?,?,?,?)""",
                            (new_desc_mp, auto_sku, new_type_mp, new_loc_mp, new_qte_mp))
                        _c_mp.commit(); _c_mp.close()
                        st.success(f"✓ Créé : {auto_sku}"); st.rerun()

            # Créer une nouvelle localisation
            with st.expander("📍 Gérer les localisations"):
                _locs_current = get_localisations()
                st.markdown("**Localisations existantes :** " + " · ".join(_locs_current))
                new_loc_name = st.text_input("Nouvelle localisation", placeholder="ex: Chez Alexis — Lyon, Showroom Paris 11e, Atelier Margiela...", key="new_loc_name_inp")
                if st.button("➕ Ajouter localisation", key=f"btn_add_loc_{id(new_loc_name)}"):
                    if new_loc_name.strip():
                        _c_loc = get_conn()
                        add_localisation(_c_loc, new_loc_name.strip())
                        _c_loc.close()
                        st.success(f"✓ '{new_loc_name}' ajoutée.")
                        st.rerun()
                # Supprimer
                del_loc = st.selectbox("Supprimer une localisation", ["—"] + _locs_current, key="del_loc_sel")
                if del_loc != "—" and st.button("🗑 Supprimer", key="btn_del_loc"):
                    _c_loc2 = get_conn()
                    _c_loc2.execute("DELETE FROM localisations WHERE nom=?", (del_loc,))
                    _c_loc2.commit(); _c_loc2.close()
                    st.success(f"✓ '{del_loc}' supprimée."); st.rerun()

            # Gérer les localisations personnalisées
            with st.expander("⚙ Gérer les localisations"):
                try:
                    _loc_conn = get_conn()
                    _cur_locs = [r[0] for r in _loc_conn.execute("SELECT nom FROM localisations ORDER BY nom").fetchall()]
                    _loc_conn.close()
                except Exception:
                    _cur_locs = []
                st.markdown("**Localisations existantes :**")
                for _l in _cur_locs:
                    lc1, lc2 = st.columns([4, 1])
                    with lc1:
                        st.markdown(f"· {_l}")
                    with lc2:
                        if st.button("🗑", key=f"del_loc_{_l}", help="Supprimer"):
                            try:
                                _dc = get_conn(); _dc.execute("DELETE FROM localisations WHERE nom=?", (_l,)); _dc.commit(); _dc.close()
                            except Exception: pass
                            st.rerun()
                st.markdown("**Créer une nouvelle localisation :**")
                new_loc_name = st.text_input("Nom", placeholder="ex: Chez fournisseur Dupont, Tokyo — Showroom...", key="new_loc_input")
                if st.button("➕ Ajouter localisation", key="btn_add_loc_2"):
                    if new_loc_name.strip():
                        try:
                            _ac = get_conn()
                            _ac.execute("INSERT OR IGNORE INTO localisations (nom, created_by) VALUES (?,?)",
                                        (new_loc_name.strip(), st.session_state.get("user_display","Jules")))
                            _ac.commit(); _ac.close()
                        except Exception: pass
                        st.success(f"✓ Localisation '{new_loc_name}' créée.")
                        st.rerun()
...
<div style="background:#fff;border:0.5px solid #ede3d3;padding:10px 14px;margin:5px 0;
  ...
  <div style="text-align:right;">
    ...
    <div style="font-size:10px;color:#8a7968;">unités · Coût unit. {fmt_eur(item['prix_unitaire'])} · Total : {fmt_eur(float(item.get('qte_stock',0) or 0)*float(item.get('prix_unitaire',0) or 0))}</div>
  </div>
</div>""", unsafe_allow_html=True)
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── KPI ────────────────────────────────────────────────────────────────────
    with tab_kpi:
        # ── KPIs Finance avec typographie améliorée ───────────────────────────
        res_color = "#2d6a4f" if res_net >= 0 else "#c1440e"
        marge_color = "#2d6a4f" if marge >= 0 else "#c1440e"

        def _kpi_card(label, value, color="#1a1a1a", sub=""):
            sub_html = f'<div style="font-family:DM Mono,monospace;font-size:9px;color:{color};margin-top:4px;">{sub}</div>' if sub else ""
            return (
                '<div style="background:#fff;border:0.5px solid #e8e4de;padding:16px 18px;">' +
                f'<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.15em;margin-bottom:8px;">{label}</div>' +
                f'<div style="font-family:DM Sans,sans-serif;font-size:26px;font-weight:500;color:{color};letter-spacing:-.02em;line-height:1;">{value}</div>' +
                sub_html +
                '</div>'
            )
        ...
        kpi_c = st.columns(5)
        cards = [
            ("CA HT",          fmt_eur(ventes_cr),           "#1a1a1a",  ""),
            ("Charges totales", fmt_eur(total_charges),       "#c9800a",  ""),
            ("Résultat net",    fmt_eur(res_net),             res_color,  "Bénéfice" if res_net>=0 else "Perte"),
            ("Marge nette",    f"{marge:.1f}%",               marge_color,""),
            ("TVA nette",      fmt_eur(tva_col - tva_ded),   "#8a7968",  ""),
        ]
        for i, (lbl, val, col, sub) in enumerate(cards):
            with kpi_c[i]:
                st.markdown(_kpi_card(lbl, val, col, sub), unsafe_allow_html=True)
        ...
        for col_idx, (grp, cats) in enumerate(charge_groups.items()):
            ...
            with cg_cols[col_idx]:
                st.markdown(_kpi_card(grp, fmt_eur(grp_total), "#8a7968"), unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not can("tva_read"):
        ...
        st.stop()
    if TVA_MODULE:
        if not can("tva_write"):
            # Corentin/Alexis : synthèse annuelle seulement
            st.info("👁 Vue lecture seule — Synthèse annuelle TVA")
            page_tva(can, DB_PATH, sel_year)
        else:
            page_tva(can, DB_PATH, sel_year)
    else:
        st.error("Module TVA non chargé.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if PRODUCTS_MODULE:
        if not can("couts_write"):
            st.info("👁 Vue lecture seule — Coûts de production")
