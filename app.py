...
def load_stock():
    try:
        ...
        # Migration silencieuse des colonnes manquantes
        try:
            _sc = [r[1] for r in conn.execute("PRAGMA table_info(stock)").fetchall()]
            for _col, _def in [("sous_type","TEXT DEFAULT ''"),("unite","TEXT DEFAULT 'Pièces'"),
                                ("made_in","TEXT DEFAULT ''")]:
                if _col not in _sc:
                    conn.execute(f"ALTER TABLE stock ADD COLUMN {_col} {_def}")
            conn.commit()
        except Exception: pass
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        pass
    _ts = ["📋 Inventaire", "🏭 Stock Produits Finis", "🔬 Stock Samples", "🧵 Stock MP & Composants", "📦 Stock Packaging", "➕ Ajouter article"]
    _os = st.tabs(_ts)
    tab_si   = _os[0]
    tab_spf  = _os[1]
    tab_smp  = _os[2]
    ...
    with tab_si:
        df_stk = load_stock()
        # Inventaire complet — tout afficher même à qte=0
        df_stk_pf = df_stk[df_stk["type_produit"] != "Sample"] if not df_stk.empty else df_stk

        # Filtre optionnel : masquer les qte=0
        _hide_zero = st.checkbox("Masquer les articles à stock 0", value=False, key="stk_hide_zero")
        if _hide_zero and not df_stk_pf.empty:
            df_stk_pf = df_stk_pf[df_stk_pf["qte_stock"] > 0]
...
<div style="background:#fff;border:0.5px solid #ede3d3;border-left:3px solid {bar_c};
  ...
  <div style="text-align:right;white-space:nowrap;margin-left:12px;">
    ...
    with tab_spf:
        ...
        df_pf_only = df_stk_pf2[
            (df_stk_pf2["type_produit"]=="Produit fini") &
            (df_stk_pf2["qte_stock"] > 0)
        ] if not df_stk_pf2.empty else pd.DataFrame()
        st.markdown('<div class="section-title">Stock Produits Finis</div>', unsafe_allow_html=True)
        if df_pf_only.empty:
            st.info("Aucun produit fini en stock (quantité > 0).")
        else:
            _cols_pf = [c for c in ["ref","description","qte_stock","unite","prix_unitaire","sous_type","made_in","notes"]
                        if c in df_pf_only.columns]
            _ren_pf  = {"ref":"Réf.","description":"Article","qte_stock":"Qté","unite":"Unité",
                        "prix_unitaire":"Coût","sous_type":"Sous-type","made_in":"Made in","notes":"Notes"}
            st.dataframe(df_pf_only[_cols_pf].rename(columns=_ren_pf),
...
<div style="background:#fff;border:0.5px solid #ede3d3;border-left:3px solid #7b506f;
  ...
  <div style="text-align:right;">
    ...
    # ── STOCK MP & COMPOSANTS ────────────────────────────────────────────────────
    with tab_mp:
        df_stk_mp2 = load_stock()
        df_mp_only = df_stk_mp2[
            (df_stk_mp2["type_produit"].isin(["Matière première","Composant"])) &
            (df_stk_mp2["qte_stock"] > 0)
        ] if not df_stk_mp2.empty else pd.DataFrame()
        st.markdown('<div class="section-title">Stock MP & Composants</div>', unsafe_allow_html=True)
        if df_mp_only.empty:
            st.info("Aucun MP ou composant en stock (quantité > 0).")
        else:
            _cols_mp = [c for c in ["ref","description","type_produit","sous_type","qte_stock","unite","prix_unitaire","made_in","notes"]
                        if c in df_mp_only.columns]
            _ren_mp  = {"ref":"Réf.","description":"Article","type_produit":"Type","sous_type":"Sous-type",
                        "qte_stock":"Qté","unite":"Unité","prix_unitaire":"Coût unitaire","made_in":"Made in","notes":"Notes"}
            _val_tot = (df_mp_only["qte_stock"] * df_mp_only["prix_unitaire"]).sum()
            st.caption(f"Valeur totale stock MP/Composants : {_val_tot:,.2f} €")
            st.dataframe(df_mp_only[_cols_mp].rename(columns=_ren_mp),
                         use_container_width=True, hide_index=True)

    with tab_pkg:
        df_stk_pkg2 = load_stock()
        df_pkg_only = df_stk_pkg2[
            df_stk_pkg2["type_produit"] == "Packaging"
        ] if not df_stk_pkg2.empty else pd.DataFrame()
        st.markdown('<div class="section-title">Stock Packaging</div>', unsafe_allow_html=True)
        if df_pkg_only.empty:
            st.info("Aucun article packaging en stock.")
        else:
            _cols_pkg = [c for c in ["ref","description","sous_type","qte_stock","unite","prix_unitaire","notes"]
                         if c in df_pkg_only.columns]
            _ren_pkg  = {"ref":"Réf.","description":"Article","sous_type":"Sous-type",
                         "qte_stock":"Qté","unite":"Unité","prix_unitaire":"Coût","notes":"Notes"}
            _val_pkg = (df_pkg_only["qte_stock"] * df_pkg_only["prix_unitaire"]).sum()
            st.caption(f"Valeur totale packaging : {_val_pkg:,.2f} €")
            st.dataframe(df_pkg_only[_cols_pkg].rename(columns=_ren_pkg),
                         use_container_width=True, hide_index=True)
    ...
    with tab_sa:
            ...
            with c1:
                ...
                _sous_cat_opts = {
                    "Matière première": ["Tissu lourd","Tissu léger","Tissus imprimé / motif","Maille","Cuir","Autre"],
                    "Composant":        ["Zip","Bouton","Étiquette","Broderie","Patch","Autre"],
                    "Packaging":        ["Packaging","Emballage","Autre"],
                    "Produit fini":     ["Chapter I","Chapter II","Archive","Sample"],
                    "Sample":           ["Chapter I","Chapter II","Autre"],
                }.get(type_s, ["Autre"])
                sous_cat_s = st.selectbox("Sous-catégorie", _sous_cat_opts)
            with c2:
                ...
                if not ref_s or not desc_s:
                    ...
                    try:
                        conn.execute("""INSERT INTO stock
                            (description,ref,type_produit,sous_type,localisation,qte_stock,prix_unitaire,besoin_reassort,notes)
                            VALUES (?,?,?,?,?,?,?,?,?)""",
                            (desc_s,ref_s,type_s,sous_cat_s,loc_s,qte_s,prix_s,int(reassort_s),notes_s))
                        conn.commit()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        from products_module import page_packaging, init_products_db
        conn_pkg = get_conn()
        init_products_db(conn_pkg)
        page_packaging(conn_pkg, can, fmt_eur)
        conn_pkg.close()
    except Exception as _e:
        st.error(f"Erreur page packaging : {_e}")
