# calendar_module.py — v2.8 — 1776503773
...
def init_calendar_db(conn):
    c = conn.cursor()
    # Créer la table events avec toutes les colonnes d'un coup
    try:
        c.execute("""CREATE TABLE IF NOT EXISTS events (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            titre TEXT NOT NULL,
            type_event TEXT DEFAULT 'Autre',
    ...
    # Migrations silencieuses : ajouter colonnes manquantes
    _ev_cols_needed = [
        ("heure_fin",       "TEXT DEFAULT ''"),
        ("date_fin",        "TEXT DEFAULT ''"),
        ("meet_link",       "TEXT DEFAULT ''"),
    ...
    try:
        existing_ev = [r[1] for r in c.execute("PRAGMA table_info(events)").fetchall()]
        for col, defn in _ev_cols_needed:
            if col not in existing_ev:
                try: c.execute(f"ALTER TABLE events ADD COLUMN {col} {defn}")
    ...
    # Reset events si jamais demandé (marqueur)
    _reset_done_key = "cal_reset_v3"  # Incrémenter pour forcer un nouveau reset
    try:
        _rm = c.execute("SELECT value FROM app_settings WHERE key=?", (_reset_done_key,)).fetchone()
        if not _rm:
            try:
...
<div style="text-align:center;font-family:'DM Mono',monospace;font-size:11px;color:#8a7968;padding:6px 0;">
  Semaine du {lundi_cal.strftime('%d %B')} au {vendredi_cal.strftime('%d %B %Y')}
        ...
        if st.session_state.get("cal_show_edit") and not df_ev_cal.empty:
            ...
            if sel_ev_label != "— Sélectionner —":
                ...
                with st.form(f"ev_edit_main_{eid}"):
                    ...
                    # Option récurrence si l'event est récurrent
                    _is_recurrent = bool(sel_ev.get("recurrent", 0))
                    if _is_recurrent:
                        _recur_scope = st.radio(
                            "Appliquer à", ["Cet événement uniquement", "Cet événement et tous les suivants"],
                            key="recur_scope", horizontal=True
                        )
                    else:
                        _recur_scope = "Cet événement uniquement"

                    save_col, del_col = st.columns(2)
                    with save_col:
                        if st.form_submit_button("💾 Sauvegarder", type="primary"):
                            if _is_recurrent and _recur_scope == "Cet événement et tous les suivants":
                                # Modifier tous les events de même titre à partir de cette date
                                _base_titre = str(sel_ev.get("titre","")).rsplit("—",1)[0].strip()
                                conn.execute("""UPDATE events
                                    SET titre=?,heure=?,heure_fin=?,lieu=?,description=?,type_event=?,meet_link=?
                                    WHERE (titre LIKE ? OR id=?) AND date_debut >= ?""",
                                    (_et,_eh,_eh_fin,_el,_ed,_etyp,_em,
                                     f"{_base_titre}%", eid, str(sel_ev.get("date_debut",""))))
                            else:
                                conn.execute("""UPDATE events
                    ...
                    with del_col:
                        if st.form_submit_button("🗑 Supprimer"):
                            if _is_recurrent and _recur_scope == "Cet événement et tous les suivants":
                                _base_titre2 = str(sel_ev.get("titre","")).rsplit("—",1)[0].strip()
                                conn.execute("DELETE FROM events WHERE (titre LIKE ? OR id=?) AND date_debut >= ?",
                                             (f"{_base_titre2}%", eid, str(sel_ev.get("date_debut",""))))
                            else:
        ...
        # heure_fin migrée dans init_calendar_db
        ...
        with st.form("add_event"):
            ...
            if st.form_submit_button("✓ Créer l'événement"):
                if not ev_titre:
                    ...
                    # Création dans Google Calendar via API Anthropic + MCP
                    if ev_invite:
                        try:
                            import requests as _req, json as _json
                            from datetime import datetime as _dti2
                            _hh, _mm = (str(ev_heure or "10:00").split(":") + ["00"])[:2]
                            _dt_start = _dti2(ev_date.year, ev_date.month, ev_date.day, int(_hh), int(_mm))
                            _hf = str(ev_heure_fin or "") if ev_heure_fin and ":" in str(ev_heure_fin) else ""
                            if _hf:
                            ...
                            _iso_s = _dt_start.strftime("%Y-%m-%dT%H:%M:%S")
                            _iso_e = _dt_end.strftime("%Y-%m-%dT%H:%M:%S")

                            # Appel API Anthropic avec MCP Google Calendar
                            _gcal_payload = {
                                "model": "claude-sonnet-4-20250514",
                                "max_tokens": 512,
                                "messages": [{"role":"user","content":
                                    f"Create a Google Calendar event titled '{ev_titre}' "
                                    f"from {_iso_s} to {_iso_e} Paris time, "
                                    f"location '{ev_lieu or 'Online'}', "
                                    f"description '{ev_desc or ''}', "
                                    f"add attendee {EMAIL_INVITATION}, "
                                    f"{'add Google Meet link, ' if ev_type=='Réunion' else ''}"
                                    f"send notifications to all attendees."
                                }],
                            ...
                            if _api_key:
                                _gcal_resp = _req.post(
                                    "https://api.anthropic.com/v1/messages",
                                    headers={**_gcal_headers, "x-api-key": _api_key},
                                    json=_gcal_payload, timeout=30
                                )
                                if _gcal_resp.status_code == 200:
                                    st.success(f"✅ Événement créé dans Google Calendar — invitation envoyée à {EMAIL_INVITATION}")
                                    if ev_type == "Réunion":
                                        # Extraire le lien Meet si possible
                                        _gcal_data = _gcal_resp.json()
                                        _full_text = " ".join(
                                            b.get("text","") for b in _gcal_data.get("content",[])
                                            if b.get("type")=="text"
                                        )
                                        if "meet.google.com" in _full_text:
                                            import re as _re2
                                            _meet_match = _re2.search(r'https://meet\.google\.com/[a-z-]+', _full_text)
                                            if _meet_match:
                                                _meet_link = _meet_match.group(0)
                                                st.info(f"🎥 Lien Meet : {_meet_link}")
                                                # Sauvegarder le lien Meet dans la DB
                                                conn.execute("UPDATE events SET meet_link=? WHERE titre=? AND date_debut=?",
                                                             (_meet_link, ev_titre, ev_date.isoformat()))
                                                conn.commit()
                                else:
                                    st.warning(f"Google Calendar API : {_gcal_resp.status_code}. Vérifiez le token OAuth.")
                            else:
                                st.warning("ANTHROPIC_API_KEY manquante dans les secrets Streamlit.")
                        except Exception as _ge:
                            st.warning(f"Erreur Google Calendar : {_ge}")
                    st.rerun()
