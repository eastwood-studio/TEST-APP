# calendar_module.py — v2.2 — 1776109443 — 1775671695 — 1775665297
# ══════════════════════════════════════════════════════════════════════════════
  ...
  <div style="font-size:14px;font-weight:500;color:{tc};">{jour_d.day}</div>
        ...
        for heure_str in heures:
            ...
            for j in range(7):
                ...
                with row_cols[j+1]:
                    cell_bg = "#f9f8f6" if (j >= 5) else "#fff"
                    border_t = "1px solid #f0ece4"
                    if ev_here:
                        for ev in ev_here:
                            col_ev = EVENT_COLORS.get(ev.get("type_event","Autre"), "#888")
                            ev_key = f"ev_{ev['id']}_{jour_d}_{heure_str}"
                            # Expander cliquable = clic sur l'événement pour modifier
                            with st.expander(f"📌 {str(ev.get('titre',''))[:20]}", expanded=False):
                                st.markdown(f'<span style="font-family:DM Mono,monospace;font-size:9px;color:{col_ev};">{ev.get("type_event","")}</span>', unsafe_allow_html=True)
                                with st.form(f"ev_edit_{ev['id']}_{heure_str}"):
                                    _et = st.text_input("Titre", value=ev.get("titre",""), key=f"et_{ev['id']}_{heure_str}")
                                    _eh = st.text_input("Heure", value=ev.get("heure",""), key=f"eh_{ev['id']}_{heure_str}")
                                    _el = st.text_input("Lieu", value=ev.get("lieu","") or "", key=f"el_{ev['id']}_{heure_str}")
                                    _ed = st.text_area("Description", value=ev.get("description","") or "", height=50, key=f"ed_{ev['id']}_{heure_str}")
                                    _etyp = st.selectbox("Type", TYPES_EVENT,
                                        index=TYPES_EVENT.index(ev.get("type_event","Autre")) if ev.get("type_event") in TYPES_EVENT else 0,
                                        key=f"etyp_{ev['id']}_{heure_str}")
                                    f1, f2 = st.columns(2)
                                    with f1:
                                        if st.form_submit_button("💾 Sauvegarder"):
                                            conn.execute("UPDATE events SET titre=?,heure=?,lieu=?,description=?,type_event=? WHERE id=?",
                                                         (_et, _eh, _el, _ed, _etyp, ev["id"]))
                        ...
                        st.markdown(f'<div style="height:40px;background:{cell_bg};border-bottom:{border_t};"></div>', unsafe_allow_html=True)
