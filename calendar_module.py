# calendar_module.py — v2.1 — 1775671695 — 1775665297
# ══════════════════════════════════════════════════════════════════════════════
...
def page_calendrier(can_fn, DB_PATH):
    ...
    tab_cal, tab_obj, tab_add, tab_todo_cal, tab_ics = st.tabs(["📅 Calendrier", "🎯 Plan Stratégie", "➕ Ajouter", "✅ Mes TODO", "📥 Import ICS"])
...
<div style="margin-top:12px;">
  <a href="#" onclick="return false;" style="font-family:'DM Mono',monospace;font-size:10px;
    ...
    with tab_ics:
        ...
        st.markdown("""
<div class="info-box">
Importez un fichier .ics (Google Calendar, Apple Calendar, Outlook).
Chaque événement sera ajouté au calendrier Eastwood.
</div>""", unsafe_allow_html=True)
        ics_file = st.file_uploader("Fichier .ics", type=["ics"], key="ics_upload")
        if ics_file:
            try:
                ics_content = ics_file.read().decode("utf-8", errors="replace")
                events_parsed = []
                current = {}
                for line in ics_content.split("\n"):
                    ...
                    if line == "BEGIN:VEVENT":
                        ...
                        if current.get("SUMMARY"):
                            events_parsed.append(current)
                        current = {}
                    elif line.startswith("DTSTART"):
                        val = line.split(":",1)[-1].strip()[:8]
                        try:
                            from datetime import datetime as _dti
                            d = _dti.strptime(val, "%Y%m%d")
                            current["DATE"] = d.strftime("%Y-%m-%d")
                        except Exception:
                            current["DATE"] = ""
                        ...
                        try:
                            ...
                            d = _dti2.strptime(val, "%Y%m%d")
                            current["DATE"] = d.strftime("%Y-%m-%d")
                        except Exception:
                            pass

                if events_parsed:
                    st.markdown(f"**{len(events_parsed)} événements détectés**")
                    for ep in events_parsed[:5]:
                        st.markdown(f"· {ep.get('DATE','?')} — {ep.get('SUMMARY','')}")
                    if len(events_parsed) > 5:
                        st.markdown(f"*... et {len(events_parsed)-5} autres*")
                    if st.button(f"✓ Importer {len(events_parsed)} événements", type="primary"):
                        for ep in events_parsed:
                            conn.execute("""INSERT INTO events
                                (titre,type_event,date_debut,description,created_by)
                                VALUES (?,?,?,?,?)""",
                                (ep.get("SUMMARY","")[:100], "Autre",
                                 ep.get("DATE", ""),
                                 ep.get("DESCRIPTION",""), "import_ics"))
                        conn.commit()
                        st.success(f"✓ {len(events_parsed)} événements importés.")
                        st.rerun()
                else:
                    st.warning("Aucun événement trouvé dans ce fichier.")
            except Exception as e:
                st.error(f"Erreur lecture ICS : {e}")
