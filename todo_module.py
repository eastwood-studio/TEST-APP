# todo_module.py — v2.7 — 1776172327
# ══════════════════════════════════════════════════════════════════════════════
# MODULE TODO ACCUEIL
    ...
    with tab_team:
        ...
        if df_team_all.empty:
            ...
            for _, todo in df_team_all.iterrows():
                ...
                with c_card:
                    _bg_card = '#fafaf8' if todo.get('fait') else '#fff'
                    _clr     = '#aaa'    if todo.get('fait') else '#1a1a1a'
                    _desc_html = f'<div style="font-size:11px;color:#8a7968;margin-top:2px;">{todo["description"]}</div>' if todo.get("description") else ''
                    _sub_badge = f'<span style="font-family:DM Mono,monospace;font-size:9px;color:#395f30;">{sub_done}/{sub_total} ✓</span>' if sub_total > 0 else ''

                    # Calculer la date cible selon la priorité et la date de création
                    try:
                        from datetime import datetime as _dtt
                        _created_at = str(todo.get("created_at","") or "")
                        _lundi_t = lundi  # Lundi de la semaine de la tâche
                        _prio_t = todo.get("priorite","")
                        if _prio_t == "Urgent":
                            _due = _lundi_t + timedelta(days=4)  # Vendredi
                        elif _prio_t == "Cette semaine":
                            _due = _lundi_t + timedelta(days=4)
                        elif _prio_t == "Deux semaines":
                            _due = _lundi_t + timedelta(weeks=2, days=4)
                        elif _prio_t == "Ce mois":
                            _due = _lundi_t + timedelta(weeks=4)
                        elif _prio_t == "Quand possible":
                            _due = None
                        else:
                            _due = None
                        _due_str = f'Pour le {_due.strftime("%d %b")}' if _due else ""
                        # Ajouter la date de création si disponible
                        _created_str = ""
                        if _created_at and len(_created_at) >= 10:
                            try:
                                _cd = _dtt.strptime(_created_at[:10], "%Y-%m-%d").date()
                                _created_str = f'Créé le {_cd.strftime("%d %b")}' + (" · " if _due_str else "")
                            except Exception:
                                pass
                        _date_badge = f'<span style="font-family:DM Mono,monospace;font-size:8px;color:#aaa49a;margin-left:6px;">{_created_str}{_due_str}</span>' if (_created_str or _due_str) else ""
                    except Exception:
                        _date_badge = ""

                    # Construire le HTML entièrement avant le markdown
                    _assignee_txt = str(todo.get("assignee","") or "")
                    _prio_txt     = str(todo.get("priorite","") or "")
                    _titre_txt    = str(todo["titre"]).replace("<","&lt;").replace(">","&gt;")
                    _team_html = (
                        f'<div style="border:0.5px solid #e0dbd2;border-left:3px solid {done_c};' +
                        f'padding:8px 12px;margin:3px 0;background:{_bg_card};">' +
                        f'<div style="font-size:13px;font-weight:500;{done_s}color:{_clr};">{_titre_txt}</div>' +
                        (_desc_html) +
                        f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{pc};margin-top:3px;">' +
                        f'<span>{_assignee_txt}</span>' +
                        f'<span style="color:#c8c3b8;margin:0 4px;">·</span>' +
                        f'<span>{_prio_txt}</span>' +
                        _sub_badge +
                        _date_badge +
                        '</div></div>'
                    )
...
<div style="display:flex;align-items:center;gap:8px;padding:3px 12px 3px 20px;">
  ...
  <div style="font-size:11px;text-decoration:{sub_s}">{sub['titre']}</div>
                ...
                with c_btns:
                    # Cocher/décocher (bloqué si sous-tâches pas finies)
                    if not todo.get("fait"):
                        _t_can = (sub_total == 0 or sub_done == sub_total)
                        if st.button("☐", key=f"tdone_{todo['id']}", help="Marquer fait",
                            ...
                            conn.commit(); st.rerun()
                        if not _t_can:
                            st.markdown(f'<div style="font-size:8px;color:#c9800a;">{sub_total-sub_done} restante(s)</div>', unsafe_allow_html=True)
