# todo_module.py — v2.2 — 1775681681 — 1775665335
# ══════════════════════════════════════════════════════════════════════════════
# MODULE TODO ACCUEIL
...
<div style="border-left:2px solid {'#ccc' if todo.get('fait') else pc};
  ...
  {f'<div style="font-size:11px;color:{EW_B};{done_style}">{todo["description"]}</div>' if todo.get("description") else ''}
    ...
    with tab_team:
        ...
        # ── Chart par individu (barres de progression) ──────────────────────
        ...
        try:
            ...
            if not df_indiv.empty:
                ...
                for m in membres_indiv:
                    ...
                    total_m = sub["nb"].sum()
                    done_m = sub[sub["fait"]==1]["nb"].sum() if not sub[sub["fait"]==1].empty else 0
                    if total_m > 0:
                        bar_data.append({"membre":m,"fait":int(done_m),"total":int(total_m)})
                if bar_data:
...
<div style="text-align:center;padding:8px;">
  ...
  <div style="height:6px;background:#ede3d3;margin:4px 0;">
    <div style="width:{pct_m}%;height:6px;background:{c_bar};"></div>
  </div>
  <div style="font-size:10px;color:#8a7968;">{bd["fait"]}/{bd["total"]} tâches</div>
</div>""", unsafe_allow_html=True)
        except Exception:
            pass

        # ── Chart suivi trimestriel ──────────────────────────────────────────
        st.markdown('<div class="section-title">Suivi trimestriel — 13 dernières semaines</div>', unsafe_allow_html=True)
        try:
            # Compter tâches faites vs total par semaine sur 13 semaines
            ...
            if not df_all_hist.empty:
                ...
                for s in sems:
                    ...
                    t = sub_s["nb"].sum()
                    d = sub_s[sub_s["fait"]==1]["nb"].sum() if not sub_s[sub_s["fait"]==1].empty else 0
                    total_q.append(int(t))
                    done_q.append(int(d))
...
new Chart(document.getElementById('chartTrimestre'), {{
  ...
  data: {{
    labels: {labels_q},
    datasets: [
      {{label:'Terminées',data:{done_q},backgroundColor:'#395f3099',borderColor:'#395f30',borderWidth:1}},
      {{label:'Total',data:{total_q},backgroundColor:'#7b506f33',borderColor:'#7b506f',borderWidth:1}}
    ]
  }},
  options: {{
    responsive:true,maintainAspectRatio:false,
    plugins:{{legend:{{position:'top',labels:{{font:{{family:'DM Mono',size:10}}}}}}}},
    scales:{{
      x:{{stacked:false,grid:{{color:'#ede3d311'}},ticks:{{font:{{family:'DM Mono',size:8}},maxRotation:45}}}},
      y:{{grid:{{color:'#ede3d322'}},ticks:{{font:{{family:'DM Mono',size:10}},stepSize:1}}}}
    }}
  }}
}});
</script>""", height=220)
