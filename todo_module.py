...
<div style="text-align:center;padding:8px 4px;background:#fff;border:0.5px solid #e0dbd2;">
  ...
  <div style="font-size:10px;color:#8a7968;">{d_cur}/{t_cur} tâches</div>
    ...
    if tab_gestion is not None:
        ...
        # ── Purge semaines précédentes (Jules only) ─────────────────────────
        try:
        ...
        if not _old_sems.empty:
            with st.expander(f"🗑 Supprimer tâches des semaines précédentes ({len(_old_sems)} semaine(s))"):
                _sel_sems = st.multiselect(
                    "Semaines à supprimer",
                    options=_old_sems["semaine"].tolist(),
                    default=[],
                    format_func=lambda s: f"Semaine {s}"
                )
                _del_col1, _del_col2 = st.columns([3,1])
                with _del_col1:
                    _del_done_only = st.checkbox("Supprimer uniquement les tâches terminées", value=True)
                with _del_col2:
