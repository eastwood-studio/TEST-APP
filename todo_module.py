# todo_module.py — v2.9 — 1777008085
# ══════════════════════════════════════════════════════════════════════════════
# MODULE TODO ACCUEIL
...
<div style="font-size:11px;color:{done_c};padding:2px 0;">
  {'☑' if todo.get('fait') else '☐'} {todo['titre']}
    ...
    tab_kpi     = tab_obs[5] if can_fn("finance_write") else None
                                ...
                                with sc2:
                                    if not sub.get("fait"):
                                        if can_fn("finance_write"):
                                            ...
                                            st.markdown('<span style="color:#d9c8ae;font-size:11px;">○</span>', unsafe_allow_html=True)
                                    else:
                                        st.markdown('<span style="color:#395f30;font-size:11px;">✓</span>', unsafe_allow_html=True)

                    with t_col2:
                        # Checkbox tâche principale — Jules only dans Objectifs équipe
                        if can_fn("finance_write"):
                            if not is_done:
                            ...
                            # Lecture seule pour Corentin/Alexis
                            if is_done:
                                st.markdown('<span style="color:#395f30;font-size:14px;">✓</span>', unsafe_allow_html=True)
                            else:
                                st.markdown('<span style="color:#d9c8ae;font-size:14px;">○</span>', unsafe_allow_html=True)
  ...
  <div style="font-size:10px;color:#8a7968;">{d_cur}/{t_cur} tâches</div>
    ...
    if tab_gestion is not None:
        with tab_gestion:
            ...
            # ── Purge semaines précédentes ──────────────────────────────────────
            try:
                _old_sems = pd.read_sql(
                    "SELECT DISTINCT semaine FROM todos WHERE semaine < ? ORDER BY semaine DESC",
                    conn, params=[semaine_id])
            except Exception:
                _old_sems = pd.DataFrame()
            if not _old_sems.empty:
                with st.expander(f"🗑 Supprimer tâches des semaines précédentes ({len(_old_sems)} semaine(s))"):
                    _sel_sems = st.multiselect("Semaines", options=_old_sems["semaine"].tolist(), default=[],
                                               format_func=lambda s: f"Semaine {s}")
                    _dc1,_dc2 = st.columns([3,1])
                    with _dc1: _del_done_only = st.checkbox("Uniquement les tâches terminées", value=True)
                    with _dc2:
                        if _sel_sems and st.button("🗑 Supprimer", type="secondary", key="del_old_sems"):
                            _n_del = 0
                            for _s in _sel_sems:
                                _q = "DELETE FROM todos WHERE semaine=?" + (" AND fait=1" if _del_done_only else "")
                                _ids_del = [r[0] for r in conn.execute(
                                    "SELECT id FROM todos WHERE semaine=?" + (" AND fait=1" if _del_done_only else ""),
                                    (_s,)).fetchall()]
                                for _tid in _ids_del:
                                    conn.execute("DELETE FROM todo_subtasks WHERE todo_id=?", (_tid,))
                                _n_del += conn.execute(_q, (_s,)).rowcount
                            conn.commit(); st.success(f"✓ {_n_del} tâche(s) supprimée(s)."); st.rerun()

            # ── Charger toutes les tâches de la semaine ─────────────────────────
            df_gestion = pd.read_sql(
                "SELECT * FROM todos WHERE semaine=? ORDER BY fait ASC, priorite ASC, assignee ASC",
                conn, params=[semaine_id])

            if df_gestion.empty:
                st.info("Aucune tâche cette semaine.")
            else:
                # ── Séparation URGENT / TODO / DONE ──────────────────────────────
                _df_urgent = df_gestion[(df_gestion["fait"]==0) & (df_gestion["priorite"]=="Urgent")]
                _df_todo   = df_gestion[(df_gestion["fait"]==0) & (df_gestion["priorite"]!="Urgent")]
                _df_done   = df_gestion[df_gestion["fait"]==1]

                def _render_task_card(todo_g, conn, editable=True):
                    """Affiche une tâche avec boutons modifier/supprimer."""
                    pc_g   = prio_color(todo_g.get("priorite","Cette semaine"))
                    done_g = bool(todo_g.get("fait"))
                    bg_g   = "#f0f0ee" if done_g else "#fff"
                    s_g    = "opacity:.45;text-decoration:line-through;" if done_g else ""
                    tid    = todo_g["id"]

                    _gc1,_gc2,_gc3 = st.columns([7,1,1])
                    with _gc1:
                        st.markdown(
                            f'<div style="border:0.5px solid #e0dbd2;border-left:3px solid {"#ccc" if done_g else pc_g};'
                            f'padding:7px 11px;margin:2px 0;background:{bg_g};">'
                            f'<div style="font-size:12px;font-weight:500;{s_g}">{str(todo_g["titre"]).replace("<","&lt;")}</div>'
                            f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{pc_g};margin-top:2px;">'
                            f'{todo_g.get("assignee","")} · {todo_g.get("priorite","")}'
                            f'</div></div>',
                            unsafe_allow_html=True)
                    with _gc2:
                        if editable and st.button("✏️", key=f"g_ed_{tid}", help="Modifier"):
                            st.session_state[f"gedit_{tid}"] = not st.session_state.get(f"gedit_{tid}", False)
                            st.rerun()
                    with _gc3:
                        if editable and st.button("🗑", key=f"g_del_{tid}", help="Supprimer"):
                            conn.execute("DELETE FROM todo_subtasks WHERE todo_id=?", (tid,))
                            conn.execute("DELETE FROM todos WHERE id=?", (tid,))
                            conn.commit(); st.rerun()

                    if st.session_state.get(f"gedit_{tid}"):
                        with st.form(f"gform_{tid}"):
                            _f1,_f2,_f3 = st.columns(3)
                            with _f1: _gt = st.text_input("Titre", value=str(todo_g.get("titre","")))
                            with _f2:
                                _gp = st.selectbox("Priorité", PRIORITES_TODO,
                                    index=PRIORITES_TODO.index(todo_g.get("priorite","Cette semaine"))
                                    if todo_g.get("priorite") in PRIORITES_TODO else 1)
                            with _f3:
                                _ga = st.selectbox("Assigné", MEMBRES,
                                    index=MEMBRES.index(todo_g.get("assignee","Tous"))
                                    if todo_g.get("assignee") in MEMBRES else 0)
                            _gd = st.text_area("Détails", value=str(todo_g.get("description","") or ""), height=45)
                            _gf = st.checkbox("✓ Marquer comme fait", value=done_g)

                            st.markdown("**Sous-tâches**")
                            try:
                                df_sub_g = pd.read_sql("SELECT * FROM todo_subtasks WHERE todo_id=? ORDER BY ordre",
                                                       conn, params=[int(tid)])
                            except Exception:
                                df_sub_g = pd.DataFrame()
                            sub_vals = []
                            for _si in range(10):
                                _ex = df_sub_g.iloc[_si] if _si < len(df_sub_g) else None
                                _sv = st.text_input(f"Sous-tâche {_si+1}",
                                    value=str(_ex["titre"]) if _ex is not None else "",
                                    key=f"gsub_{tid}_{_si}", placeholder=f"Sous-tâche {_si+1}")
                                sub_vals.append(_sv.strip())

                            _fb1,_fb2 = st.columns([4,1])
                            with _fb1: _save = st.form_submit_button("💾 Enregistrer", type="primary")
                            with _fb2: _cancel = st.form_submit_button("✕ Annuler")

                            if _save:
                                _done_at = str(date.today()) if _gf and not done_g else (todo_g.get("done_at","") if _gf else None)
                                conn.execute("UPDATE todos SET titre=?,priorite=?,assignee=?,description=?,fait=?,done_at=? WHERE id=?",
                                             (_gt,_gp,_ga,_gd,int(_gf),_done_at,tid))
                                conn.execute("DELETE FROM todo_subtasks WHERE todo_id=?", (tid,))
                                for _si,_sv in enumerate(sub_vals):
                                    if _sv:
                                        conn.execute("INSERT INTO todo_subtasks (todo_id,titre,assignee,ordre) VALUES (?,?,?,?)",
                                                     (tid,_sv,_ga,_si))
                                conn.commit()
                                st.session_state.pop(f"gedit_{tid}",None); st.rerun()
                            if _cancel:
                                st.session_state.pop(f"gedit_{tid}",None); st.rerun()

                # ── 3 colonnes ────────────────────────────────────────────────────
                col_urg, col_todo, col_done = st.columns(3)

                with col_urg:
                    st.markdown(
                        f'<div style="background:#c1440e18;border-radius:6px;padding:6px 10px;margin-bottom:8px;'
                        f'font-family:DM Mono,monospace;font-size:9px;font-weight:700;color:#c1440e;'
                        f'text-transform:uppercase;letter-spacing:.12em;">🔴 URGENT — {len(_df_urgent)}</div>',
                        unsafe_allow_html=True)
                    if _df_urgent.empty:
                        st.markdown('<div style="font-size:11px;color:#ccc;padding:4px 0;">Aucune tâche urgente</div>', unsafe_allow_html=True)
                    for _, t in _df_urgent.iterrows():
                        _render_task_card(t, conn)

                with col_todo:
                    st.markdown(
                        f'<div style="background:#7b506f18;border-radius:6px;padding:6px 10px;margin-bottom:8px;'
                        f'font-family:DM Mono,monospace;font-size:9px;font-weight:700;color:{EW_V};'
                        f'text-transform:uppercase;letter-spacing:.12em;">🟡 TODO — {len(_df_todo)}</div>',
                        unsafe_allow_html=True)
                    if _df_todo.empty:
                        st.markdown('<div style="font-size:11px;color:#ccc;padding:4px 0;">Aucune tâche en cours</div>', unsafe_allow_html=True)
                    for _, t in _df_todo.iterrows():
                        _render_task_card(t, conn)

                with col_done:
                    st.markdown(
                        f'<div style="background:#395f3018;border-radius:6px;padding:6px 10px;margin-bottom:8px;'
                        f'font-family:DM Mono,monospace;font-size:9px;font-weight:700;color:{EW_G};'
                        f'text-transform:uppercase;letter-spacing:.12em;">✅ DONE — {len(_df_done)}</div>',
                        unsafe_allow_html=True)
                    if _df_done.empty:
                        st.markdown('<div style="font-size:11px;color:#ccc;padding:4px 0;">Aucune tâche terminée</div>', unsafe_allow_html=True)
                    for _, t in _df_done.iterrows():
                        _render_task_card(t, conn)

            # ── Export CSV semaine ─────────────────────────────────────────────
            st.markdown("---")
            if not df_gestion.empty:
                df_export_g = df_gestion.copy()
                ...
                st.download_button(f"⬇ Export CSV — semaine {semaine_id}", buf_exp,
                    file_name=f"taches_{semaine_id}.csv", mime="text/csv", key="export_taches_csv")
    ...
    if tab_kpi is not None:
        with tab_kpi:
            st.markdown('<div class="section-title">KPI & Historique des tâches</div>', unsafe_allow_html=True)

            # ── Sélection de période ─────────────────────────────────────────
            _kp1,_kp2,_kp3 = st.columns(3)
            with _kp1:
                kpi_from = st.date_input("Depuis", value=date.today() - timedelta(days=90), key="kpi_from")
            with _kp2:
                kpi_to   = st.date_input("Jusqu'au", value=date.today(), key="kpi_to")
            with _kp3:
                kpi_mem  = st.selectbox("Membre", ["Tous"] + [m for m in MEMBRES if m != "Tous"], key="kpi_mem")

            # Convertir dates → format semaine
            _from_sem = (kpi_from - timedelta(days=kpi_from.weekday())).strftime("%Y-W%V")
            _to_sem   = (kpi_to   - timedelta(days=kpi_to.weekday())).strftime("%Y-W%V")

            # ── Charger les données ──────────────────────────────────────────
            try:
                _kpi_q = """SELECT t.*, ts.id as sub_id, ts.titre as sub_titre, ts.fait as sub_fait
                    FROM todos t
                    LEFT JOIN todo_subtasks ts ON ts.todo_id = t.id
                    WHERE t.semaine >= ? AND t.semaine <= ?"""
                _kpi_params = [_from_sem, _to_sem]
                if kpi_mem != "Tous":
                    _kpi_q += " AND t.assignee=?"
                    _kpi_params.append(kpi_mem)
                df_kpi_raw = pd.read_sql(_kpi_q, conn, params=_kpi_params)
            ...
            if df_kpi_raw.empty:
                ...
                # ── Stats globales ───────────────────────────────────────────
                df_todos_kpi = df_kpi_raw.drop_duplicates(subset=["id"])[["id","titre","assignee","semaine","fait","done_at","priorite","description"]]
                _total_tasks = len(df_todos_kpi)
                _done_tasks  = int(df_todos_kpi["fait"].sum())
                _pct_global  = round(_done_tasks/_total_tasks*100,1) if _total_tasks else 0

                st.markdown(
                    f'<div style="display:flex;gap:20px;margin:10px 0 16px;font-family:DM Mono,monospace;">'
                    f'<div><div style="font-size:9px;color:#8a7968;text-transform:uppercase;">Total tâches</div>'
                    f'<div style="font-size:22px;font-weight:600;color:#1a1a1a;">{_total_tasks}</div></div>'
                    f'<div><div style="font-size:9px;color:#8a7968;text-transform:uppercase;">Réalisées</div>'
                    f'<div style="font-size:22px;font-weight:600;color:#395f30;">{_done_tasks}</div></div>'
                    f'<div><div style="font-size:9px;color:#8a7968;text-transform:uppercase;">Taux</div>'
                    f'<div style="font-size:22px;font-weight:600;color:#7b506f;">{_pct_global}%</div></div>'
                    f'</div>',
                    unsafe_allow_html=True)

                # ── Par membre ───────────────────────────────────────────────
                st.markdown('<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.1em;margin:8px 0 4px;">Par membre</div>', unsafe_allow_html=True)
                _kpi_rows = []
                for _mb in [m for m in MEMBRES if m != "Tous"]:
                    _df_mb = df_todos_kpi[df_todos_kpi["assignee"]==_mb]
                    if _df_mb.empty: continue
                    _t = len(_df_mb); _d = int(_df_mb["fait"].sum())
                    _kpi_rows.append({
                        "Membre":_mb, "Total":_t, "Réalisées":_d,
                        "Non réalisées":_t-_d,
                        "Taux (%)":round(_d/_t*100,1) if _t else 0
                    })
                if _kpi_rows:
                    df_kpi_summary = pd.DataFrame(_kpi_rows)
                    st.dataframe(df_kpi_summary, hide_index=True, use_container_width=True)

                # ── Détail par tâche ─────────────────────────────────────────
                st.markdown('<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.1em;margin:12px 0 4px;">Détail des tâches</div>', unsafe_allow_html=True)
                df_detail = df_todos_kpi[["assignee","semaine","titre","priorite","fait","done_at","description"]].copy()
                df_detail["fait"] = df_detail["fait"].apply(lambda x: "✓" if x else "✗")
                df_detail = df_detail.rename(columns={
                    "assignee":"Membre","semaine":"Semaine","titre":"Tâche",
                    "priorite":"Priorité","fait":"Réalisée","done_at":"Date réalisation","description":"Description"
                })
                st.dataframe(df_detail.sort_values(["Membre","Semaine"]), hide_index=True, use_container_width=True)

                # ── Export CSV ───────────────────────────────────────────────
                st.markdown('---')
                _ec1,_ec2 = st.columns(2)
                with _ec1:
                    # Export synthèse par membre
                    if _kpi_rows:
                        import io as _io
                        _buf_sum = _io.BytesIO()
                        pd.DataFrame(_kpi_rows).to_csv(_buf_sum, index=False, encoding="utf-8-sig")
                        st.download_button(
                            f"⬇ Synthèse KPI ({kpi_from} → {kpi_to})",
                            _buf_sum.getvalue(),
                            file_name=f"kpi_synthese_{kpi_from}_{kpi_to}.csv",
                            mime="text/csv"
                        )
                with _ec2:
                    # Export détail complet
                    import io as _io
                    _buf_det = _io.BytesIO()
                    df_detail.to_csv(_buf_det, index=False, encoding="utf-8-sig")
                    st.download_button(
                        f"⬇ Détail tâches ({kpi_from} → {kpi_to})",
                        _buf_det.getvalue(),
                        file_name=f"kpi_detail_{kpi_from}_{kpi_to}.csv",
                        mime="text/csv"
                    )
