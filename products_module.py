# products_module.py — v5.4 — 1777106934
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
  ...
  <div style="text-align:right;">
    ...
    with st.form(f"costs_finance_{sel_pid}"):
        ...
        for sect, fields in COST_SECTIONS.items():
            ...
            if sect == "Packaging":
                st.markdown('<div style="font-size:10px;color:#8a7968;margin:2px 0 6px;">Sélectionner un packaging prédéfini — coût calculé automatiquement</div>', unsafe_allow_html=True)
                try:
                    _pkg_types = pd.read_sql("SELECT * FROM packaging_types ORDER BY nom", conn)
                    _pkg_opts  = ["— Aucun —"] + _pkg_types["nom"].tolist()
                    _pkg_ids   = {r["nom"]: r["id"] for _, r in _pkg_types.iterrows()}
                    _pkg_costs = {r["nom"]: float(r.get("cout_total",0) or 0) for _, r in _pkg_types.iterrows()}
                except Exception:
                    _pkg_opts=[]; _pkg_ids={}; _pkg_costs={}
                # Récupérer le packaging actuel de ce produit
                _cur_pkg_id = int(costs_dict.get("packaging_type_id",0) or 0)
                _cur_pkg_nom = ""
                try:
                    if _cur_pkg_id:
                        _r = conn.execute("SELECT nom FROM packaging_types WHERE id=?",(_cur_pkg_id,)).fetchone()
                        if _r: _cur_pkg_nom = _r[0]
                except Exception: pass
                _pkg_default = _cur_pkg_nom if _cur_pkg_nom in _pkg_opts else "— Aucun —"
                _pkg_sel = st.selectbox("Packaging", _pkg_opts,
                    index=_pkg_opts.index(_pkg_default) if _pkg_default in _pkg_opts else 0,
                    key=f"pkg_sel_{sel_pid}")
                if _pkg_sel != "— Aucun —":
                    _pkg_cout = _pkg_costs.get(_pkg_sel, 0)
                    all_inputs["packaging_type_id"] = _pkg_ids.get(_pkg_sel, 0)
                    all_inputs["cout_packaging"] = _pkg_cout
                    # Afficher le détail des items
                    try:
                        _pkg_id_sel = _pkg_ids.get(_pkg_sel,0)
                        _items = conn.execute("""SELECT nom_item, quantite, cout_unitaire FROM packaging_items
                            WHERE packaging_type_id=?""", (_pkg_id_sel,)).fetchall()
                        for _it in _items:
                            _it_tot = float(_it[1] or 0) * float(_it[2] or 0)
                            st.markdown(f'<div style="display:flex;justify-content:space-between;font-size:11px;padding:2px 0;border-bottom:1px solid #f0ece4;"><span>{_it[0]}</span><span style="font-family:DM Mono,monospace;">{_it[1]} × {fmt_eur_fn(_it[2])} = {fmt_eur_fn(_it_tot)}</span></div>', unsafe_allow_html=True)
                    except Exception: pass
                    st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:12px;font-weight:600;color:#395f30;margin-top:4px;">Total packaging : {fmt_eur_fn(_pkg_cout)}</div>', unsafe_allow_html=True)
                else:
                    all_inputs["packaging_type_id"] = 0
                    all_inputs["cout_packaging"] = 0.0
                continue

            if sect == "Étiquettes":
                st.markdown('<div style="font-size:10px;color:#8a7968;margin:2px 0 6px;">Cochez les étiquettes incluses — le coût est calculé depuis la fiche composition</div>', unsafe_allow_html=True)
                _etiq_fields = [
                ...
                for _ei, (_ek, _el) in enumerate(_etiq_fields):
                    with _ecols[_ei]:
                        _ev = st.checkbox(_el, value=bool(costs_dict.get(_ek, 0)), key=f"etiq_{_ek}_{sel_pid}")
                        all_inputs[_ek] = 1 if _ev else 0
                        # Afficher le coût associé depuis product_components
                        try:
                            _etiq_stk = conn.execute("""SELECT pc.cout_unitaire FROM product_components pc
                                WHERE pc.product_id=? AND LOWER(pc.nom_exact) LIKE ?""",
                                (sel_pid, f"%{_el.lower()[:8]}%")).fetchone()
                            if _etiq_stk:
                                st.markdown(f'<div style="font-size:9px;color:#8a7968;">{fmt_eur_fn(float(_etiq_stk[0] or 0))}</div>', unsafe_allow_html=True)
                        except Exception: pass
                continue
...
<div style="display:flex;justify-content:space-between;font-size:12px;padding:3px 0;border-bottom:1px solid #f0ece4;">
  ...
  <span style="font-family:'DM Mono',monospace;">{comp['quantite']} {comp['unite']} × {fmt_eur_fn(comp['cout_unitaire'])} = {fmt_eur_fn(val_c)}</span>
        ...
        with _pc1:
            srp_eu_new  = st.number_input("SRP EU (€)",  min_value=0.0, step=5.0, value=_srp_eu_cur, key=f"srp_eu_{sel_pid}")
        with _pc2:
            srp_jp_new  = st.number_input("SRP JP (¥)",  min_value=0.0, step=100.0, value=_srp_jp_cur, key=f"srp_jp_{sel_pid}")
        with _pc3:
            srp_us_new  = st.number_input("SRP US ($)",  min_value=0.0, step=5.0, value=_srp_us_cur,  key=f"srp_us_{sel_pid}")
        with _pc4:
...
<div style="background:#f5f0e8;border-left:3px solid #395f30;padding:8px 14px;margin:8px 0;font-family:'DM Mono',monospace;font-size:11px;">
  ...
  {f" · Marge brute : <strong>{_marge_v}%</strong>" if _marge_v else ""}
        ...
        if st.form_submit_button("💾 Enregistrer les coûts & prix", type="primary"):
