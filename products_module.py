# products_module.py — v5.32 — 1778145985
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    ...
    try:
        ...
        n_samples = c.execute("SELECT COUNT(*) FROM stock WHERE type_produit='Sample'").fetchone()[0]
        n_pkgs  = c.execute("SELECT COUNT(*) FROM packaging_types").fetchone()[0]

        # Tout est complet → rien à faire
        # Vérifier aussi que les produits finis sont dans le stock
        n_pf_in_stock = c.execute(
            "SELECT COUNT(*) FROM stock WHERE type_produit='Produit fini' AND ref LIKE 'EWS%'"
        ).fetchone()[0]

        # Vérifier des refs précises issues des tableaux
        try:
            _chk_wool  = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSWOOL-001A'").fetchone()[0]
            _chk_comp  = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSTRIM-003A'").fetchone()[0]
            _chk_pf    = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSJACKET-001A' AND type_produit='Produit fini'").fetchone()[0]
            _chk_pkg   = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSETQ-005A'").fetchone()[0]
            # Compter les items total MP+Composants (doit être 34)
            _n_mp_comp = c.execute("SELECT COUNT(*) FROM stock WHERE type_produit IN ('Matière première','Composant')").fetchone()[0]
        except Exception:
            _chk_wool = _chk_comp = _chk_pf = _chk_pkg = _n_mp_comp = 0

        # Tout est complet si les 34 MP/composants + 14 PF sont en DB
        if (n_prods >= 14 and n_comps >= 90 and n_pkgs >= 4
                and _chk_wool and _chk_comp and _chk_pf and _chk_pkg
                and _n_mp_comp >= 34 and n_pf_in_stock >= 14):
            return
        ...
        # STOCK manquant → vérifier les refs exactes du tableau inventaire
        try:
            _has_wool  = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSWOOL-001A'").fetchone()[0]
            _has_comp  = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSTRIM-003A'").fetchone()[0]
            _has_pf    = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSJACKET-001A' AND type_produit='Produit fini'").fetchone()[0]
            _has_pkg   = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSETQ-005A'").fetchone()[0]
            _n_mp_c    = c.execute("SELECT COUNT(*) FROM stock WHERE type_produit IN ('Matière première','Composant')").fetchone()[0]
        except Exception:
            _has_wool = _has_comp = _has_pf = _has_pkg = _n_mp_c = 0

        if not _has_wool or not _has_comp or not _has_pf or not _has_pkg or _n_mp_c < 34 or n_pf_in_stock < 14:
            try:
        ...
        # FOURNISSEURS — insérer si manquants (vérifier sur 15+)
        try:
            n_four = c.execute(
                "SELECT COUNT(*) FROM contacts WHERE type_contact='Fournisseur'"
            ).fetchone()[0]
            if n_four < 15:
                _insert_only_fournisseurs(conn)
        except Exception: _tb.print_exc()
...
def _insert_only_fournisseurs(conn):
    """Insère les fournisseurs depuis le tableau Eastwood Studio v1."""
    import traceback
    try:
        c = conn.cursor()
        # (nom, type_contact, sous_type, statut_fourn, pays, ville, adresse, email, telephone, nom_contact, site_web, instagram, notes, deja_travaille)
        FOURNISSEURS = [
            ("Ebay (vinty82)",         "Fournisseur","Composant",       "Testé - Stand By",  "Angleterre","",         "",                                         "","",                    "","https://www.ebay.de/usr/vinty82","",   "Zip chainette doré Talon Vintage 1960",1),
            ("Dam Bouton",             "Fournisseur","Composant",       "Testé - Stand By",  "France",    "",         "",                                         "","",                    "","","https://www.instagram.com/dam_boutons/","Boutton simple Maron 25mm",1),
            ("Yiwu Aiyi E-Commerce Firm","Fournisseur","Composant",     "Testé - Stand By",  "","",        "",                                         "","",                    "","","",     "Boutons nacrés Eastwood Studio",1),
            ("mapetitemercerie",        "Fournisseur","Composant",       "Testé - Stand By",  "France",    "Gaillac",  "316 avenue Gustave Eiffel, 81600 Gaillac, France","contact@mapetitemercerie.com","","","https://www.mapetitemercerie.com/fr/","","Étiquette textile Eastwood Studio",1),
            ("A&A Patrons (ma fashion mercerie)","Fournisseur","Composant","Testé - Stand By","France",   "Maves",    "Villerussien, 41500 Maves France",          "","‭+33680585557",        "","https://www.a-et-a.com/","https://www.instagram.com/aetapatrons","Étiquette de composition",1),
            ("Rubantin",               "Fournisseur","Composant",       "Testé - Stand By",  "France",    "Paris",    "15 bis rue Robert Blache 75010 Paris",      "pascal@rubantin.fr","", "Pascal (Manager)","https://www.rubantin.fr/","https://www.instagram.com/rubantin_officiel/","Étiquette de taille",1),
            ("LeBonCoin",              "Fournisseur","Composant",       "Testé - Out",       "France",    "",         "",                                         "","",                    "","","",                                  "Boutons Équipement Militaires Vintage 1960",1),
            ("Le bar à broder",        "Fournisseur","Impression / Broderie","Testé - Stand By","France", "Boulogne", "",                                         "tianarkv@gmail.com","‭+33‭6 65 47 15 93","Tianna (brodeuse)","","","Broderie légère patch logo EWS",1),
            ("Salhedine Chainstitch",  "Fournisseur","Impression / Broderie","Actif",         "France",   "Paris",    "",                                         "kaidsaleheddine@gmail.com","‭+33‭6 41 25 16 20","Salhedine","https://arp-paris.com/","","Broderie chainstitch dos Recherche Club Shirt",1),
            ("Textrend.fr",            "Fournisseur","Matière première", "Testé - Stand By", "France",    "",         "",                                         "info@textrend.fr","",   "","https://textrend.fr/","https://www.instagram.com/textrend_fr/","Lin & Viscose Bleu Pigeon / Vert Pastel / Blanc Cassé",1),
            ("Insho Atelier",          "Fournisseur","Matière première", "Testé - Stand By", "France",    "Angers",   "2 Pl. de la Paix, 49100 Angers, France",    "raphael@insho-atelier.com","‭+33‭6 08 50 76 87","Raphaël (Co-founder)","https://www.insho-atelier.com/","https://www.instagram.com/atelier_insho/","Sergé de viscose All-Over / Soie Motif Trésor",1),
            ("Rêve de Gosse",          "Fournisseur","Matière première", "Actif",            "France",    "Paris",    "18 Rue de Picardie, 75003 Paris, France",   "","‭+33663750772",      "","","https://www.instagram.com/revedegosse","Laines Whipcord (5 couleurs) + Tartans Shadow & Clear",1),
            ("Kiryu Jumper Lab",       "Fournisseur","Production / Confection","Actif",       "Japon",     "Kiryu",    "",                                         "","",                    "Kiryu Jumper Lab","","","Fabrication Miura Jacket & Akagi Jacket. Broderie Yokoburi.",1),
            ("Laperruque",             "Fournisseur","Matière première", "Actif",            "France",    "Paris",    "",                                         "","",                    "","","","Cuir de veau tanné marron pour Memory Wallet",1),
            ("Xavier Boully",          "Fournisseur","Impression / Broderie","Actif",        "France",    "Paris",    "",                                         "","",                    "Xavier Boully","","","Peinture main tigre sur Memory Wallet",1),
            ("Xiangtong",              "Fournisseur","Matière première", "Actif",            "Chine",     "",         "",                                         "","",                    "","","","Coton blanc casquette + lanière cuir végan pour Souvenir Cap",1),
            ("Mutsaers Textiles B.V.", "Fournisseur","Matière première", "À tester",         "Pays-Bas",  "Tilbourg", "Technopol 29, 5026 SB Tilburg",             "info@mutsaerstexiles.nl","‭+31 13 5351025","","","","Tissu lin",0),
            ("Metexco",                "Fournisseur","Matière première", "À tester",         "France",    "Paris",    "10 Rue du Croissant, 75002 Paris",          "metexco@hotmail.com","‭+331 42 36 90 52","","https://www.metexco.fr/","https://www.instagram.com/metexcoparis/","Tissu neuf",0),
            ("Eurotessile",            "Fournisseur","Matière première", "À tester",         "France",    "Saint-Soupplets","2 avenue du Montboulon, 77165 Saint-Soupplets","info@eurotessile.com","‭+331 45 98 78 55","","https://eurotessile.com/","","Tissu neuf — MOQ 1 rouleau",0),
            ("Wouters Textiles",       "Fournisseur","Matière première", "À tester",         "Pays-Bas",  "Harderwijk","Edisonstraat 1-3, 3846 AS Harderwijk, NL", "","‭+31 341 415 984",   "","https://www.wouters-textiles.com/","https://www.instagram.com/wouterstextiles/","Tissu neuf — MOQ 10 mètres",0),
            ("Beglarian Fabrics",      "Fournisseur","Matière première", "À tester",         "Pays-Bas",  "Tilburg",  "Docklands 1, 5026 SL Tilburg, NL",          "support@rijstextiles.com","‭+31 13 737 0133","","https://www.rijstextiles.com/","","Tissu neuf",0),
            ("Rijs Textiles",          "Fournisseur","Matière première", "À tester",         "Pays-Bas",  "",         "",                                         "info@northern-linen.com","‭+31 6 5366 6141","Hans Diekema","https://northern-linen.com/","","Tissu lin",0),
            ("Mutsaerstextiles",       "Fournisseur","Matière première", "À tester",         "France",    "Gleizé",   "144 Allée des Caillotières, 69400 Gleizé",  "info@beglarianfabrics.com","","","https://beglarianfabrics.com/","","Tissu neuf",0),
            ("Véronique Jordan",       "Fournisseur","Patronage",        "Actif",            "France",    "",         "",                                         "","‭+33000000000",       "","","","Patronage",1),
        ]
        for (nom,type_c,sous_t,statut,pays,ville,adresse,email,tel,nom_ct,web,ig,notes,deja) in FOURNISSEURS:
            # Vérifier doublon dans contacts
            ex_ct = c.execute("SELECT id FROM contacts WHERE nom=? AND type_contact='Fournisseur'", (nom,)).fetchone()
            if not ex_ct:
                c.execute("""INSERT INTO contacts (nom,type_contact,sous_type,pays,ville,adresse,email,telephone,notes,importance)
                    VALUES (?,?,?,?,?,?,?,?,?,'Normal')""",
                    (nom,type_c,sous_t,pays,ville,adresse,email,tel,notes))
            # Vérifier doublon dans fournisseurs
            ex_f = c.execute("SELECT id FROM fournisseurs WHERE nom=?", (nom,)).fetchone()
            if not ex_f:
                try:
                    c.execute("""INSERT INTO fournisseurs
                        (nom,sous_type,statut_fourn,pays,ville,adresse,email,telephone,nom_contact,site_web,instagram,notes,deja_travaille,importance)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'Normal')""",
                        (nom,sous_t,statut,pays,ville,adresse,email,tel,nom_ct,web,ig,notes,deja))
                except Exception: pass
        conn.commit()
    except Exception:
        traceback.print_exc()
...
def _insert_only_stock(conn):
    """Insère TOUS les items stock depuis les tableaux Eastwood Studio.
    Source : tableau inventaire v1 + tableau produits finis.
    Tout ce qui n'est pas en stock physique = 0, mais l'item existe.
    """
    import traceback
    try:
        c = conn.cursor()
        ...
        # ── Matières premières ────────────────────────────────────────────────
        # (ref, nom, type_produit, sous_type, prix_unitaire, qte_stock, unite, made_in)
        MP = [
            ("EWSWOOL-001A","Laine Whipcord Chocolate",    "Matière première","Tissu lourd",            60.0, 2,   "Mètre",  "Angleterre"),
            ("EWSWOOL-001B","Laine Whipcord Tobacco",      "Matière première","Tissu lourd",            60.0, 4,   "Mètre",  "Angleterre"),
            ("EWSWOOL-001C","Laine Whipcord Greige",       "Matière première","Tissu lourd",            60.0, 0,   "Mètre",  "Angleterre"),
            ("EWSWOOL-001D","Laine Whipcord Sand",         "Matière première","Tissu lourd",            60.0, 2,   "Mètre",  "Angleterre"),
            ("EWSWOOL-001E","Laine Whipcord Black",        "Matière première","Tissu lourd",            60.0, 0,   "Mètre",  "Angleterre"),
            ("EWSCOTT-001A","Mix coton laine tartan Shadow","Matière première","Tissu léger",           30.0, 20,  "Mètre",  "France"),
            ("EWSCOTT-001B","Mix coton laine tartan Clear", "Matière première","Tissu léger",           30.0, 10,  "Mètre",  "France"),
            ("EWSCELL-001A","Rayonne satinée rembourée Verte","Matière première","Tissu lourd",          0.0, 0,   "Mètre",  "Japon"),
            ("EWSCELL-001B","Rayonne satinée Nishijin All-Over","Matière première","Tissus imprimé / motif",0.0,0, "Mètre",  "Japon"),
            ("EWSCELL-002A","Gabardine de rayonne Tobacco", "Matière première","Tissu léger",            0.0, 0,   "Mètre",  "Japon"),
            ("EWSCELL-003A","Rayonne satinée Crème",        "Matière première","Tissu léger",            0.0, 0,   "Mètre",  "Japon"),
            ("EWSBLND-001A","Lin & Viscose Bleu Pigeon",   "Matière première","Tissu léger",            15.0, 0,   "Mètre",  "France"),
            ("EWSBLND-001B","Lin & Viscose Vert Pastel",   "Matière première","Tissu léger",            15.0, 0,   "Mètre",  "France"),
            ("EWSBLND-001C","Lin & Viscose Blanc Cassé",   "Matière première","Tissu léger",            15.0, 2,   "Mètre",  "France"),
            ("EWSCOTT-003A","Voile de coton blanc",        "Matière première","Tissu léger",             0.0, 0,   "Mètre",  "France"),
            ("EWSVSC-001A", "Sergé de viscose All-Over",   "Matière première","Tissus imprimé / motif", 21.67,0,   "Mètre",  "France"),
            ("EWSCOTT-002A","Coton blanc casquette",       "Matière première","Maille",                  0.0, 0,   "Pièces", "France"),
            ("EWSCELL-004A","Soie Motif Trésor",           "Matière première","Tissus imprimé / motif", 50.0, 0,   "Pièces", "France"),
            ("EWSLEAT-001A","Wallet Cuir de veau tanné marron","Matière première","Cuir",                0.0, 0,   "Pièces", "France"),
            ("EWSLEAT-002A","Lanière cuir végan",          "Matière première","Cuir",                    0.0, 0,   "Pièces", "France"),
        ]

        # ── Composants ────────────────────────────────────────────────────────
        COMP = [
            ("EWSTRIM-005A","Zip double face WALDES",                  "Composant","Zip",     5.0,  0,   "Pièces","États-Unis"),
            ("EWSTRIM-004A","Zip chainette doré Talon Vintage 1960",   "Composant","Zip",     9.0,  7,   "Pièces","États-Unis"),
            ("EWSTRIM-004B","Zip doré Talon Vintage",                  "Composant","Zip",     0.0,  0,   "Pièces","États-Unis"),
            ("EWSTRIM-002A","Boutton simple Maron 25mm",               "Composant","Bouton",  1.0,  0,   "Pièces","France"),
            ("EWSTRIM-006A","Boutons cat's eye noir",                   "Composant","Bouton",  1.0,  0,   "Pièces","Japon"),
            ("EWSTRIM-003A","Boutons nacrés Eastwood Studio",          "Composant","Bouton",  0.41, 150, "Pièces","France"),
            ("EWSTRIM-001A","Boutons Équipement Militaires Vintage 1960","Composant","Bouton", 1.35, 5,   "Pièces","France"),
            ("EWSPKG-001A", "Étiquette de composition",               "Composant","Étiquette",1.0, 0,   "Pièces","France"),
            ("EWSETQ-002A", "Étiquette de taille T1 (S)",             "Composant","Étiquette",1.0, 0,   "Pièces","France"),
            ("EWSETQ-002B", "Étiquette de taille T2 (M)",             "Composant","Étiquette",1.0, 0,   "Pièces","France"),
            ("EWSETQ-002C", "Étiquette de taille T3 (L)",             "Composant","Étiquette",1.0, 0,   "Pièces","France"),
            ("EWSETQ-002D", "Étiquette de taille T4 (XL)",            "Composant","Étiquette",1.0, 0,   "Pièces","France"),
            ("EWSETQ-003A", "Étiquette fabrication Française",        "Composant","Étiquette",1.0, 0,   "Pièces","France"),
            ("EWSETQ-004A", "Étiquette textile Eastwood Studio",      "Composant","Étiquette",1.0, 0,   "Pièces","France"),
        ]

        # ── Packaging ─────────────────────────────────────────────────────────
        PKG = [
            ("EWSETQ-005A","Tag numéro identification",           "Packaging","Packaging",1.0, 0, "Pièces","France"),
            ("EWSPKG-004A","Sticker logo blanc cercle",           "Packaging","Packaging",1.0, 0, "Pièces","France"),
            ("EWSPKG-002A","Pochette enveloppe postal XL craft",  "Packaging","Packaging",1.0, 0, "Pièces","France"),
            ("EWSPKG-003A","Feuille recherche A4 craft",          "Packaging","Packaging",1.0, 0, "Pièces","France"),
        ]

        # Insérer/remplacer MP + Composants + Packaging
        for items in [MP, COMP, PKG]:
            for ref, nom, type_p, sous_t, cout, qte, unite, made_in in items:
                try:
                    c.execute("""INSERT OR REPLACE INTO stock
                        (ref, description, type_produit, sous_type, prix_unitaire, qte_stock, unite, notes)
                        VALUES (?,?,?,?,?,?,?,?)""",
                        (ref, nom, type_p, sous_t, cout, qte, unite, f"Made in: {made_in}"))
                except Exception:
                    # Si colonne notes n'existe pas
                    try:
                        c.execute("""INSERT OR REPLACE INTO stock
        ...
        # ── Produits finis ─────────────────────────────────────────────────────
        # (ref, qte_stock, qte_sample, detail_tailles)
        # Source: tableau inventaire — "cout unitaire ref produit" = SRP/coût depuis fiche produit
        STOCK_PF = [
            ("EWSJACKET-001A","Waterfowl Jacket Chocolate",    0, 0, ""),
            ("EWSJACKET-001B","Waterfowl Jacket Tobacco",      1, 0, "1×T2"),
            ("EWSJACKET-001C","Waterfowl Jacket Greige",       0, 0, ""),
            ("EWSJACKET-001D","Waterfowl Jacket Sand",         0, 0, ""),
            ("EWSJACKET-001E","Waterfowl Jacket Black",        0, 0, ""),
            ("EWSJACKET-002A","Miura Jacket",                  0, 1, "Sample"),
            ("EWSJACKET-003A","Akagi Jacket",                  0, 0, ""),
            ("EWSSHIRT-001A", "Recherche Club Shirt Cloud Blue",  3, 1, "2×T2 + 1×T1"),
            ("EWSSHIRT-001B", "Recherche Club Shirt Pastel Green",3, 1, "1×T1 + 1×T2 + 1×T3"),
            ("EWSSHIRT-002A", "Lutèce Plage Shirt",            0, 1, ""),
            ("EWSGEAR-001A",  "Souvenir Cap Deep Rust",        11, 1, ""),
            ("EWSGEAR-001B",  "Souvenir Cap Faded Green",      11, 1, ""),
            ("EWSGEAR-002A",  "Paris le trésor Silk Square",   0,  1, ""),
            ("EWSGEAR-003A",  "Memory Wallet",                 8,  1, ""),
        ]

        for ref_pf, nom_pf, qte_r, qte_s, detail in STOCK_PF:
            # Récupérer le coût total depuis product_costs si disponible
            try:
                prod_row = c.execute(
                    "SELECT p.id, pc.cout_mp_principale FROM products p "
                    "LEFT JOIN product_costs pc ON pc.product_id=p.id WHERE p.ref=?",
                    (ref_pf,)).fetchone()
                cout_pf = float(prod_row[1] or 0) if prod_row and prod_row[1] else 0.0
            except Exception:
                cout_pf = 0.0

            notes_pf = f"Stock retail: {qte_r} | Sample: {qte_s}"
            if detail: notes_pf += f" | {detail}"

            try:
                c.execute("""INSERT OR REPLACE INTO stock
                    (ref, description, type_produit, sous_type, prix_unitaire, qte_stock, unite, notes)
                    VALUES (?,?,'Produit fini','Chapter I',?,?,'Pièces',?)""",
                    (ref_pf, nom_pf, cout_pf, qte_r, notes_pf))
            except Exception:
                try:
                    c.execute("""INSERT OR REPLACE INTO stock
...
def _insert_migration_data(conn):
    ...
    try:
        ...
        # ══ STOCK PRODUITS FINIS + SAMPLES ════════════════════════════════════
        # (ref, qte_retail, qte_sample, detail_tailles)
        STOCK_PF = [
            ("EWSJACKET-001A", 0,  0,  ""),
            ("EWSJACKET-001B", 1,  0,  "1xT2"),
            ("EWSJACKET-001C", 0,  0,  ""),
            ("EWSJACKET-001D", 0,  0,  ""),
            ("EWSJACKET-001E", 0,  0,  ""),
            ("EWSJACKET-002A", 0,  1,  ""),
            ("EWSJACKET-003A", 0,  0,  ""),
            ("EWSSHIRT-001A",  3,  1,  "2xT2 + 1xT1"),
            ("EWSSHIRT-001B",  3,  1,  "1xT1 + 1xT2 + 1xT3"),
            ("EWSSHIRT-002A",  0,  1,  ""),
            ("EWSGEAR-001A",   11, 1,  ""),
            ("EWSGEAR-001B",   11, 1,  ""),
            ("EWSGEAR-002A",   0,  1,  ""),
        ...
        for sku, qte_r, qte_s, det in STOCK_PF:
            row = c.execute("SELECT nom,couleurs FROM products WHERE ref=?", (sku,)).fetchone()
            if not row: continue
            db = row[0] + (" — " + row[1] if row[1] else "")
            dr = db + (f" ({det})" if det else "")
            c.execute("INSERT OR REPLACE INTO stock (ref,description,type_produit,qte_stock,unite) VALUES (?,?,'Produit fini',?,'Pièces')",
                (sku, dr, qte_r))
            if qte_s > 0:
                c.execute("INSERT OR REPLACE INTO stock (ref,description,type_produit,sous_type,qte_stock,unite) VALUES (?,?,'Sample','Sample',?,'Pièces')",
                    (sku+"-SAMPLE", db+" — Sample", qte_s))
        conn.commit()
