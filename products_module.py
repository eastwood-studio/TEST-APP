# products_module.py — v5.25 — 1777371909
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
<span style="color:#8a7968;font-size:10px;">Mesures · quantités · composition matières · composants. Usage interne production.</span>
    ...
    # ── Marketing (hors du form — st.button autorisé ici) ─────────────────────
    if sel_pid:
        _costs_mkt_dict = {}
        try:
            _row = get_costs(conn, sel_pid)
            _costs_mkt_dict = dict(_row) if _row is not None else {}
        except Exception:
            pass
        _pct_cur = float(_costs_mkt_dict.get("cout_marketing_pct", 5.0) or 5.0)
        st.markdown(
            ...
            f'border-bottom:2px solid #d9c8ae;padding:8px 0 4px;margin:14px 0 8px;">'
            f'Marketing</div>',
            unsafe_allow_html=True)
        _mki1, _mki2, _mki3 = st.columns([2, 2, 3])
        with _mki1:
            _pct_in = st.number_input("Pourcentage marketing (%)",
                min_value=0.0, max_value=100.0, step=0.5, value=_pct_cur,
                key=f"mkt_pct_final_{sel_pid}",
                disabled=not _couts_editable)
        with _mki2:
            _tot_all = compute_cost_totals(_costs_mkt_dict).get("TOTAL", 0.0)
            _mkt_exist = float(_costs_mkt_dict.get("cout_marketing", 0) or 0)
            _tot_base_mkt = max(0.0, _tot_all - _mkt_exist)
            _mkt_final = round(_tot_base_mkt * _pct_in / 100, 2)
            st.metric(f"Montant ({_pct_in:.0f}%)", fmt_eur_fn(_mkt_final))
        with _mki3:
            st.markdown('<div style="margin-top:28px;"></div>', unsafe_allow_html=True)
            if _couts_editable and st.button(f"✓ Appliquer {_pct_in:.0f}%", key=f"mkt_btn_final_{sel_pid}"):
                try:
                    conn.execute(
                        "UPDATE product_costs SET cout_marketing_pct=?, cout_marketing=? WHERE product_id=?",
                        (_pct_in, _mkt_final, sel_pid))
                    conn.commit()
                    st.success(f"✓ Marketing {_pct_in:.0f}% → {fmt_eur_fn(_mkt_final)}")
                    st.rerun()
                except Exception as _e:
                    st.error(str(_e))
