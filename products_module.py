# products_module.py — v5.23 — 1777359676
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
  ...
  <div style="text-align:right;">
    ...
    with st.form(f"costs_finance_{sel_pid}"):
        ...
        for sect, fields in COST_SECTIONS.items():
            ...
            if sect == "Marketing":
                st.markdown('<div style="font-size:10px;color:#8a7968;margin:2px 0 6px;">Le % marketing s applique sur le total des coûts. Entrez le %, cliquez Appliquer.</div>', unsafe_allow_html=True)
                _mk1, _mk2, _mk3 = st.columns([2,2,3])
                with _mk1:
                    pct_mkt = st.number_input("Pourcentage (%)", min_value=0.0, max_value=100.0, step=0.5,
                        value=float(costs_dict.get("cout_marketing_pct",5.0) or 5.0),
                        key=f"mkt_pct_{sel_pid}", disabled=not _couts_editable)
                    all_inputs["cout_marketing_pct"] = pct_mkt
                with _mk2:
                    # Calculer le total actuel pour afficher le montant marketing
                    _total_for_mkt = sum(float(all_inputs.get(k,0) or 0)
                        for sect2, flds2 in COST_SECTIONS.items()
                        for k,_ in flds2
                        if sect2 != "Marketing" and k in all_inputs)
                    _mkt_amount = round(_total_for_mkt * pct_mkt / 100, 2)
                    all_inputs["cout_marketing"] = _mkt_amount
                    st.metric("Montant marketing", fmt_eur_fn(_mkt_amount))
                with _mk3:
                    if _couts_editable:
                        st.markdown('<div style="margin-top:28px;"></div>', unsafe_allow_html=True)
                        if st.button(f"✓ Appliquer {pct_mkt:.0f}% sur le total", key=f"mkt_apply_{sel_pid}"):
                            try:
                                conn.execute("UPDATE product_costs SET cout_marketing_pct=?, cout_marketing=? WHERE product_id=?",
                                             (pct_mkt, _mkt_amount, sel_pid))
                                conn.commit()
                                st.success(f"✓ Marketing {pct_mkt:.0f}% appliqué → {fmt_eur_fn(_mkt_amount)}")
                                st.rerun()
