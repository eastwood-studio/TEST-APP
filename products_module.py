# products_module.py — v5.8 — 1777277962
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    """Migration data : vérifie les SKU réels en DB. Se lance tant que les données ne sont pas à jour."""
    try:
        # Vérifier si les bons SKU sont déjà présents
        prod_ok = conn.execute("SELECT COUNT(*) FROM products WHERE ref IN ('EWSJACKET-001A','EWSJACKET-001B','EWSSHIRT-001A','EWSGEAR-001A')").fetchone()
        stock_ok = conn.execute("SELECT COUNT(*) FROM stock WHERE ref IN ('EWSWOOL-001A','EWSTRIM-003A','EWSETQ-002A')").fetchone()
        pkg_ok   = conn.execute("SELECT COUNT(*) FROM packaging_types WHERE nom='Packaging Chapter I'").fetchone()
        if prod_ok and prod_ok[0] >= 4 and stock_ok and stock_ok[0] >= 3 and pkg_ok and pkg_ok[0] >= 1:
...
def _insert_migration_data(conn):
    ...
    try:
        ...
        c.execute("DELETE FROM stock WHERE type_produit='Produit fini'")
        ...
        PKGS = [
            ...
            ("Packaging Chapter II","Miura / Akagi / Shirts & Accessories B2C",[("EWSETQ-005A","Tag numéro identification",1),("EWSPKG-004A","Sticker logo blanc cercle",2),("EWSPKG-003A","Feuille recherche A4 craft",1)]),
        ]
...
def init_products_db(conn):
    ...
    # Migration données produits/stock/packaging (vérifie les SKU réels)
    try:
...
<div style="margin-bottom:10px;">
  ...
  {badge_cout}
                        ...
                        _is_selected = (st.session_state.get("cat_detail_id") == int(prod["id"]))
                        btn_label = "✕ Fermer" if _is_selected else "Voir →"
                        if st.button(btn_label, key=f"voir_{prod['id']}",
                                     type="primary" if _is_selected else "secondary"):
                            if _is_selected:
                                st.session_state.pop("cat_detail_id", None)
                            else:
                                st.session_state["cat_detail_id"] = int(prod["id"])
                            st.rerun()


    # ── PANNEAU DÉTAIL PRODUIT (lecture seule) ──────────────────────────────────
    _detail_pid = st.session_state.get("cat_detail_id")
    if _detail_pid and tab_cat:
        with tab_cat:
            df_det = get_product(conn, _detail_pid)
            if not df_det.empty:
                det = df_det.iloc[0]
                det_imgs = get_images(conn, _detail_pid)
                det_comps = get_components(conn, _detail_pid)
                try:
                    det_arcs = pd.read_sql("SELECT type_archive,notes,nom_fichier FROM product_archives WHERE product_id=? ORDER BY id",
                                           conn, params=[_detail_pid])
                except Exception:
                    det_arcs = pd.DataFrame()

                st.markdown(f'<div style="border-top:2px solid #d9c8ae;margin:20px 0 12px;padding-top:16px;"><span style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;letter-spacing:.2em;text-transform:uppercase;">Détail produit</span></div>', unsafe_allow_html=True)

                d_left, d_right = st.columns([1, 2])
                with d_left:
                    if not det_imgs.empty and det_imgs.iloc[0]["data"] is not None:
                        try:
                            _db64 = base64.b64encode(bytes(det_imgs.iloc[0]["data"])).decode()
                            _dext = (det_imgs.iloc[0]["nom_fichier"] or "img.jpg").split(".")[-1].lower()
                            _dmime = "image/jpeg" if _dext in ("jpg","jpeg") else f"image/{_dext}"
                            st.markdown(f'<img src="data:{_dmime};base64,{_db64}" style="width:100%;border-radius:2px;"/>', unsafe_allow_html=True)
                        except Exception:
                            st.markdown(f'<div style="aspect-ratio:3/4;background:{EW_C};display:flex;align-items:center;justify-content:center;"><span style="font-size:9px;color:{EW_B};">AUCUN VISUEL</span></div>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<div style="aspect-ratio:3/4;background:{EW_C};display:flex;align-items:center;justify-content:center;"><span style="font-size:9px;color:{EW_B};">AUCUN VISUEL</span></div>', unsafe_allow_html=True)

                with d_right:
                    _stat_c = {"Disponible":EW_G,"Sample & Testing":"#c9800a","Recherche":EW_V,"Archive":"#888"}.get(str(det.get("statut","")),"#888")
                    st.markdown(f"""
<div style="padding:4px 0 12px;">
  <div style="font-family:DM Mono,monospace;font-size:9px;color:{EW_B};">{det.get('ref','')} · {det.get('collection','')}</div>
  <div style="font-size:22px;font-weight:500;color:#1a1a1a;margin:4px 0 2px;">{det['nom']}</div>
  {f'<div style="font-size:14px;font-style:italic;color:{EW_B};">{det["couleurs"]}</div>' if det.get('couleurs') else ''}
  <div style="font-family:DM Mono,monospace;font-size:9px;color:{_stat_c};margin-top:4px;">● {det.get('statut','')}</div>
</div>""", unsafe_allow_html=True)

                    if det.get("description"):
                        st.markdown(f'<div style="font-size:12px;color:#1a1a1a;line-height:1.6;margin-bottom:8px;">{det["description"]}</div>', unsafe_allow_html=True)

                    _det_rows = [
                        ("MATIÈRES", str(det.get("matieres","") or "").replace("\n"," · ")),
                        ("TAILLES",  str(det.get("tailles","") or "")),
                        ("MADE IN",  str(det.get("made_in","") or "")),
                        ("LIVRAISON",str(det.get("delivery","") or "")),
                        ("WP EU",    f"{det.get('prix_wholesale_eu',0) or 0:.0f} €"),
                        ("SRP",      f"{det.get('prix_retail_eur',0) or 0:.0f} €"),
                        ("MOQ",      f"{det.get('moq','—')} units"),
                    ]
                    for _fl, _fv in _det_rows:
                        if _fv and _fv.strip() and _fv not in ("0 €","— units"):
                            st.markdown(f"""
<div style="display:flex;gap:10px;padding:5px 0;border-bottom:0.5px solid #ede3d3;">
  <div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};min-width:70px;text-transform:uppercase;padding-top:1px;">{_fl}</div>
  <div style="font-size:11px;color:#1a1a1a;">{_fv}</div>
</div>""", unsafe_allow_html=True)

                    if not det_comps.empty:
                        st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;margin-top:10px;margin-bottom:4px;">Composants</div>', unsafe_allow_html=True)
                        for _, comp in det_comps.iterrows():
                            st.markdown(f'<div style="font-size:10px;color:#1a1a1a;padding:2px 0;">· {comp.get("nom_exact") or comp.get("nom","")} — {comp.get("quantite","")} {comp.get("unite","")}</div>', unsafe_allow_html=True)

                    if not det_arcs.empty:
                        st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;margin-top:10px;margin-bottom:4px;">Archives</div>', unsafe_allow_html=True)
                        for _, arc in det_arcs.iterrows():
                            st.markdown(f'<div style="font-size:10px;color:{EW_V};padding:2px 0;">— {arc.get("type_archive","")} : {arc.get("notes","") or arc.get("nom_fichier","")}</div>', unsafe_allow_html=True)

                btn1, btn2 = st.columns(2)
                with btn1:
                    if st.button("✕ Fermer le détail", key="det_close"):
                        st.session_state.pop("cat_detail_id", None); st.rerun()
                with btn2:
                    if can_fn("finance_write") and st.button("✏️ Modifier →", key="det_goto_gestion"):
                        st.session_state["gestion_art_presel_id"] = _detail_pid
                        st.session_state.pop("cat_detail_id", None); st.rerun()

                st.markdown('<div style="height:1px;background:#d9c8ae;margin:20px 0;"></div>', unsafe_allow_html=True)
    ...
    if tab_new is not None:
        with tab_new:
            ...
            # ── Composition (optionnelle à la création) ──────────────────────
            st.markdown('<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.12em;margin:8px 0 4px;">Composition (optionnel)</div>', unsafe_allow_html=True)

            # Charger les fournisseurs depuis contacts
            try:
                _fournisseurs_db = [r[0] for r in conn.execute(
                    "SELECT DISTINCT nom FROM contacts WHERE type_contact='Fournisseur' ORDER BY nom"
                ).fetchall() if r[0]]
            except Exception:
                _fournisseurs_db = []
            _fournisseurs_opts = ["— Saisie libre —"] + _fournisseurs_db

            # Charger les matières en stock (catégorie MP)
            try:
                _mp_stock = conn.execute(
                    "SELECT ref, description, type_produit FROM stock ORDER BY type_produit, description"
                ).fetchall()
                _mp_opts_all = {f"{s[1] or s[0]} ({s[2]})": s[0] for s in _mp_stock}
            except Exception:
                _mp_opts_all = {}

            _CATS_COMP_N = ["MP Principale (Main Fabric)","MP Secondaire","Doublure","Broderie","Zip","Bouton","Étiquette","Packaging","Autre"]
            _CATS_TYPES  = {
                "MP Principale (Main Fabric)": ["Produit fini","Matière première"],
                "MP Secondaire": ["Matière première","Composant"],
                "Doublure": ["Matière première","Composant"],
                "Broderie": ["Composant"],
                "Zip": ["Composant"],
                "Bouton": ["Composant"],
                "Étiquette": ["Composant","Packaging"],
                "Packaging": ["Packaging"],
                "Autre": None,
            }

            # Étiquettes obligatoires auto-ajoutées
            _etiq_auto = [
                ("Étiquette composition","Étiquette","1","Pièces"),
                ("Étiquette taille","Étiquette","1","Pièces"),
                ("Étiquette fabrication française","Étiquette","1","Pièces"),
                ("Étiquette textile","Étiquette","1","Pièces"),
            ]
            st.markdown('<div style="font-size:10px;color:#395f30;margin:2px 0 6px;">✓ 4 étiquettes ajoutées automatiquement à chaque produit (composition, taille, fabrication française, textile)</div>', unsafe_allow_html=True)

            n_comp_n = st.number_input("Nombre de composants supplémentaires", min_value=0, max_value=12, value=0, step=1, key="new_prod_n_comp")
            _new_comps = list(_etiq_auto)  # Partir des étiquettes auto
            for _ci in range(int(n_comp_n)):
                _nc0,_nc1,_nc2,_nc3,_nc4,_nc5 = st.columns([2,2,2,1,1,2])
                with _nc0:
                    _c_cat = st.selectbox("Catégorie", _CATS_COMP_N, key=f"nc_cat_{_ci}")
                with _nc1:
                    # Filtrer le stock selon la catégorie
                    _types_filtre = _CATS_TYPES.get(_c_cat)
                    if _types_filtre:
                        _mp_filtered = ["— Saisie libre —"] + [k for k,v in _mp_opts_all.items()
                                        if any(t in k for t in _types_filtre)]
                    else:
                        _mp_filtered = ["— Saisie libre —"] + list(_mp_opts_all.keys())
                    _c_stock_sel = st.selectbox("Stock/MP", _mp_filtered, key=f"nc_stk_{_ci}")
                with _nc2:
                    if _c_stock_sel == "— Saisie libre —":
                        _c_nom = st.text_input("Nom composant", placeholder="ex: Whipcord Wool", key=f"nc_nom_{_ci}")
                        _c_ref = ""
                    else:
                        _c_nom = _c_stock_sel.split(" (")[0]
                        _c_ref = _mp_opts_all.get(_c_stock_sel, "")
                        st.markdown(f'<div style="font-size:10px;color:#395f30;padding:5px 0;">✓ Ref: {_c_ref}</div>', unsafe_allow_html=True)
                with _nc3:
                    _c_qte = st.number_input("Qté", min_value=0.0, step=0.5, key=f"nc_qte_{_ci}")
                with _nc4:
                    _c_unit = st.selectbox("Unité", ["Mètre","Pièces","kg","g","m²"], key=f"nc_unit_{_ci}")
                with _nc5:
                    _c_four_sel = st.selectbox("Fournisseur", _fournisseurs_opts, key=f"nc_four_sel_{_ci}")
                    if _c_four_sel == "— Saisie libre —":
                        _c_four = st.text_input("Nom fournisseur", key=f"nc_four_{_ci}")
                    else:
                        _c_four = _c_four_sel
                nom_final = _c_nom.strip() if _c_stock_sel == "— Saisie libre —" else _c_nom
                if nom_final:
                    _new_comps.append((nom_final, _c_cat, str(_c_qte), _c_unit, _c_four or ""))

            # ── Archives ────────────────────────────────────────────────────────
            st.markdown('<div style="font-family:DM Mono,monospace;font-size:9px;color:#8a7968;text-transform:uppercase;letter-spacing:.12em;margin:10px 0 4px;">Archives (optionnel)</div>', unsafe_allow_html=True)
            n_arc_n = st.number_input("Nombre d'archives", min_value=0, max_value=6, value=0, step=1, key="new_prod_n_arc")
            _new_arcs = []
            _ARC_TYPES = ["Photo","Sketch","Moodboard","Référence","Inspiration","Autre"]
            for _ai in range(int(n_arc_n)):
                _aa1,_aa2,_aa3,_aa4 = st.columns([2,2,1,3])
                with _aa1: _a_type = st.selectbox("Type", _ARC_TYPES, key=f"na_type_{_ai}")
                with _aa2: _a_nom  = st.text_input("Nom", key=f"na_nom_{_ai}")
                with _aa3: _a_ann  = st.number_input("Année", min_value=2000, max_value=2050, value=2025, step=1, key=f"na_ann_{_ai}")
                with _aa4: _a_det  = st.text_input("Détails / Lieu", key=f"na_det_{_ai}")
                if _a_nom.strip():
                    _new_arcs.append({"type":_a_type,"nom":_a_nom,"annee":int(_a_ann),"details":_a_det})
            ...
            if st.button("✓ Créer le produit", type="primary"):
                if not n_ref or not n_nom:
                    ...
                    try:
                        ...
                        conn.commit()
                        # Insérer les composants + étiquettes auto
                        if "_new_comps" in dir() and _new_comps:
                            _np = conn.execute("SELECT id FROM products WHERE ref=?", (n_ref,)).fetchone()
                            if _np:
                                for _nc in _new_comps:
                                    try:
                                        if isinstance(_nc, dict):
                                            _nc_nom,_nc_cat,_nc_qte,_nc_unit,_nc_four = (
                                                _nc.get("nom",""),_nc.get("cat",""),_nc.get("qte",0),
                                                _nc.get("unit","Pièces"),_nc.get("fournisseur",""))
                                        else:
                                            _nc_nom,_nc_cat,_nc_qte,_nc_unit = _nc[0],_nc[1],_nc[2],_nc[3]
                                            _nc_four = _nc[4] if len(_nc) > 4 else ""
                                        conn.execute("""INSERT INTO product_components
                                            (product_id,nom_exact,categorie_comp,quantite,unite,fournisseur)
                                            VALUES (?,?,?,?,?,?)""",
                                            (_np[0],_nc_nom,_nc_cat,float(_nc_qte),_nc_unit,_nc_four))
                                    except Exception: pass
                                conn.commit()
                        # Insérer les archives si saisies
                        if "_new_arcs" in dir() and _new_arcs:
                            _np2 = conn.execute("SELECT id FROM products WHERE ref=?", (n_ref,)).fetchone()
                            if _np2:
                                for _na in _new_arcs:
                                    try:
                                        conn.execute("""INSERT INTO product_archives
                                            (product_id,type_archive,nom_archive,annee,details)
                                            VALUES (?,?,?,?,?)""",
                                            (_np2[0],_na["type"],_na["nom"],_na.get("annee",0),_na.get("details","")))
                                    except Exception: pass
                                conn.commit()
        ...
        if df_pf_art.empty:
            ...
            with ga_t2:
                ...
                if not df_arc_ga.empty:
                    for _, arc in df_arc_ga.iterrows():
                        ...
                        with ag1:
                            ...
                            st.markdown(f"**{arc.get('type_archive','')}** · {_ann_str}{arc.get('nom_archive','') or arc.get('notes','') or arc.get('nom_fichier','')}"
                ...
                with st.expander("➕ Ajouter une archive"):
                    ...
                    arc_ann  = st.number_input("Année", min_value=2000, max_value=2050, value=2025, step=1, key=f"arc_ann_{pid_art}")
                    ...
                    if st.button("✓ Ajouter archive", key=f"arc_btn_{pid_art}"):
                        if arc_nom.strip() or arc_file:
                            ...
                            try:
                                conn.execute("""INSERT INTO product_archives
                                    (product_id,type_archive,nom_archive,annee,lieu,details,nom_fichier,fichier_data)
                                    VALUES (?,?,?,?,?,?,?,?)""",
                                    (pid_art, arc_type, arc_nom, int(arc_ann),
                                     arc_lieu, "", arc_fname, arc_bytes))
                                conn.commit(); st.rerun()
...
<div style="background:#f5f0e8;border-left:3px solid #7b506f;padding:8px 12px;margin:4px 0;">
  ...
  {'<div style="font-size:10px;color:#395f30;font-weight:500;">✓ Marketing appliqué</div>' if apply_mkt else '<div style="font-size:10px;color:#aaa;">Marketing non appliqué</div>'}
            ...
            # Afficher le total de la section
            if _sect_total > 0:
                st.markdown(
                    f'<div style="font-family:DM Mono,monospace;font-size:11px;font-weight:600;'
...
<div style="background:#f5f0e8;border-left:3px solid #395f30;padding:8px 14px;margin:8px 0;font-family:'DM Mono',monospace;font-size:11px;">
  ...
  {f" · Marge brute : <strong>{_marge_v}%</strong>" if _marge_v else ""}
        ...
        if st.form_submit_button(_submit_label, type="primary", disabled=not _couts_editable):
            ...
            conn.commit()
            st.success("✓ Coûts & prix synchronisés vers la fiche produit.")
            st.rerun()
