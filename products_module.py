# products_module.py — v5.15 — 1777320516
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    """Déclenche la migration si les données ne sont pas complètes."""
    try:
        n_stock = conn.execute("SELECT COUNT(*) FROM stock").fetchone()[0]
        n_comps = conn.execute("SELECT COUNT(*) FROM product_components").fetchone()[0]
        n_prods = conn.execute("SELECT COUNT(*) FROM products WHERE ref LIKE 'EWS%'").fetchone()[0]
        n_pkgs  = conn.execute("SELECT COUNT(*) FROM packaging_types").fetchone()[0]
        # Conditions strictes : 14 produits, 35+ stock, 90+ composants, 4 packagings
        if n_prods >= 14 and n_stock >= 35 and n_comps >= 90 and n_pkgs >= 4:
                ...
                for ci, (_, prod) in enumerate(df_coll.iterrows()):
                    ...
                    with cols_g[ci % 3]:
                        ...
                        _cats_order = ["MP Principale (Main Fabric)","MP Secondaire","Doublure",
                                       "Broderie","Peinture","Zip","Bouton","Étiquette"]
                        _lbl_map   = {"MP Principale (Main Fabric)":"MP principale",
                                      "MP Secondaire":"MP secondaire","Doublure":"Doublure",
                                      "Broderie":"Broderie","Peinture":"Peinture","Zip":"Zip",
                                      "Bouton":"Bouton","Étiquette":"Étiquettes"}
                        _by_cat = {}
                        ...
                        for _cat in _cats_order:
                            ...
                            _items = " · ".join(str(r.get("nom_exact","") or r.get("nom","")) for r in _by_cat[_cat])
                            _comp_rows += (
                                f'<div style="display:flex;gap:6px;padding:2px 0;border-bottom:0.5px solid #f0ece4;">'
                                f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#bbb;min-width:72px;text-transform:uppercase;padding-top:1px;">{_lbl_map.get(_cat,_cat)}</div>'
                                f'<div style="font-size:10px;color:#1a1a1a;line-height:1.4;">{_items}</div>'
                                f'</div>')

                        # Archives
                        try:
                            det_arcs = conn.execute(
                                "SELECT type_archive,nom_archive,annee FROM product_archives WHERE product_id=? ORDER BY annee DESC",
                                (pid,)).fetchall()
                        except Exception:
                            det_arcs = []
                        _arc_rows = ""
                        for arc in det_arcs:
                            _arc_rows += (
                                f'<div style="font-size:9px;color:#7b506f;padding:1px 0;">'
                                f'<span style="font-family:DM Mono,monospace;font-size:7px;color:#bbb;text-transform:uppercase;">{arc[0]}</span>'
                                f' {arc[1]}{(" — "+str(arc[2])) if arc[2] and arc[2]!=0 else ""}'
                                f'</div>')

                        # Infos inline
                        _infos_html = ""
                        for _lbl_i, _key_i in [("Made in","made_in"),("Tailles","tailles"),("Delivery","delivery")]:
                            _v = str(prod.get(_key_i,"") or "")
                            if _v:
                                _infos_html += (
                                    f'<span style="font-family:DM Mono,monospace;font-size:7px;color:#bbb;text-transform:uppercase;">{_lbl_i}</span>'
                                    f'<span style="font-size:9px;color:#1a1a1a;margin-right:12px;"> {_v}</span>')

                        # Description
                        _desc = str(prod.get("description","") or "")[:200]

                        st.markdown(
                            f'<div style="border:0.5px solid #d9c8ae;border-radius:6px;'
                            f'padding:14px;background:#fff;margin-bottom:8px;">'

                            # En-tête
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;margin-bottom:2px;">{prod.get("ref","")}</div>'
                            ...
                            # Description
                            + (f'<div style="font-size:10px;color:#555;font-style:italic;line-height:1.5;margin-bottom:8px;">{_desc}{"…" if len(str(prod.get("description","") or ""))>200 else ""}</div>' if _desc else "")

                            # Infos clés
                            + (f'<div style="margin-bottom:8px;">{_infos_html}</div>' if _infos_html else "")

                            # Composition
                            + (f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;text-transform:uppercase;letter-spacing:.1em;margin:6px 0 4px;border-top:0.5px solid #f0ece4;padding-top:6px;">Composition</div>{_comp_rows}' if _comp_rows else "")

                            # Archives
                            + (f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;text-transform:uppercase;letter-spacing:.1em;margin:6px 0 4px;border-top:0.5px solid #f0ece4;padding-top:6px;">Archives</div>{_arc_rows}' if _arc_rows else "")

                            + '</div>',
                            unsafe_allow_html=True)

                        if can_fn("finance_write"):
                            if st.button("✏️ Modifier", key=f"mod_{pid}", use_container_width=True):
