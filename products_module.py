# products_module.py — v5.13 — 1777319329
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def page_produits(can_fn, DB_PATH, fmt_eur_fn=None, fmt_jpy_fn=None):
    ...
    with tab_cat:
        ...
        with fc1:
            f_coll = st.selectbox("Collection", ["Toutes"] + COLLECTIONS_DYN, key="pcat_coll")
        with fc2:
            f_stat = st.selectbox("Statut", ["Tous"] + STATUTS_PROD, key="pcat_stat")
        with fc3:
            f_cat2 = st.selectbox("Catégorie", ["Toutes"] + CATEGORIES_PROD, key="pcat_cat")
        ...
        if df_prods.empty:
            ...
            # Grouper par collection
            colls_order = df_prods["collection"].dropna().unique().tolist()
            for coll in colls_order:
                df_coll = df_prods[df_prods["collection"] == coll]
                st.markdown(
                    f'<div style="font-family:DM Mono,monospace;font-size:8px;color:#8a7968;'
                    f'text-transform:uppercase;letter-spacing:.2em;margin:18px 0 8px;'
                ...
                for ci, (_, prod) in enumerate(df_coll.iterrows()):
                    with cols_g[ci % 4]:
                        ...
                        _sc   = {"Disponible":"#395f30","Sample & Testing":"#c9800a",
                                 "Out of stock":"#c1440e","Recherche":"#7b506f",
                                 "Archive":"#888"}.get(_stat,"#888")
                        _nom_affiche = prod["nom"] + (" — " + _coul if _coul else "")

                        _is_sel = (st.session_state.get("cat_detail_id") == int(prod["id"]))
                        btn_lbl = "✕" if _is_sel else "Voir →"

                        st.markdown(
                            f'<div style="border:0.5px solid #d9c8ae;border-radius:4px;'
                            f'padding:10px;background:{"#f0ece4" if _is_sel else "#fff"};'
                            f'margin-bottom:4px;">'
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;">{prod.get("ref","")}</div>'
                            f'<div style="font-size:12px;font-weight:600;color:#1a1a1a;line-height:1.3;margin:2px 0;">{_nom_affiche}</div>'
                            f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{_sc};">● {_stat}</div>'
                            f'</div>',
                            unsafe_allow_html=True)

                        def _toggle_detail(pid=int(prod["id"]), is_sel=_is_sel):
                            if is_sel:
                                st.session_state.pop("cat_detail_id", None)
                            else:
                                st.session_state["cat_detail_id"] = pid
                        st.button(btn_lbl, key=f"voir_{prod['id']}",
                                  type="primary" if _is_sel else "secondary",
                                  use_container_width=True,
                                  on_click=_toggle_detail)

        # ── PANNEAU DÉTAIL (dans le même with tab_cat) ───────────────────────
            ...
            if not df_det.empty:
                ...
                try:
                    det_arcs = pd.read_sql(
                        "SELECT type_archive,nom_archive,annee,details FROM product_archives WHERE product_id=? ORDER BY annee DESC",
                        conn, params=[_detail_pid])
                except Exception:
                    det_arcs = pd.DataFrame()

                st.markdown('<div style="border-top:2px solid #d9c8ae;margin:20px 0 12px;"></div>', unsafe_allow_html=True)

                # En-tête
                _coul2 = str(det.get("couleurs","") or det.get("variant","") or "")
                _titre = det["nom"] + (" — " + _coul2 if _coul2 else "")
                _sc2   = {"Disponible":"#395f30","Sample & Testing":"#c9800a",
                          "Out of stock":"#c1440e","Archive":"#888"}.get(str(det.get("statut","")),"#888")
                st.markdown(
                    f'<div style="margin-bottom:12px;">'
                    f'<div style="font-family:DM Mono,monospace;font-size:8px;color:#8a7968;">{det.get("ref","")} · {det.get("collection","")}</div>'
                    f'<div style="font-size:22px;font-weight:700;color:#1a1a1a;margin:3px 0;">{_titre}</div>'
                    f'<div style="font-family:DM Mono,monospace;font-size:9px;color:{_sc2};">● {det.get("statut","")}</div>'
                    f'</div>',
                    unsafe_allow_html=True)
                ...
                if det.get("description"):
                    st.markdown(
                        f'<div style="font-size:11px;color:#1a1a1a;line-height:1.7;'
                        f'font-style:italic;margin-bottom:14px;max-width:800px;">{det["description"]}</div>',
                        unsafe_allow_html=True)
                ...
                _rows = [
                    ...
                    ("MOQ",       f'{det.get("moq","")}' if det.get("moq") else ""),
                ...
                for _lbl, _val in _rows:
                    if _val and _val not in ("0 €",):
                        _info_html += (
                            f'<div style="display:inline-flex;gap:6px;align-items:baseline;'
                            f'margin-right:20px;margin-bottom:4px;">'
                            f'<span style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;text-transform:uppercase;">{_lbl}</span>'
                            f'<span style="font-size:11px;color:#1a1a1a;font-weight:500;">{_val}</span>'
                            f'</div>')
                if _info_html:
                    st.markdown(
                        f'<div style="background:#f5f0e8;border-radius:5px;padding:10px 14px;'
                        f'margin-bottom:14px;flex-wrap:wrap;">{_info_html}</div>',
                        unsafe_allow_html=True)
                ...
                if not det_comps.empty:
                    st.markdown(
                        f'<div style="font-family:DM Mono,monospace;font-size:8px;color:#8a7968;'
                        f'text-transform:uppercase;letter-spacing:.15em;margin:0 0 8px;'
                        f'border-bottom:1px solid #d9c8ae;padding-bottom:4px;">Composition</div>',
                        unsafe_allow_html=True)
                    _cats_order = ["MP Principale (Main Fabric)","MP Secondaire","Doublure",
                    ...
                    _comp_html = '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px;">'
                    for _cat in _cats_order + [c for c in _by_cat if c not in _cats_order]:
                        ...
                        _lbl_map = {
                            ...
                            "MP Secondaire":"Matière secondaire","Doublure":"Doublure",
                            "Broderie":"Broderie","Peinture":"Peinture","Zip":"Zip",
                            "Bouton":"Bouton","Étiquette":"Étiquettes","Packaging":"Packaging","Autre":"Autre"}
                        _cat_lbl = _lbl_map.get(_cat, _cat)
                        _comp_html += (
                            f'<div style="background:#fff;border:0.5px solid #d9c8ae;'
                            f'border-radius:5px;padding:8px 12px;min-width:150px;flex:1;">'
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;'
                            f'text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px;">{_cat_lbl}</div>')
                        for row in _by_cat[_cat]:
                            _cn = str(row.get("nom_exact","") or row.get("nom",""))
                            _cq = str(row.get("quantite","") or "")
                            _cu = str(row.get("unite","") or "")
                            _qs = f" — {_cq} {_cu}".strip() if _cq else ""
                            _comp_html += f'<div style="font-size:10px;color:#1a1a1a;padding:2px 0;line-height:1.4;">{_cn}{_qs}</div>'
                        _comp_html += '</div>'
                    _comp_html += '</div>'
                ...
                if not det_arcs.empty:
                    st.markdown(
                        f'<div style="font-family:DM Mono,monospace;font-size:8px;color:#8a7968;'
                        f'text-transform:uppercase;letter-spacing:.15em;margin:0 0 8px;'
                        f'border-bottom:1px solid #d9c8ae;padding-bottom:4px;">Archives</div>',
                        unsafe_allow_html=True)
                    _arc_html = '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px;">'
                    for _, arc in det_arcs.iterrows():
                        _an  = str(arc.get("nom_archive","") or "")
                        _ann = str(arc.get("annee","") or "")
                        _aty = str(arc.get("type_archive","") or "")
                        _ad  = str(arc.get("details","") or "")
                        _header = _an + (f" — {_ann}" if _ann and _ann != "0" else "")
                        _arc_html += (
                            f'<div style="background:#f5f0e8;border:0.5px solid #d9c8ae;'
                            f'border-radius:5px;padding:10px 12px;min-width:180px;flex:1;max-width:280px;">'
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#7b506f;'
                            f'text-transform:uppercase;margin-bottom:3px;">{_aty}</div>'
                            f'<div style="font-size:11px;font-weight:600;color:#1a1a1a;margin-bottom:4px;">{_header}</div>'
                            f'<div style="font-size:9px;color:#8a7968;line-height:1.5;">{_ad}</div>'
                            f'</div>')
                    _arc_html += '</div>'
                ...
                with _bc1:
                    def _close_panel():
                        st.session_state.pop("cat_detail_id", None)
                    st.button("✕ Fermer", key="det_close", on_click=_close_panel)
                with _bc2:
                    if can_fn("finance_write"):
                        def _goto_gestion(pid=_detail_pid):
                            st.session_state["gestion_art_presel_id"] = pid
                            st.session_state.pop("cat_detail_id", None)
                        st.button("✏️ Modifier →", key="det_goto_gestion", on_click=_goto_gestion)
