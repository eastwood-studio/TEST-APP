# products_module.py — v4.7 — 1776504942
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def generate_fiche_client(conn, product_id):
    ...
    # Construire la page avec deux colonnes
    story = []
    # En-tête
    story.append(Paragraph("EASTWOOD STUDIO", s_brand))
    story.append(Paragraph("PARIS · HANDCRAFTED IN FRANCE", s_sub))
    story.append(HRFlowable(width="100%", thickness=0.5, color=EW_S, spaceBefore=6, spaceAfter=4))

    # Image + texte en grille
    from reportlab.platypus import KeepInFrame
    txt_col_content = (
        [Paragraph(str(prod.get("collection","") or "").upper(), s_cat),
         Paragraph(str(prod.get("nom","") or ""), s_nom)] +
        ([Paragraph(str(prod.get("couleurs","")), s_var)] if prod.get("couleurs") else []) +
        ([Paragraph(str(prod.get("description","")), s_body)] if prod.get("description") else []) +
        [Spacer(1,3*mm), Paragraph("MATIÈRES", s_label)] +
        [Paragraph(m.strip(), s_val) for m in str(prod.get("matieres","")).split("\n") if m.strip()] +
        [Paragraph("RÉFÉRENCE", s_label),
         Paragraph(str(prod.get("ref","") or ""), s_val)] +
        ([Paragraph("ORIGINE", s_label), Paragraph(str(prod.get("made_in","")), s_val)] if prod.get("made_in") else []) +
        ([Paragraph("ARCHIVES & RÉFÉRENCES", s_label)] +
         [Paragraph(f"{str(arc.get('type_archive',''))} — {str(arc.get('notes','') or arc.get('nom_fichier',''))}", s_tag)
          for _, arc in df_arc.iterrows()] if not df_arc.empty else []) +
        [Spacer(1,6*mm),
         HRFlowable(width="100%", thickness=0.3, color=EW_S),
         Paragraph("www.eastwood-studio.fr  ·  contact@eastwood-studio.fr", s_foot)]
    )
    txt_frame = KeepInFrame(COL_TXT, H - 4*M, txt_col_content, mode="shrink")

    row = Table(
        [[cell_img, txt_frame]],
        colWidths=[COL_IMG + 4*mm, COL_TXT],
    )
...
<div style="margin-bottom:14px;">
  ...
  <div style="font-family:'DM Mono',monospace;font-size:11px;margin-top:4px;">
    WP : {fmt_eur_fn(prod.get('prix_wholesale_fr'))} · SRP : {fmt_eur_fn(prod.get('prix_retail_eur'))} · MOQ : {prod.get('moq','—')} units
            ...
            # Vue infos produit dans catalogue (lecture seule)
            st.markdown("---")
            ft1, ft2 = st.tabs(["🧵 Composition & Archives", "ℹ️ Informations"])
