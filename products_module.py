# products_module.py — v5.33 — 1778260564
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    ...
    try:
        ...
        # Tout est complet : vérifier aussi made_in et sous_type remplis
        try:
            _has_madein = c.execute(
                "SELECT COUNT(*) FROM stock WHERE made_in!='' AND made_in IS NOT NULL AND ref LIKE 'EWS%'"
            ).fetchone()[0]
        except Exception:
            _has_madein = 0
        if (n_prods >= 14 and n_comps >= 90 and n_pkgs >= 4
                and _chk_wool and _chk_comp and _chk_pf and _chk_pkg
                and _n_mp_comp >= 34 and n_pf_in_stock >= 14 and _has_madein >= 30):
            return
...
def _insert_only_stock(conn):
    """Insère tous les items stock avec toutes les infos du tableau inventaire."""
    import traceback
    try:
        c = conn.cursor()
        # Migrer les colonnes si nécessaire
        _sc = [r[1] for r in c.execute("PRAGMA table_info(stock)").fetchall()]
        for _col, _def in [("sous_type","TEXT DEFAULT ''"),("unite","TEXT DEFAULT 'Pièces'"),("made_in","TEXT DEFAULT ''")]:
            if _col not in _sc:
                try: c.execute(f"ALTER TABLE stock ADD COLUMN {_col} {_def}")
                except Exception: pass
        conn.commit()

        # ── Format: (ref, description, type_produit, sous_type, prix_unitaire, qte_stock, unite, made_in) ──
        ITEMS = [
            # ── Matières premières ──────────────────────────────────────────────
        ...
        for ref,nom,type_p,sous_t,cout,qte,unite,made_in in ITEMS:
            try:
                c.execute("""INSERT OR REPLACE INTO stock
                    (ref,description,type_produit,sous_type,prix_unitaire,qte_stock,unite,made_in)
                    VALUES (?,?,?,?,?,?,?,?)""",
                    (ref,nom,type_p,sous_t,cout,qte,unite,made_in))
            except Exception:
                try:
                    c.execute("""INSERT OR REPLACE INTO stock
                        (ref,description,type_produit,prix_unitaire,qte_stock)
                        VALUES (?,?,?,?,?)""",
                        (ref,nom,type_p,cout,qte))
                except Exception: pass
        conn.commit()

        # ── Produits finis ──────────────────────────────────────────────────────
        STOCK_PF = [
            # (ref, nom, qte_retail, qte_sample, detail)
            ...
            ("EWSJACKET-002A","Miura Jacket",                  0, 1, "1 sample"),
            ...
            ("EWSSHIRT-002A", "Lutèce Plage Shirt",            0, 1, "1 sample"),
            ("EWSGEAR-001A",  "Souvenir Cap Deep Rust",        11, 1, ""),
            ("EWSGEAR-001B",  "Souvenir Cap Faded Green",      11, 1, ""),
            ("EWSGEAR-002A",  "Paris le trésor Silk Square",   0,  1, "1 sample"),
        ...
        for ref_pf, nom_pf, qte_r, qte_s, detail in STOCK_PF:
            ...
            try:
                c.execute("""INSERT OR REPLACE INTO stock
                    (ref,description,type_produit,sous_type,qte_stock,unite,made_in,notes)
                    VALUES (?,?,'Produit fini','Produit fini',?,'Pièces','France',?)""",
                    (ref_pf, nom_pf, qte_r, notes_pf))
            except Exception:
                try:
                    c.execute("""INSERT OR REPLACE INTO stock
                        (ref,description,type_produit,qte_stock)
                        VALUES (?,?,'Produit fini',?)""",
                        (ref_pf, nom_pf, qte_r))
                except Exception: pass
        conn.commit()
