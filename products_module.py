# products_module.py — v5.24 — 1777369202
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
  ...
  <div style="text-align:right;">
    ...
    with st.form(f"costs_finance_{sel_pid}"):
        ...
        for sect, fields in COST_SECTIONS.items():
            ...
            # Dans MP & Composants : afficher le nom du composant depuis la fiche produit
            if sect == "MP & Composants":
                # Mapping colonne coût → catégorie composition
                _cost_to_cat = {
                ...
                n_cols_mp = min(len(fields), 3)
                fcols_mp = st.columns(n_cols_mp)
                _sect_total = 0.0
                for i, (key, label) in enumerate(fields):
                    # Trouver le nom du composant depuis la fiche produit
                    _cat_comp = _cost_to_cat.get(key)
                    _comp_name = ""
                    if _cat_comp and not _comps_prod.empty:
                        _matching = _comps_prod[_comps_prod["categorie_comp"]==_cat_comp]
                        if not _matching.empty:
                            _comp_name = " · ".join(_matching["nom_exact"].tolist())
                    _display_label = f"{label}"
                    if _comp_name:
                        _display_label = f"{label.split('(')[0].strip()} — {_comp_name} (€)"
                    with fcols_mp[i % n_cols_mp]:
                        val = float(costs_dict.get(key, 0) or 0)
                        all_inputs[key] = st.number_input(
                            _display_label, min_value=0.0, step=0.01, value=val,
    ...
    # ── Marketing (hors form — st.button autorisé) ──────────────────────────
    if sel_pid and _couts_editable:
        costs_mkt = get_costs(conn, sel_pid)
        _pct_cur = float(dict(costs_mkt).get("cout_marketing_pct", 5.0) or 5.0) if costs_mkt is not None else 5.0
        st.markdown(
            f'<div style="font-family:DM Mono,monospace;font-size:12px;font-weight:700;'
            f'letter-spacing:.12em;text-transform:uppercase;color:#1a1a1a;'
            f'border-bottom:2px solid #d9c8ae;padding:8px 0 4px;margin:14px 0 8px;">Marketing</div>',
            unsafe_allow_html=True)
        _mk1, _mk2, _mk3 = st.columns([2, 2, 3])
        with _mk1:
            _pct_new = st.number_input("Pourcentage marketing (%)",
                min_value=0.0, max_value=100.0, step=0.5, value=_pct_cur,
                key=f"mkt_pct_out_{sel_pid}")
        with _mk2:
            # Recalculer le total depuis la DB
            _costs_row = get_costs(conn, sel_pid)
            _total_base = compute_cost_totals(dict(_costs_row)).get("TOTAL", 0) if _costs_row is not None else 0
            _total_base = max(0, _total_base - float(dict(_costs_row).get("cout_marketing",0) or 0) if _costs_row else 0)
            _mkt_amt = round(_total_base * _pct_new / 100, 2)
            st.metric(f"Montant ({_pct_new:.0f}% du total)", fmt_eur_fn(_mkt_amt))
        with _mk3:
            st.markdown('<div style="margin-top:28px;"></div>', unsafe_allow_html=True)
            if st.button(f"✓ Appliquer {_pct_new:.0f}% marketing", key=f"mkt_apply_out_{sel_pid}"):
                try:
                    conn.execute("UPDATE product_costs SET cout_marketing_pct=?, cout_marketing=? WHERE product_id=?",
                                 (_pct_new, _mkt_amt, sel_pid))
                    conn.commit()
                    st.success(f"✓ {_pct_new:.0f}% appliqué → {fmt_eur_fn(_mkt_amt)}")
                    st.rerun()
