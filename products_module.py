# products_module.py — v4.5 — 1776324870
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def generate_fiche_client(conn, product_id):
    """Fiche technique client — document marketing pour les acheteurs finaux."""
    if not REPORTLAB_OK:
        return None, "reportlab non installé"
    ...
    s_brand  = ParagraphStyle("brand",  fontName="Helvetica-Bold", fontSize=20, textColor=EW_K, letterSpacing=5)
    s_sub    = ParagraphStyle("sub",    fontName="Helvetica", fontSize=7, textColor=EW_B, letterSpacing=8, spaceBefore=2)
    s_cat    = ParagraphStyle("cat",    fontName="Helvetica", fontSize=8, textColor=EW_V, letterSpacing=3, spaceBefore=8)
    s_nom    = ParagraphStyle("nom",    fontName="Helvetica-Bold", fontSize=22, textColor=EW_K, spaceBefore=4)
    s_var    = ParagraphStyle("var",    fontName="Helvetica-Oblique", fontSize=14, textColor=EW_B, spaceBefore=2)
    s_body   = ParagraphStyle("body",   fontName="Helvetica", fontSize=9, textColor=EW_K, leading=14, spaceBefore=6)
    s_label  = ParagraphStyle("label",  fontName="Helvetica-Bold", fontSize=7.5, textColor=EW_B, letterSpacing=1.5, spaceBefore=12)
    s_val    = ParagraphStyle("val",    fontName="Helvetica", fontSize=9, textColor=EW_K, leading=13)
    s_foot   = ParagraphStyle("foot",   fontName="Helvetica", fontSize=7, textColor=EW_B, alignment=TA_CENTER, spaceBefore=20)
    s_tag    = ParagraphStyle("tag",    fontName="Helvetica", fontSize=7.5, textColor=EW_V, letterSpacing=1)
    ...
    if not imgs.empty and imgs.iloc[0]["data"] is not None:
        try:
            img_io = io.BytesIO(bytes(imgs.iloc[0]["data"]))
            cell_img = RLImage(img_io, width=COL_IMG, height=COL_IMG*1.2, kind="proportional")
        except Exception:
            cell_img = None

    if cell_img is None:
        ph = Table([[""]], colWidths=[COL_IMG], rowHeights=[COL_IMG*1.2])
        ph.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),EW_C)]))
        cell_img = ph
    ...
    txt_elems = []
    txt_elems.append(Paragraph("EASTWOOD STUDIO", s_brand))
    txt_elems.append(Paragraph("PARIS · HANDCRAFTED IN FRANCE", s_sub))
    txt_elems.append(HRFlowable(width="100%", thickness=0.5, color=EW_S, spaceBefore=8, spaceAfter=2))
    txt_elems.append(Paragraph(str(prod.get("collection","") or "").upper(), s_cat))
    txt_elems.append(Paragraph(str(prod.get("nom","") or ""), s_nom))
    if prod.get("couleurs"):
        txt_elems.append(Paragraph(str(prod.get("couleurs","")), s_var))

    if prod.get("description"):
        txt_elems.append(Paragraph(str(prod.get("description","")), s_body))

    # Matières
    if prod.get("matieres"):
        txt_elems.append(Paragraph("MATIÈRES", s_label))
        mats = str(prod.get("matieres","")).split("\n")
        for m in mats:
            if m.strip():
                txt_elems.append(Paragraph(m.strip(), s_val))

    # SKU / Référence
    txt_elems.append(Paragraph("RÉFÉRENCE", s_label))
    txt_elems.append(Paragraph(str(prod.get("ref","") or ""), s_val))

    # Made in
    if prod.get("made_in"):
        txt_elems.append(Paragraph("ORIGINE", s_label))
        txt_elems.append(Paragraph(str(prod.get("made_in","")), s_val))

    # Archives utilisées
    if not df_arc.empty:
        txt_elems.append(Paragraph("ARCHIVES & RÉFÉRENCES", s_label))
        for _, arc in df_arc.iterrows():
            arc_txt = f"<b>{str(arc.get('type_archive',''))}</b> — {str(arc.get('notes','') or arc.get('nom_fichier',''))}"
            txt_elems.append(Paragraph(arc_txt, s_tag))

    # Séparateur + pied
    txt_elems.append(HRFlowable(width="100%", thickness=0.3, color=EW_S, spaceBefore=20, spaceAfter=6))
    txt_elems.append(Paragraph("www.eastwood-studio.fr  ·  contact@eastwood-studio.fr", s_foot))

    from reportlab.platypus import KeepInFrame
    txt_frame = KeepInFrame(COL_TXT, H - 2*M, txt_elems, mode="shrink")

    layout = Table(
        [[cell_img, txt_frame]],
        colWidths=[COL_IMG + 6*mm, COL_TXT],
        rowHeights=[H - 2*M]
    )
...
def generate_fiche_atelier(conn, product_id):
    ...
    if prod.get("description"):
        story.append(Paragraph(str(prod.get("description","")), s_body if hasattr(s_body,'name') else s_val))
    instr_rows = [
    ...
    WP : {fmt_eur_fn(prod.get('prix_wholesale_fr'))} · SRP : {fmt_eur_fn(prod.get('prix_retail_eur'))} · MOQ : {prod.get('moq','—')} units
                ...
                if can_fn("stock_write"):
                    with st.form(f"edit_prod_{pid}"):
                        ...
                        with ep2:
                            e_mat   = st.text_area("Matières (une par ligne)", value=prod.get("matieres","") or "", height=80, help="Ex:\n78% Rayon\n22% Linen")
                        ...
                        if st.form_submit_button("💾 Enregistrer"):
                            conn.execute("""UPDATE products SET
                                nom=?,ref=?,collection=?,statut=?,categorie=?,
                                ...
                                (e_nom,e_ref,e_coll,e_stat,e_cat,
...
<div style="border-left:2px solid {EW_S};padding:8px 14px;margin:6px 0;">
  ...
  {f'<div style="font-size:11px;color:{EW_B};margin-top:3px;">{arch["details"]}</div>' if arch.get("details") else ''}
    ...
    if tab_gestion is not None:
        with tab_gestion:
            st.markdown('<div class="section-title">Gestion des articles — Produits finis</div>', unsafe_allow_html=True)
            df_all_pf = get_products(conn, statut=None)
            df_pf_only = df_all_pf[df_all_pf["categorie"].notna()] if not df_all_pf.empty else pd.DataFrame()
            if df_pf_only.empty:
                st.info("Aucun produit.")
            else:
                sel_prod_g = st.selectbox("Sélectionner un article",
                    ["— Choisir —"] + [f"{p['ref']} — {p['nom']}" for _, p in df_pf_only.iterrows()],
                    key="gestion_art_sel")
                if sel_prod_g != "— Choisir —":
                    sel_idx_g = [f"{p['ref']} — {p['nom']}" for _, p in df_pf_only.iterrows()].index(sel_prod_g)
                    prod_g = df_pf_only.iloc[sel_idx_g]
                    pid_g = int(prod_g["id"])
                    with st.form(f"gestion_edit_{pid_g}"):
                        ga1, ga2 = st.columns(2)
                        with ga1:
                            ge_nom  = st.text_input("Nom", value=str(prod_g.get("nom","") or ""))
                            ge_ref  = st.text_input("SKU", value=str(prod_g.get("ref","") or ""))
                            ge_coll = st.selectbox("Collection", COLLECTIONS_DYN,
                                index=COLLECTIONS_DYN.index(prod_g.get("collection","")) if prod_g.get("collection") in COLLECTIONS_DYN else 0)
                            ge_stat = st.selectbox("Statut", STATUTS_PROD,
                                index=STATUTS_PROD.index(prod_g.get("statut","")) if prod_g.get("statut") in STATUTS_PROD else 0)
                            ge_cat  = st.selectbox("Catégorie", CATEGORIES_PROD,
                                index=CATEGORIES_PROD.index(prod_g.get("categorie","")) if prod_g.get("categorie") in CATEGORIES_PROD else 0)
                        with ga2:
                            ge_mat  = st.text_area("Matières", value=str(prod_g.get("matieres","") or ""), height=80)
                            ge_coul = st.text_input("Couleurs", value=str(prod_g.get("couleurs","") or ""))
                            ge_tail = st.text_input("Tailles",  value=str(prod_g.get("tailles","") or ""))
                            ge_made = st.text_input("Made in",  value=str(prod_g.get("made_in","") or ""))
                            ge_moq  = st.number_input("MOQ", value=int(prod_g.get("moq",0) or 0), min_value=0)
                            ge_del  = st.text_input("Delivery", value=str(prod_g.get("delivery","") or ""))
                        ge_desc = st.text_area("Description", value=str(prod_g.get("description","") or ""), height=70)
                        st.markdown("**Prix**")
                        gp1,gp2,gp3 = st.columns(3)
                        with gp1: ge_reu = st.number_input("Retail EU (€)", value=float(prod_g.get("prix_retail_eur",0) or 0), min_value=0.0)
                        with gp2: ge_weu = st.number_input("Wholesale EU (€)", value=float(prod_g.get("prix_wholesale_eu",0) or 0), min_value=0.0)
                        with gp3: ge_ff  = st.number_input("F&F (€)", value=float(prod_g.get("prix_ff",0) or 0), min_value=0.0)
                        if st.form_submit_button("💾 Enregistrer", type="primary"):
                            conn.execute("""UPDATE products SET
                                nom=?,ref=?,collection=?,statut=?,categorie=?,
                                description=?,matieres=?,couleurs=?,tailles=?,made_in=?,
                                moq=?,delivery=?,prix_retail_eur=?,prix_wholesale_eu=?,prix_ff=?
                                WHERE id=?""",
                                (ge_nom,ge_ref,ge_coll,ge_stat,ge_cat,
                                 ge_desc,ge_mat,ge_coul,ge_tail,ge_made,
                                 ge_moq,ge_del,ge_reu,ge_weu,ge_ff,pid_g))
                            conn.commit(); st.success("✓ Article mis à jour."); st.rerun()

                    # Supprimer
                    if can_fn("products_delete") and st.button("🗑 Supprimer ce produit", key=f"del_gestion_{pid_g}"):
                        conn.execute("DELETE FROM products WHERE id=?", (pid_g,))
                        conn.commit(); st.rerun()
    ...
    # ── EXPORT DOCUMENTS ───────────────────────────────────────────────────────
    # ── GESTION DES ARTICLES (produits finis uniquement) ──────────────────────
    with tab_gestion_art:
        st.markdown('<div class="section-title">Gestion des articles — Produits finis</div>', unsafe_allow_html=True)
        ...
        if df_pf_art.empty:
            st.info("Aucun produit fini disponible.")
        else:
            sel_art_ref = st.selectbox(
                "Sélectionner un article",
                options=[f"{r['ref']} — {r['nom']} / {r.get('couleurs','')}" for _, r in df_pf_art.iterrows()],
                key="gestion_art_sel"
            )
            ...
            if new_img:
                img_b64 = __import__("base64").b64encode(new_img.read()).decode()
                conn.execute("""INSERT INTO product_images (product_id,nom_fichier,image_data,is_main)
                    VALUES (?,?,?,0)""", (pid_art, new_img.name, img_b64))
