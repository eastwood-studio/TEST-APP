# products_module.py — v5.11 — 1777318197
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    """Déclenche la migration si les données v9 ne sont pas en place.
    Condition : EWSBTN-001A dans stock (ref unique à la migration v9).
    """
    try:
        # EWSBTN-001A est le ref bouton v9 — absent dans toutes les versions précédentes
        btn_ok = conn.execute(
            "SELECT COUNT(*) FROM stock WHERE ref='EWSBTN-001A'"
        ).fetchone()
        # EWSJACKET-001B doit avoir couleur = 'Tobacco' (pas 'Black')
        col_ok = conn.execute(
            "SELECT COUNT(*) FROM products WHERE ref='EWSJACKET-001B' AND couleurs='Tobacco'"
        ).fetchone()
        # 98 composants minimum
        comp_ok = conn.execute(
            "SELECT COUNT(*) FROM product_components"
        ).fetchone()
        if (btn_ok and btn_ok[0] >= 1 and
                col_ok and col_ok[0] >= 1 and
                comp_ok and comp_ok[0] >= 90):
            return  # Migration déjà correcte
    except Exception:
        pass
                    ...
                    with cols[i % 4]:
                        ...
                        if not imgs.empty and imgs.iloc[0]["data"] is not None:
                            try:
                                ...
                                st.markdown(f'<div style="aspect-ratio:1/1;background:{EW_C};display:flex;align-items:center;justify-content:center;margin-bottom:4px;"><span style="font-family:\'DM Mono\',monospace;font-size:9px;color:{EW_B};">VISUEL</span></div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div style="aspect-ratio:1/1;background:{EW_C};display:flex;align-items:center;justify-content:center;margin-bottom:4px;"><span style="font-family:\'DM Mono\',monospace;font-size:9px;color:{EW_B};">VISUEL</span></div>', unsafe_allow_html=True)

                        stat_c = {"Disponible":EW_G,"Sample & Testing":"#c9800a","Recherche":EW_V,
                                  "Out of stock":"#c1440e","Archive":"#555","Concept":"#888"}.get(prod.get("statut",""),"#888")

                        # Couleur variant (colonne couleurs)
                        ...
                        st.markdown(
                            ...
                            f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};">{prod.get("ref","")}</div>'
                            f'<div style="font-size:12px;font-weight:600;color:#1a1a1a;margin:1px 0;line-height:1.3;">{prod["nom"]}'
                            f'{(" — " + _coul) if _coul else ""}</div>'
                            f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{stat_c};margin-top:2px;">● {prod.get("statut","")}</div>'
                            f'{badge_cout}</div>',
    ...
    if _detail_pid and tab_cat:
        with tab_cat:
            ...
            if not df_det.empty:
                ...
                try:
                    det_arcs = pd.read_sql(
                        "SELECT type_archive, nom_archive, annee, details, lieu FROM product_archives WHERE product_id=? ORDER BY annee DESC, id",
                        conn, params=[_detail_pid])
                except Exception:
                    det_arcs = pd.DataFrame()

                st.markdown(f'<div style="border-top:2px solid #d9c8ae;margin:16px 0 12px;padding-top:14px;"><span style="font-family:DM Mono,monospace;font-size:8px;color:#8a7968;letter-spacing:.2em;text-transform:uppercase;">Détail produit</span></div>', unsafe_allow_html=True)

                # ── En-tête : nom + variant ──────────────────────────────────
                _nom_variant = str(det.get("nom","") or "")
                _coul_variant = str(det.get("couleurs","") or "")
                _titre_complet = f"{_nom_variant} — {_coul_variant}" if _coul_variant else _nom_variant
                _stat_c = {"Disponible":EW_G,"Sample & Testing":"#c9800a","Recherche":EW_V,
                           "Out of stock":"#c1440e","Archive":"#888"}.get(str(det.get("statut","")),"#888")

                st.markdown(
                    f'<div style="margin-bottom:14px;">'
                    f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};letter-spacing:.1em;">'
                    f'{det.get("ref","")} · {det.get("collection","")}</div>'
                    f'<div style="font-size:22px;font-weight:600;color:#1a1a1a;margin:3px 0 2px;">{_titre_complet}</div>'
                    f'<div style="font-family:DM Mono,monospace;font-size:9px;color:{_stat_c};">● {det.get("statut","")}</div>'
                    f'</div>',
                    unsafe_allow_html=True)
                ...
                with d_left:
                    ...
                    if not det_imgs.empty and det_imgs.iloc[0]["data"] is not None:
                        try:
                            ...
                            st.markdown(f'<img src="data:{_dmime};base64,{_db64}" style="width:100%;border-radius:3px;"/>', unsafe_allow_html=True)
                        except Exception:
                ...
                with d_right:
                    ...
                    if det.get("description"):
                        st.markdown(f'<div style="font-size:11px;color:#1a1a1a;line-height:1.7;margin-bottom:10px;font-style:italic;">{det["description"]}</div>', unsafe_allow_html=True)

                    # Infos principales
                    _infos = [
                        ("Product type", "Produit fini"),
                        ("Statut", str(det.get("statut","") or "")),
                        ("Collection", str(det.get("collection","") or "")),
                        ("Made in", str(det.get("made_in","") or "")),
                        ("Tailles", str(det.get("tailles","") or "")),
                        ("Delivery", str(det.get("delivery","") or "")),
                        ("MOQ", f"{det.get('moq','—')} units" if det.get("moq") else ""),
                        ("SRP EU", f"{det.get('prix_retail_eur',0) or 0:.0f} €" if det.get("prix_retail_eur") else ""),
                        ("WP EU", f"{det.get('prix_wholesale_eu',0) or 0:.0f} €" if det.get("prix_wholesale_eu") else ""),
                    ]
                    _info_html = ""
                    for _lbl, _val in _infos:
                        if _val and _val not in ("0 €",""):
                            _info_html += (
                                f'<div style="display:flex;gap:10px;padding:4px 0;border-bottom:0.5px solid #ede3d3;">'
                                f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};min-width:80px;text-transform:uppercase;padding-top:1px;">{_lbl}</div>'
                                f'<div style="font-size:11px;color:#1a1a1a;">{_val}</div>'
                                f'</div>')
                    st.markdown(_info_html, unsafe_allow_html=True)

                # ── COMPOSITION — affiché sous les deux colonnes, par catégorie ──
                if not det_comps.empty:
                    st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;letter-spacing:.15em;margin:14px 0 6px;border-bottom:1px solid #d9c8ae;padding-bottom:4px;">Composition</div>', unsafe_allow_html=True)

                    # Grouper par catégorie dans l'ordre logique
                    _cat_order = ["MP Principale (Main Fabric)","MP Secondaire","Doublure","Broderie","Peinture",
                                  "Zip","Bouton","Étiquette","Packaging","Autre"]
                    _comp_by_cat = {}
                    for _, row in det_comps.iterrows():
                        cat = str(row.get("categorie_comp","") or "Autre")
                        _comp_by_cat.setdefault(cat, []).append(row)

                    _comp_html = '<div style="display:flex;flex-wrap:wrap;gap:6px;">'
                    for _cat in _cat_order:
                        if _cat not in _comp_by_cat:
                            continue
                        _cat_label = {
                            "MP Principale (Main Fabric)": "Matière principale",
                            "MP Secondaire": "Matière secondaire",
                            "Doublure": "Doublure",
                            "Broderie": "Broderie",
                            "Peinture": "Peinture",
                            "Zip": "Zip",
                        ...
                        _comp_html += (
                            f'<div style="background:#fff;border:0.5px solid #d9c8ae;border-radius:5px;padding:8px 12px;min-width:160px;flex:1;">'
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;color:{EW_B};text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px;">{_cat_label}</div>')
                        for row in _comp_by_cat[_cat]:
                            _cn = str(row.get("nom_exact") or row.get("nom",""))
                            _cq = str(row.get("quantite","") or "")
                            _cu = str(row.get("unite","") or "")
                            _cr = str(row.get("ref_stock","") or "")
                            _qte_str = f" — {_cq} {_cu}".strip(" —") if _cq else ""
                            _ref_str = f'<span style="color:#bbb;font-size:8px;"> ({_cr})</span>' if _cr else ""
                            _comp_html += f'<div style="font-size:10px;color:#1a1a1a;padding:2px 0;line-height:1.4;">{_cn}{_qte_str}{_ref_str}</div>'
                        _comp_html += '</div>'

                    # Ajouter les catégories non listées
                    for _cat, _rows in _comp_by_cat.items():
                        if _cat not in _cat_order:
                            _comp_html += (
                                f'<div style="background:#fff;border:0.5px solid #d9c8ae;border-radius:5px;padding:8px 12px;min-width:160px;flex:1;">'
                                f'<div style="font-family:DM Mono,monospace;font-size:7px;color:{EW_B};text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px;">{_cat}</div>')
                            for row in _rows:
                                _cn = str(row.get("nom_exact") or row.get("nom",""))
                                _cq = str(row.get("quantite","") or "")
                                _cu = str(row.get("unite","") or "")
                                _qte_str = f" — {_cq} {_cu}".strip(" —") if _cq else ""
                                _comp_html += f'<div style="font-size:10px;color:#1a1a1a;padding:2px 0;">{_cn}{_qte_str}</div>'
                            _comp_html += '</div>'
                    _comp_html += '</div>'
                ...
                if not det_arcs.empty:
                    st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_B};text-transform:uppercase;letter-spacing:.15em;margin:14px 0 6px;border-bottom:1px solid #d9c8ae;padding-bottom:4px;">Archives</div>', unsafe_allow_html=True)
                    _arc_html = '<div style="display:flex;flex-wrap:wrap;gap:6px;">'
                    for _, arc in det_arcs.iterrows():
                        _arc_nom  = str(arc.get("nom_archive","") or arc.get("notes","") or "")
                        _arc_ann  = str(arc.get("annee","") or "")
                        _arc_type = str(arc.get("type_archive","") or "")
                        _arc_det  = str(arc.get("details","") or arc.get("lieu","") or "")
                        _arc_header = f"{_arc_nom}{' — ' + _arc_ann if _arc_ann and _arc_ann != '0' else ''}"
                        _arc_html += (
                            f'<div style="background:{EW_C};border:0.5px solid #d9c8ae;border-radius:5px;padding:10px 12px;min-width:200px;flex:1;max-width:300px;">'
                            f'<div style="font-family:DM Mono,monospace;font-size:8px;color:{EW_V};text-transform:uppercase;margin-bottom:3px;">{_arc_type}</div>'
                            f'<div style="font-size:11px;font-weight:600;color:#1a1a1a;margin-bottom:4px;">{_arc_header}</div>'
                            f'<div style="font-size:9px;color:{EW_B};line-height:1.5;">{_arc_det}</div>'
                            f'</div>')
                    _arc_html += '</div>'
                    st.markdown(_arc_html, unsafe_allow_html=True)
