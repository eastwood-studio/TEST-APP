# products_module.py — v5.17 — 1777321660
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
            ...
            for coll in df_prods["collection"].dropna().unique():
                ...
                for ci, (_, prod) in enumerate(df_coll.iterrows()):
                    ...
                    for _, row in det_comps.iterrows():
                        _c = str(row.get("categorie_comp","") or "Autre")
                        _by_cat.setdefault(_c, []).append(row)
                    ...
                    for _cat in _cats_order + [c for c in _by_cat if c not in _cats_order]:
                        ...
                        for r in _by_cat[_cat]:
                            ...
                            _items_html += f'<div style="font-size:10px;color:#1a1a1a;padding:1px 0;">{_cn}{_qs}</div>'
                        _comp_html += (
                            f'<div style="margin-bottom:6px;">'
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;color:#8a7968;'
                            f'text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px;">{_cat_lbl}</div>'
                            f'{_items_html}</div>')
                    ...
                    _arc_html = ""
                    for arc in arcs:
                        _ann = str(arc[2]) if arc[2] and arc[2] != 0 else ""
                        _det = str(arc[3] or "")[:120]
                        _arc_html += (
                            f'<div style="margin-bottom:5px;">'
                            f'<div style="font-size:9px;font-weight:600;color:#7b506f;">'
                            f'{arc[1]}{(" — " + _ann) if _ann else ""}'
                            f'<span style="font-family:DM Mono,monospace;font-size:7px;'
                            f'color:#bbb;margin-left:6px;text-transform:uppercase;">{arc[0]}</span>'
                            f'</div>'
                            f'<div style="font-size:9px;color:#8a7968;line-height:1.4;">{_det}</div>'
                            f'</div>')

                    # Infos
                    _infos = []
                    for _lbl_i, _key_i in [("Made in","made_in"),("Tailles","tailles"),
                                           ("Delivery","delivery"),("MOQ","moq")]:
                        _v = str(prod.get(_key_i,"") or "")
                        if _v:
                            _infos.append(
                                f'<span style="font-family:DM Mono,monospace;font-size:7px;'
                                f'color:#bbb;text-transform:uppercase;">{_lbl_i}</span>'
                                f'<span style="font-size:9px;color:#1a1a1a;margin-right:12px;"> {_v}</span>')
                    _infos_html = "".join(_infos)

                    # Prix
                    _prix_html = ""
                    srp = prod.get("prix_retail_eur",0) or 0
                    wp  = prod.get("prix_wholesale_eu",0) or 0
                    if srp or wp:
                        _prix_html = (
                            f'<div style="margin-top:8px;padding-top:6px;border-top:0.5px solid #f0ece4;">'
                            + (f'<span style="font-family:DM Mono,monospace;font-size:7px;color:#bbb;text-transform:uppercase;">SRP</span>'
                               f'<span style="font-size:10px;color:#1a1a1a;margin-right:12px;"> {srp:.0f} €</span>' if srp else "")
                            + (f'<span style="font-family:DM Mono,monospace;font-size:7px;color:#bbb;text-transform:uppercase;">WP</span>'
                               f'<span style="font-size:10px;color:#1a1a1a;"> {wp:.0f} €</span>' if wp else "")
                            + '</div>')
                    ...
                    with cols_g[ci % 3]:
                        html = (
                            ...
                            # En-tête
                            f'<div style="font-family:DM Mono,monospace;font-size:7px;'
                            f'color:#8a7968;margin-bottom:2px;">{_ref}</div>'
                            f'<div style="font-size:14px;font-weight:700;color:#1a1a1a;'
                            f'margin-bottom:2px;">{_nom}</div>'
                            f'<div style="font-family:DM Mono,monospace;font-size:8px;'
                            f'color:{_sc};margin-bottom:10px;">● {_stat}</div>'
                        )
                        ...
                        if _desc:
                            html += (
                                ...
                                f'line-height:1.6;margin-bottom:10px;padding-bottom:8px;'
                                f'border-bottom:0.5px solid #f0ece4;">{_desc}</div>')

                        # Infos clés
                        if _infos_html:
                            html += f'<div style="margin-bottom:10px;">{_infos_html}</div>'

                        # Composition
                        if _comp_html:
                            html += (
                                f'<div style="border-top:0.5px solid #f0ece4;padding-top:8px;'
                                f'margin-bottom:8px;">'
                                f'<div style="font-family:DM Mono,monospace;font-size:7px;'
                                f'color:#8a7968;text-transform:uppercase;letter-spacing:.1em;'
                                f'margin-bottom:6px;">Composition</div>'
                                f'{_comp_html}</div>')
                        ...
                        if _arc_html:
                            html += (
                                ...
                                f'<div style="font-family:DM Mono,monospace;font-size:7px;'
                                f'color:#8a7968;text-transform:uppercase;letter-spacing:.1em;'
                                f'margin-bottom:6px;">Archives</div>'
                                f'{_arc_html}</div>')
