# products_module.py — v5.9 — 1777299642
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    """Vérifie les SKU réels — relance si données absentes ou incomplètes."""
    try:
        prod_ok = conn.execute(
            "SELECT COUNT(*) FROM products WHERE ref IN ('EWSJACKET-001A','EWSJACKET-001B','EWSSHIRT-001A','EWSGEAR-001A')"
        ).fetchone()
        stk_ok = conn.execute(
            "SELECT COUNT(*) FROM stock WHERE ref IN ('EWSWOOL-001A','EWSBTN-003A','EWSETQ-002A')"
        ).fetchone()
        pkg_ok = conn.execute(
            "SELECT COUNT(*) FROM packaging_types WHERE nom='Packaging Chapter I'"
        ).fetchone()
        if (prod_ok and prod_ok[0] >= 4 and
                stk_ok and stk_ok[0] >= 3 and
                pkg_ok and pkg_ok[0] >= 1):
            return
    except Exception:
        pass
