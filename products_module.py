        ...
        for p in PRODUCTS_SEED:
            c.execute("""INSERT OR IGNORE INTO products
                (ref, internal_ref, nom, variant, categorie, collection, statut,
                 ...
                 cost_eur, prix_wholesale_fr, prix_retail_eur, moq, delivery, origine)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (p["ref"], p["internal_ref"], p["nom"], p["variant"],
                 p["categorie"], p["collection"], p["statut"],
                 p["description"], p["matieres"], p["couleurs"],
                 p["tailles"], p.get("made_in",""),
                 p["cost_eur"], p["prix_wholesale_fr"], p["prix_retail_eur"],
                 p.get("moq",0), p.get("delivery",""), p.get("origine","")))
...
COST_SECTIONS = {
    ...
    "Étiquettes": [
        ...
        ("cout_etiq_fab_fr", "Étiquette Fab. Française"),
...
<div style="margin-bottom:10px;">
  ...
  {f'<div style="font-size:11px;color:{EW_B};font-style:italic;">{prod["variant"]}</div>' if prod.get("variant") else ''}
  <div style="font-family:'DM Mono',monospace;font-size:9px;color:{stat_c};">● {prod.get('statut','')}</div>
  {f'<div style="font-size:10px;color:{EW_B};">WP {fmt_eur_fn(prod.get("prix_wholesale_eu") or prod.get("prix_wholesale_fr"))} · SRP {fmt_eur_fn(prod["prix_retail_eur"])}</div>' if prod.get("prix_retail_eur") else ''}
  {badge_cout}
...
<div style="border-left:2px solid {EW_S};padding:8px 14px;margin:6px 0;">
  ...
  {f'<div style="font-size:11px;color:{EW_B};margin-top:3px;">{arch["details"]}</div>' if arch.get("details") else ''}
    ...
    # ── FICHE TECHNIQUE ────────────────────────────────────────────────────────
    with tab_fiche:
        st.markdown(f'<div class="section-title">Générer une fiche technique</div>', unsafe_allow_html=True)
        st.markdown("""
<div class="info-box">
Format A4 paysage · Style dossier confidentiel typewriter · Visuel à gauche · Composition et archives à droite
</div>""", unsafe_allow_html=True)
        ...
        if df_all_p.empty:
            ...
            prod_opts = {f"{r['ref']} — {r['nom']} / {r['variant']}": r["id"]
                        for _, r in df_all_p.iterrows()}
            sel_ft = st.selectbox("Produit", list(prod_opts.keys()), key="ft_sel")
            sel_ft_id = prod_opts[sel_ft]

            if st.button("📄 Générer la fiche technique", key="gen_ft"):
                with st.spinner("Génération..."):
                    pdf_b, err = generate_fiche_technique(conn, sel_ft_id)
                if err:
                    st.error(f"Erreur : {err}")
                elif pdf_b:
                    ref_slug = sel_ft.split(" — ")[0].replace("/","_")
                    st.download_button("⬇ Télécharger la fiche technique",
                                       pdf_b, file_name=f"fiche_technique_{ref_slug}.pdf",
