# products_module.py — v5.1 — 1776858818
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
  ...
  <div style="font-size:11px;color:#1a1a1a;">{_fv}</div>
    ...
    if tab_new is not None:
        with tab_new:
            ...
            n_comp_n = st.number_input("Nombre de composants", min_value=0, max_value=8, value=0, step=1, key="new_prod_n_comp")
            _new_comps = []
            ...
            for _ci in range(int(n_comp_n)):
                _nc1,_nc2,_nc3,_nc4 = st.columns([2,2,1,1])
                with _nc1: _c_nom = st.text_input(f"Composant {_ci+1}", placeholder="ex: Whipcord Wool", key=f"nc_nom_{_ci}")
                with _nc2: _c_cat = st.selectbox("Catégorie", _CATS_COMP_N, key=f"nc_cat_{_ci}")
                with _nc3: _c_qte = st.number_input("Qté", min_value=0.0, step=0.5, key=f"nc_qte_{_ci}")
                with _nc4: _c_unit = st.selectbox("Unité", ["Mètre","Pièces","kg","g","m²"], key=f"nc_unit_{_ci}")
                if _c_nom.strip():
                    _new_comps.append({"nom":_c_nom.strip(),"cat":_c_cat,"qte":_c_qte,"unit":_c_unit})
            ...
            if st.button("✓ Créer le produit", type="primary"):
                if not n_ref or not n_nom:
                    ...
                    try:
                        ...
                        conn.commit()
                        # Insérer les composants si saisis
                                ...
                                for _nc in _new_comps:
                                    try:
                                        conn.execute("""INSERT INTO product_components
                                            (product_id,nom_exact,categorie_comp,quantite,unite)
                                            VALUES (?,?,?,?,?)""",
                                            (_np[0],_nc["nom"],_nc["cat"],float(_nc["qte"]),_nc["unit"]))
                                    except Exception: pass
                                conn.commit()
