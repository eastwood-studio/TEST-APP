# products_module.py — v5.31 — 1777653188
# ══════════════════════════════════════════════════════════════════════════════
# MODULE PRODUITS v3
...
def run_data_migration(conn):
    ...
    try:
        ...
        # Vérifier des refs précises en plus des compteurs
        try:
        ...
        # Tout est complet → rien à faire
        if n_prods >= 14 and n_comps >= 90 and n_pkgs >= 4 and n_pf_in_stock >= 14 and _chk_wool and _chk_pf:
            return
        ...
        # STOCK manquant OU items spécifiques absents → insérer le stock
        # Vérifier des refs précises qui ne peuvent exister que si la migration a tourné
        try:
            _has_wool  = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSWOOL-001A'").fetchone()[0]
            _has_btn   = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSBTN-003A'").fetchone()[0]
            _has_pf    = c.execute("SELECT COUNT(*) FROM stock WHERE ref='EWSJACKET-001A' AND type_produit='Produit fini'").fetchone()[0]
        except Exception:
            _has_wool = _has_btn = _has_pf = 0

        if not _has_wool or not _has_btn or not _has_pf or n_pf_in_stock < 14:
            try:
...
def _insert_only_stock(conn):
    ...
    try:
        ...
        # Stock produits finis (retail) + samples
        STOCK_PF = [
            # (ref, qte_retail, qte_sample, detail_tailles)
            ("EWSJACKET-001A", 0,  0,  ""),
            ("EWSJACKET-001B", 1,  0,  "1×T2"),
            ("EWSJACKET-001C", 0,  0,  ""),
            ("EWSJACKET-001D", 0,  0,  ""),
            ("EWSJACKET-001E", 0,  0,  ""),
            ("EWSJACKET-002A", 0,  1,  ""),
            ("EWSJACKET-003A", 0,  0,  ""),
            ("EWSSHIRT-001A",  3,  1,  "2×T2 + 1×T1"),
            ("EWSSHIRT-001B",  3,  1,  "1×T1 + 1×T2 + 1×T3"),
            ("EWSSHIRT-002A",  0,  1,  ""),
            ("EWSGEAR-001A",   11, 1,  ""),
            ("EWSGEAR-001B",   11, 1,  ""),
            ("EWSGEAR-002A",   0,  1,  ""),
        ...
        for sku, qte_retail, qte_sample, detail in STOCK_PF:
            row = c.execute("SELECT nom,couleurs FROM products WHERE ref=?", (sku,)).fetchone()
            if not row: continue
            desc_base = row[0] + (" — " + row[1] if row[1] else "")
            # Stock retail (Produit fini)
            try:
                desc_retail = desc_base + (f" ({detail})" if detail else "")
                c.execute("INSERT OR REPLACE INTO stock (ref,description,type_produit,qte_stock,unite) VALUES (?,?,'Produit fini',?,'Pièces')",
                    (sku, desc_retail, qte_retail))
            except Exception: pass
            # Stock sample
            if qte_sample > 0:
                ref_sample = sku + "-SAMPLE"
                try:
                    c.execute("INSERT OR REPLACE INTO stock (ref,description,type_produit,sous_type,qte_stock,unite) VALUES (?,?,'Sample','Sample',?,'Pièces')",
                        (ref_sample, desc_base + " — Sample", qte_sample))
                except Exception: pass
        conn.commit()
    except Exception:
        traceback.print_exc()
