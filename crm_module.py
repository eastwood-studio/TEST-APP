...
def page_crm(can_fn, DB_PATH):
    ...
    with tab_pipe:
        ...
        if df_all.empty:
            ...
            total_pipeline = len(df_all[~df_all["statut_pipeline"].isin(["Perdu","On hold"])])
            converted = len(df_all[df_all["statut_pipeline"]=="Commande wholesale"])
            taux_conv = round(converted/total_pipeline*100) if total_pipeline > 0 else 0
            df_relances = df_all[df_all["date_prochaine_action"].notna() & (df_all["date_prochaine_action"] != "")]
            relances_dues = 0
            try:
                from datetime import date as _d
                for _, r in df_relances.iterrows():
                    try:
                        if _d.fromisoformat(str(r["date_prochaine_action"])[:10]) <= _d.today():
                            relances_dues += 1
                    except Exception:
                        pass
            except Exception:
                pass
            with k5: st.metric("Taux conversion", f"{taux_conv}%")
            with k6: st.metric("En négociation", len(df_all[df_all["statut_pipeline"]=="Négociation"]))
            with k7: st.metric("⚡ Relances dues", relances_dues, delta=None if relances_dues==0 else f"{relances_dues} urgentes")
            with k8:
                ca_potentiel = df_all["potentiel_annuel"].sum() if "potentiel_annuel" in df_all.columns else 0
                st.metric("Potentiel annuel", fmt_eur(ca_potentiel))
...
<div style="border-left:2px solid {EW_S};padding:6px 12px;margin:6px 0;">
  ...
  {f'<div style="font-size:11px;color:{EW_V};margin-top:1px;">Prochaine étape : {inter["prochaine_etape"]}</div>' if inter.get('prochaine_etape') else ''}
                        ...
                        # Ajouter interaction — accessible à tous (Jules, Corentin, Alexis)
                        if can_fn("contacts_read"):  # tous les utilisateurs connectés
