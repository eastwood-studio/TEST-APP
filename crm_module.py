...
<div style="border:0.5px solid {EW_S};padding:10px 16px;margin:6px 0;background:#fff;">
  <div style="display:flex;justify-content:space-between;">
    ...
    if not df_export.empty:
        st.markdown("---")
        buf_crm = io.BytesIO()
        with pd.ExcelWriter(buf_crm, engine="openpyxl") as writer:
            df_export.drop(columns=["contact_id"],errors="ignore").to_excel(
                writer, sheet_name="Comptes CRM", index=False)
            get_wholesale(conn).to_excel(writer, sheet_name="Commandes wholesale", index=False)
        st.download_button(
            "⬇ Export CRM Excel",
            buf_crm.getvalue(),
            file_name=f"crm_eastwood_{date.today()}.xlsx",
